...
describe("ControlConnection", function () {
  ...
  describe("#init()", function () {
    ...
    it("should resolve IPv4 and IPv6 addresses, default host (localhost) and port", () => {
      const ControlConnectionMock = proxyquire("../../lib/control-connection", {
        dns: {
    ...
    it("should listen to socketClose and reconnect", async () => {
      const state = {};
      const peersRows = [{ rpc_address: types.InetAddress.fromString("::2") }];

      const lbp = new policies.loadBalancing.RoundRobinPolicy();

      const cc = newInstance(
        { contactPoints: ["::1", "::2"], policies: { loadBalancing: lbp } },
        getContext({
          state,
  ...
  describe("#getAddressForPeerHost()", function () {
    it("should handle null, 0.0.0.0 and valid addresses", async () => {
      ...
      assert.strictEqual(
        await cc.getAddressForPeerHost({ rpc_address: null, peer }, 9042),
        null,
      );
    });
  ...
  describe("#refresh()", function () {
    it("should schedule reconnection when it cant borrow a connection", async () => {
      ...
      lbp.newQueryPlan = function (ks, o, cb) {
        ...
        return cb(
          null,
          [lbp.hosts.values()[1], lbp.hosts.values()[0]][Symbol.iterator](),
        );
      };
      ...
      const cc = newInstance(
        ...
        getContext({
          state: state,
          queryResults: {
            peers: [{ rpc_address: types.InetAddress.fromString("::2") }],
          },
          failBorrow: [-1, 1],
        }),
      );
