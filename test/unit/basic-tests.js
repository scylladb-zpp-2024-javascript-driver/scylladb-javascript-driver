    ...
    it("should return a valid number for int greater than 2^53 and less than -2^53", function () {
      [
        ...
        assert.ok(
          ...
          util.format("Value should be greater than 2^53 for %s", item),
        );
      });
      [new Long(0, 0xf0000000), new Long(0, 0xf0000001)].forEach(
        function (item) {
          assert.ok(
            item.toNumber() < Math.pow(2, 53),
            util.format("Value should be less than -2^53 for %s", item),
          );
        },
      );
    });
  ...
  describe("Tuple", function () {
    ...
    describe("#toJSON()", function () {
      it("should return the string of the elements surrounded by square brackets", function () {
        ...
        assert.strictEqual(
          JSON.stringify(t),
          '["' + id.toString() + '","' + decimal.toString() + '",1,{"z":1}]',
        );
      });
    });
...
describe("clientOptions", function () {
  describe("#extend()", function () {
    ...
    it("should validate authProvider", () => {
      ...
      assert.throws(
        () => clientOptions.extend({ contactPoints, authProvider: "abc" }),
        message,
      );
      ...
      assert.doesNotThrow(() =>
        clientOptions.extend({ contactPoints, authProvider: undefined }),
      );
    });
