        ...
        describe("#fromString()", function () {
            it("should parse the string representation", function () {
                values.forEach(function (item) {
                    ...
                    assert.ok(
                        new LocalTime(Long.fromString(item[0])).equals(val),
                    );
                    assert.ok(
        ...
        it("should be serializable to json", function () {
            ...
            rowValues = [
                ...
                new types.InetAddress(
                    utils.allocBufferFromArray([192, 168, 0, 1]),
                ),
                null,
            ];
            columns = [
            ...
            expected =
                '{"cdecimal":"1.762","inet1":"192.168.0.1","inet2":null}';
            assert.strictEqual(JSON.stringify(row), expected);
        });
...
describe("writers", function () {
    describe("WriteQueue", function () {
        it("should buffer until threshold is passed", async () => {
            ...
            for (let i = 0; i < 10; i++) {
                queue.push(
                    new OperationState(request, null, utils.noop),
                    itemCallback,
                );
            }
            ...
            buffers.forEach((b) =>
                assert.strictEqual(b.length, coalescingThreshold),
            );
        });
