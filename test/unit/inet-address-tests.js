...
describe("InetAddress", function () {
    describe("constructor", function () {
        // DataStax throw TypeErrors, but due to napi-rs limitations, we throw Errors.
    ...
    describe("#toString()", function () {
        it("should convert IPv6 to string representation", function () {
            let val = new InetAddress(
                utils.allocBufferFromString(
                    "aabb0000eeff00112233445566778899",
                    "hex",
                ),
            );
            ...
            val = new InetAddress(
                utils.allocBufferFromString(
                    "aabb0000000000112233445566778899",
                    "hex",
                ),
            );
            ...
            val = new InetAddress(
                utils.allocBufferFromString(
                    "aabb0001000100112233445500000000",
                    "hex",
                ),
            );
            ...
            val = new InetAddress(
                utils.allocBufferFromString(
                    "000000000000000022330000000000bb",
                    "hex",
                ),
            );
        ...
        it("should convert IPv4 to string representation", function () {
            ...
            val = new InetAddress(
                utils.allocBufferFromArray([10, 12, 254, 32]),
            );
    ...
    describe("#equals()", function () {
        it("should return true when the bytes are the same", function () {
            ...
            const val2 = new InetAddress(
                utils.allocBufferFromString(hex2, "hex"),
            );
            assert.ok(val1.equals(new InetAddress(buf1)));
            assert.ok(
                val1.equals(
                    new InetAddress(utils.allocBufferFromString(hex1, "hex")),
                ),
            );
