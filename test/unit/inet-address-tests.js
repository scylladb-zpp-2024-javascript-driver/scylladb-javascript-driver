...
describe("InetAddress", function () {
  ...
  describe("#toString()", function () {
    it("should convert IPv6 to string representation", function () {
      let val = new InetAddress(
        utils.allocBufferFromString("aabb0000eeff00112233445566778899", "hex"),
      );
      assert.strictEqual(val.version, 6);
      assert.strictEqual(val.toString(), "aabb::eeff:11:2233:4455:6677:8899");
      val = new InetAddress(
        utils.allocBufferFromString("aabbccddeeff00112233445566778899", "hex"),
      );
      assert.strictEqual(
        val.toString(),
        "aabb:ccdd:eeff:11:2233:4455:6677:8899",
      );
      val = new InetAddress(
        utils.allocBufferFromString("aabb0000000000112233445566778899", "hex"),
      );
      assert.strictEqual(val.toString(), "aabb::11:2233:4455:6677:8899");
      val = new InetAddress(
        utils.allocBufferFromString("aabb0001000100112233445500000000", "hex"),
      );
      ...
      val = new InetAddress(
        utils.allocBufferFromString("000000000000000022330000000000bb", "hex"),
      );
      assert.strictEqual(val.toString(), "::2233:0:0:bb");
      val = new InetAddress(
        utils.allocBufferFromString("00000000000000000000000000000001", "hex"),
      );
  ...
  describe("#equals()", function () {
    it("should return true when the bytes are the same", function () {
      ...
      assert.ok(
        val1.equals(new InetAddress(utils.allocBufferFromString(hex1, "hex"))),
      );
