...
const {
    ...
    graphProtocol,
} = require("../../../lib/datastax/graph/options");
...
describe("GraphExecutor", function () {
    describe("#send()", () => {
        it("should encode the parameters and build GraphExecutionOptions", async () => {
            ...
            assert.deepStrictEqual(setup.executionArguments.params, [
                '{"a":1}',
            ]);
            helper.assertInstanceOf(
                setup.executionArguments.options,
                GraphExecutionOptions,
            );
        ...
        it("should not allow a namespace of type different than string", async () => {
            ...
            await helper.assertThrowsAsync(
                setup.instance.send("Q1", {}, { graphName: 123 }),
                TypeError,
            );
        });
        ...
        it("should set the same default options when not set", async () => {
            ...
            assert.strictEqual(
                firstOptions.getRawQueryOptions(),
                setup.executionArguments.options.getRawQueryOptions(),
            );
        ...
        it("should set the default payload for the executions", async () => {
            ...
            assert.strictEqual(
                optionsParameter.anotherOption,
                actualOptions.getRawQueryOptions().anotherOption,
            );
            assert.ok(actualOptions.getCustomPayload());
            helper.assertBufferString(
                actualOptions.getCustomPayload()["graph-language"],
                "gremlin-groovy",
            );
            helper.assertBufferString(
                actualOptions.getCustomPayload()["graph-results"],
                graphProtocol.graphson1,
            );
            helper.assertBufferString(
                actualOptions.getCustomPayload()["graph-source"],
                "a1",
            );
            helper.assertBufferString(
                actualOptions.getCustomPayload()["graph-name"],
                "name1",
            );
            helper.assertBufferString(
                actualOptions.getCustomPayload()["graph-read-consistency"],
                "LOCAL_ONE",
            );
            assert.strictEqual(
                actualOptions.getCustomPayload()["graph-write-consistency"],
                undefined,
            );
        });
        ...
        it("should set the default readTimeout in the payload", async () => {
            ...
            assert.strictEqual(
                typeof actualOptions.getCustomPayload()["request-timeout"],
                "undefined",
            );

            // with payload defined
            await setup.instance.send(
                "Q10",
                { c: 0 },
                { customPayload: { z: utils.allocBufferFromString("zValue") } },
            );
            actualOptions = setup.executionArguments.options;
            assert.ok(actualOptions);
            assert.ok(actualOptions.getCustomPayload());
            ...
            helper.assertBufferString(
                actualOptions.getCustomPayload()["z"],
                "zValue",
            );
            assert.strictEqual(
                typeof actualOptions.getCustomPayload()["request-timeout"],
            ...
            assert.deepEqual(
                actualOptions.getCustomPayload()["request-timeout"],
        ...
        it("should reuse the values from the default payload for the executions", async () => {
            ...
            Object.keys(firstOptions.getCustomPayload())
                ...
                .forEach((k) => {
                    assert.strictEqual(
                        firstOptions.getCustomPayload()[k],
                        setup.executionArguments.options.getCustomPayload()[k],
                    );
                });
        });
        ...
        it("should set the payload with the options provided", async () => {
            ...
            assert.deepStrictEqual(
                actualOptions.getCustomPayload()["graph-name"],
                utils.allocBufferFromString("namespace2"),
            );
        });
        ...
        it("should set the payload with the user/role provided", async () => {
            ...
            assert.strictEqual(
                actualOptions.getCustomPayload()[proxyExecuteKey],
                undefined,
            );
            ...
            assert.notStrictEqual(
                previousPayload,
                actualOptions.getCustomPayload(),
            );
            helper.assertBufferString(
                actualOptions.getCustomPayload()["graph-language"],
                "gremlin-groovy",
        ...
        it("should use the retry policy from the default profile when specified", async () => {
            ...
            await setup.instance.send("Q", null, {
                executionProfile: "graph-olap",
            });
            actualOptions = setup.executionArguments.options;
            assert.ok(actualOptions);
        ...
        it("should use the graph results provided in the profile", async () => {
            ...
            assert.ok(actualOptions && actualOptions.getCustomPayload());
            assert.strictEqual(
                actualOptions.getCustomPayload()["graph-results"].toString(),
                graphProtocol.graphson2,
            );
            await setup.instance.send("Q", null, null);
            actualOptions = setup.executionArguments.options;
        ...
        context("with analytics queries", function () {
            it("should query for analytics master", async () => {
                ...
                assert.ok(
                    actualOptions.getPreferredHost().address,
                    "10.10.10.10:9042",
                );
            });
            ...
            it("should call address translator", async () => {
                ...
                const translator =
                    new policies.addressResolution.AddressTranslator();
            ...
            it("should set preferredHost to null when RPC errors", async () => {
                ...
                const clientProperties = {
                    execute: function (q) {
                        if (
                            q === "CALL DseClientTool.getAnalyticsGraphServer()"
                        ) {
...
function setupGraphExecutor(clientOptions, clientProperties) {
    ...
    result.instance = new GraphExecutor(
        client,
        clientOptions,
    ...
    client.execute = (q) => {
        if (q === "CALL DseClientTool.getAnalyticsGraphServer()") {
            return Promise.resolve({
                rows: [{ result: { location: "10.10.10.10:1234" } }],
            });
        }
    ...
    if (clientProperties) {
        Object.keys(clientProperties).forEach(
            (propName) => (client[propName] = clientProperties[propName]),
        );
    }
