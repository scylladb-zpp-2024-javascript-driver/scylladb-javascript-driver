...
const {
  ...
  GraphSON3Reader,
  GraphSON3Writer,
} = require("../../../lib/datastax/graph/graph-serializer");
...
describe("GraphSON2Reader", function () {
  const reader = new GraphSON2Reader({
  ...
  describe("#read()", function () {
    ...
    (function defineObjectTest() {
      [
        ...
        [
          "gx:BigDecimal",
          types.BigDecimal,
          types.BigDecimal.fromString("123.32"),
        ],
        ["gx:BigInteger", types.Integer, types.Integer.fromString("99901")],
        [
          "gx:InetAddress",
          types.InetAddress,
          types.InetAddress.fromString("123.123.123.201"),
        ],
        ...
        [
          "dse:LineString",
          geometry.LineString,
          geometry.LineString.fromString("LINESTRING (1 1, 2 2, 3 3)"),
        ],
        [
          "dse:Polygon",
          geometry.Polygon,
          new geometry.Polygon.fromString(
    ...
    it("should read Vertex with nested properties", function () {
      ...
      assert.ok(
        result.properties["age"][0].id.equals(types.Long.fromString("1")),
      );
...
describe("GraphSON3Reader", function () {
  const reader = new GraphSON3Reader({
    serializers: getCustomTypeSerializers(),
  });
...
describe("GraphSON3Writer", function () {
  const writer = new GraphSON3Writer({
    serializers: getCustomTypeSerializers(),
  });
  ...
  describe("adaptObject", () => {
    it("should support wrapped values", () => {
      [
        ...
        [
          asTimestamp(new Date(1580477249207)),
          { "@type": "g:Timestamp", "@value": 1580477249207 },
        ],
      ].forEach((item) => {
    ...
    it("should support simple tuples", () => {
      [
        ...
        [
          new Tuple("a", asInt(10)),
          ["text", "int"],
          ["a", { "@type": "g:Int32", "@value": 10 }],
        ],
        [
          new Tuple(
            "b",
            types.Uuid.fromString("bb6d7af7-f674-4de7-8b4c-c0fdcdaa5cca"),
          ),
          ["text", "uuid"],
          [
            "b",
            {
              "@type": "g:UUID",
              "@value": "bb6d7af7-f674-4de7-8b4c-c0fdcdaa5cca",
            },
          ],
        ],
        [
          new Tuple("b", types.Long.fromString("1234")),
          ["text", "bigint"],
          ["b", { "@type": "g:Int64", "@value": "1234" }],
        ],
      ].forEach((item) => {
        const tuple = item[0];
        ...
        assert.deepEqual(
          result["@value"].definition.map((d) => d.cqlType),
          item[1],
        );
      });
    });
