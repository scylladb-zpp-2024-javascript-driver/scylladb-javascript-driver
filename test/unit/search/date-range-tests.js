  ...
  const values = [
    [
      ...
      getDateRange(
        getUtcDate(2010),
        precision.year,
        getUtcDate(2011, 12, 31, 23, 59, 59, 999),
        precision.month,
      ),
    ],
    [
      ...
      getDateRange(
        null,
        null,
        getUtcDate(2011, 8, 31, 23, 59, 59, 999),
        precision.month,
      ),
      "[* TO 2011-08]",
    ],
    [
      "[2015-01 TO *]",
      new DateRange(
        new DateRangeBound(getUtcDate(2015), precision.month),
    ...
    [
      ...
      getDateRange(
        getUtcDate(2017, 1),
        precision.month,
        getUtcDate(2017, 2, 28, 23, 59, 59, 999),
        precision.month,
      ),
      "[2017-01 TO 2017-02]",
    ],
    ...
    [
      ...
      getDateRange(
        getUtcDate(2016, 1),
        precision.month,
        getUtcDate(2016, 2, 29, 23, 59, 59, 999),
        precision.month,
      ),
      "[2016-01 TO 2016-02]",
    ],
    [
      "2012-1-2",
      getDateRange(getUtcDate(2012, 1, 2), precision.day),
      "2012-01-02",
    ],
    [
      "2012-1-2T",
    ...
    [
      "1-2-3T23:5:7",
      getDateRange(getUtcDate(1, 2, 3, 23, 5, 7), precision.second),
      "0001-02-03T23:05:07",
    ],
    [
      "2015-01T03",
      getDateRange(getUtcDate(2015, 1, 1, 3), precision.hour),
      "2015-01-01T03",
    ],
    [
      "2015-04T03:02",
      getDateRange(getUtcDate(2015, 4, 1, 3, 2), precision.minute),
      "2015-04-01T03:02",
    ],
    [
      "2015-04T03:02:01.081",
      getDateRange(getUtcDate(2015, 4, 1, 3, 2, 1, 81), precision.millisecond),
    ...
    [
      "0001-01-01",
      new DateRange(new DateRangeBound(getUtcDate(1, 1, 1), precision.day)),
    ],
    [
      "-0001-01-01",
      new DateRange(new DateRangeBound(getUtcDate(-1, 1, 1), precision.day)),
    ...
    [
      "-0009",
      new DateRange(new DateRangeBound(getUtcDate(-9, 1, 1), precision.year)),
    ],
    [
      "0000",
      new DateRange(new DateRangeBound(getUtcDate(0, 1, 1), precision.year)),
    ],
  ];
  describe("fromString()", function () {
    it("should parse valid values", function () {
      values.forEach(function (item) {
        ...
        assert.ok(
          actual.equals(item[1]),
          util.format(
            'Parsed value "%s" not equals to expected: %j, got: %j',
            item[0],
            item[1],
            actual,
          ),
  ...
  describe("#toBuffer() and fromBuffer()", function () {
    it("should serialize and deserialize the values", function () {
      values.forEach(function (item) {
        ...
        assert.ok(
          deserialized.equals(dateRange),
          util.format(
            "Serialization or deserialization failed for %j",
            dateRange,
          ),
        );
      });
...
describe("DateRangeBound", function () {
  ...
  const values = [
    ...
    [
      new DateRangeBound(date, precision.millisecond),
      "2017-01-20T06:54:01.578Z",
    ],
    [
      new DateRangeBound(
        getUtcDate(142017, 1, 20, 6, 54, 1, 570),
    ...
    [
      new DateRangeBound(
        getUtcDate(12017, 3, 5, 6, 54, 1, 570),
        precision.millisecond,
      ),
      "+12017-03-05T06:54:01.570Z",
    ],
    [
      new DateRangeBound(bcDate, precision.millisecond),
      "-2001-11-20T16:05:01.999Z",
    ],
  ];
  describe("#toString()", function () {
...
function getDateRange(date1, precision1, date2, precision2) {
  const lowerBound = date1
    ? new DateRangeBound(date1, precision1)
    : DateRangeBound.unbounded;
  let upperBound = null;
  if (date2) {
