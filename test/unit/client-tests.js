...
describe("Client", function () {
  describe("constructor", function () {
    ...
    it("should validate client id, application name and version", () => {
      assert.throws(
        () =>
          new Client(
            Object.assign({ applicationName: 123 }, helper.baseOptions),
          ),
      ...
      assert.throws(
        () =>
          new Client(
            Object.assign({ applicationVersion: 123 }, helper.baseOptions),
          ),
        /applicationVersion should be a String/,
      );

      assert.throws(
        () => new Client(Object.assign({ id: 123 }, helper.baseOptions)),
      ...
      assert.doesNotThrow(
        () =>
          new Client(
            Object.assign(
              { applicationName: "App_Z", applicationVersion: "1.2.3" },
              helper.baseOptions,
            ),
          ),
      );
      ...
      assert.doesNotThrow(
        () =>
          new Client(
            Object.assign({ applicationName: "App_Z" }, helper.baseOptions),
          ),
      );
      assert.doesNotThrow(
        () =>
          new Client(
            Object.assign({ id: types.TimeUuid.now() }, helper.baseOptions),
          ),
      );
    });
    ...
    it("should set DefaultLoadBalancingPolicy as default", function () {
      ...
      helper.assertInstanceOf(
        client.options.policies.loadBalancing,
        policies.loadBalancing.DefaultLoadBalancingPolicy,
      );
    ...
    context("with useBigIntAsLong or useBigIntAsVarint set", () => {
      if (typeof BigInt === "undefined") {
        it("should throw an error on engines that do not support it", () => {
          [{ useBigIntAsLong: true }, { useBigIntAsVarint: true }].forEach(
            (encoding) => {
              assert.throws(
                () => new Client({ contactPoints: ["10.10.1.1"], encoding }),
    ...
    context("with cloud options", () => {
      it("should support string secure bundle", () => {
        assert.doesNotThrow(
          () => new Client({ cloud: { secureConnectBundle: "a/b/c" } }),
        );
      });
      ...
      if (typeof URL !== "undefined") {
        it("should support URL secure bundle", () => {
          assert.doesNotThrow(
            () =>
              new Client({
                cloud: { secureConnectBundle: new URL("https://a/b/c") },
              }),
          );
        });
      }
      ...
      it("should not allow secure bundle of other types", () => {
        assert.throws(
          () => new Client({ cloud: { secureConnectBundle: {} } }),
          TypeError,
          "must be of type string",
        );
        assert.throws(
          () => new Client({ cloud: { secureConnectBundle: 1 } }),
          TypeError,
          "must be of type string",
        );
      });
      ...
      it("should validate that contact points and ssl are not set", () => {
        ...
        [
          ...
          assert.throws(
            () => new Client(options),
            TypeError,
            "can not be defined when cloud settings are provided",
          ),
        );
      });
    });
  ...
  describe("#connect()", function () {
    ...
    it("should fail if no host name can be resolved", function (done) {
      ...
      client.connect(function (err) {
        ...
        const resolvedContactPoints =
          client.controlConnection.getResolvedContactPoints();
        contactPoints.forEach((name) =>
          assert.strictEqual(resolvedContactPoints.get(name), utils.emptyArray),
        );
    ...
    it("should fail when trying to connect after shutdown", function (done) {
      ...
      utils.series(
        [client.connect.bind(client), client.shutdown.bind(client)],
        function (err) {
          assert.ifError(err);
          client.connect(function (err) {
            assert.instanceOf(err, errors.NoHostAvailableError);
            assert.ok(
              err.message.indexOf("after shutdown") >= 0,
              "Error message does not contain occurrence: " + err.message,
            );
            done();
          });
        },
  ...
  describe("#execute()", function () {
    ...
    it("should build the routingKey based on routingNames for query requests", async () => {
      ...
      const query = "SELECT * FROM dummy WHERE id2=:key2 and id1=:key1";
      const queryOptions = {
        routingNames: ["key1", "key2"],
        hints: { key1: "int", key2: "int" },
      };
      await client.execute(query, { key2: 2, key1: 1 }, queryOptions);
    ...
    it("should use the default execution profile options", async () => {
      ...
      assert.strictEqual(
        execOptions.getSerialConsistency(),
        profile.serialConsistency,
      );
    ...
    it("should set the timestamp", async () => {
      ...
      for (const version of protocolVersions) {
        ...
        if (version > 2) {
          assert.ok(timestamp);
          assert.ok(
            timestamp instanceof types.Long || typeof timestamp === "number",
          );
        } else {
    ...
    it("should not set the timestamp when timestampGeneration is null", async () => {
      ...
      const client = newConnectedInstance(handlerMock, {
        policies: { timestampGeneration: null },
      });
    ...
    context("with no callback specified", function () {
      ...
      it("should reject the promise when an ExecutionProfile is not found", function (done) {
        const client = new Client(helper.baseOptions);
        const p = client.execute("Q", [1, 2], {
          executionProfile: "non_existent",
        });
        assert.instanceOf(p, Promise);
        p.catch(function (err) {
          assert.instanceOf(err, errors.ArgumentError);
        ...
        function (version, next) {
          ...
          client.batch(["q1", "q2", "q3"], function (err) {
            ...
            if (version > 2) {
              assert.ok(timestamp);
              assert.ok(
                timestamp instanceof types.Long ||
  ...
  describe("#shutdown()", function () {
    const options = clientOptions.extend({}, helper.baseOptions, {
      policies: {
        reconnection: new policies.reconnection.ConstantReconnectionPolicy(100),
      },
          ...
          setTimeout(function () {
            ...
            setTimeout(function assertAfterSomeTime() {
              assert.strictEqual(
                logEvents.length,
                0,
                "Expected no log events after shutdown but was: " +
                  util.inspect(logEvents),
              );
              done();
            }, 100);
          }, 20);
  ...
  describe("#_waitForSchemaAgreement()", function () {
    ...
    it("should callback when there is an error retrieving versions", async () => {
      ...
      const err = await helper.assertThrowsAsync(
        client._waitForSchemaAgreement(null),
      );
...
function getConnections(length = 1) {
  return Array(length)
    .fill(0)
    .map(() => ({ closeAsync: sinon.spy(() => Promise.resolve()) }));
}
