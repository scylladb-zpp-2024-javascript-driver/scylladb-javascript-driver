...
describe("Client", function () {
        ...
        it("should fail if no host name can be resolved", function (done) {
            ...
            client.connect(function (err) {
                ...
                contactPoints.forEach((name) =>
                    assert.strictEqual(
                        resolvedContactPoints.get(name),
                        utils.emptyArray,
                    ),
                );
                ...
                function (err) {
                    ...
                    client.connect(function (err) {
                        ...
                        assert.ok(
                            ...
                            "Error message does not contain occurrence: " +
                                err.message,
                        );
                        done();
                    });
    ...
    describe("#shutdown()", function () {
        const options = clientOptions.extend({}, helper.baseOptions, {
            policies: {
                reconnection:
                    new policies.reconnection.ConstantReconnectionPolicy(100),
            },
    ...
    describe("#_waitForSchemaAgreement()", function () {
        ...
        it("should timeout if there is no agreement", async () => {
            ...
            client.hosts = { length: 5 };
            client.metadata = {
                compareSchemaVersions: sinon.fake((c) =>
