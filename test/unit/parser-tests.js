...
describe("Parser", function () {
  describe("#_transform()", function () {
    ...
    it("should buffer a AUTHENTICATE response until complete", function (done) {
      ...
      parser._transform(
        { header: header, chunk: utils.allocBufferFromString("a"), offset: 0 },
        null,
        doneIfError(done),
      );
      parser._transform(
        { header: header, chunk: utils.allocBufferFromString("bc"), offset: 0 },
        null,
        doneIfError(done),
      );
    });
    ...
    it("should read a WRITE_FAILURE", function (done) {
      ...
      bodyArray.push(
        utils.allocBufferFromArray([0, 3, 0, 0, 0, 2, 0, 0, 0, 3, 0, 0, 0, 1]),
      );
      // Write Type 'COUNTER'
    ...
    it("should emit empty result one column no rows", function (done) {
      ...
      parser.on("readable", function () {
        ...
        assert.strictEqual(item.header.opcode, types.opcodes.result);
        assert.ok(
          item.result && item.result.rows && item.result.rows.length === 0,
        );
        done();
      });
      //kind
    ...
    describe("with multiple chunk lengths", function () {
      ...
      [1, 3, 5, 13].forEach(function (chunkLength) {
        it(
          "should emit rows chunked with chunk length of " + chunkLength,
          function (done) {
            result = {};
            const expected = [
