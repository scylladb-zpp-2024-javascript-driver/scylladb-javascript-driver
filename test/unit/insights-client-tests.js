    ...
    describe("#init()", () => {
        it("should send the startup event rpc immediately", () =>
            getStartupMessage().then((result) => {
                ...
                assert.deepStrictEqual(message.metadata.tags, {
                    language: "nodejs",
                });
                ...
                assert.strictEqual(
                    message.data.driverVersion,
                    packageInfo.version,
                );
                assert.strictEqual(
                    message.data.clientId,
                    client.options.id.toString(),
                );
                ...
                assert.strictEqual(
                    typeof message.data.executionProfiles,
                    "object",
                );
                ...
                assert.strictEqual(
                    typeof message.data.heartbeatInterval,
                    "number",
                );
                ...
                assert.strictEqual(
                    typeof message.data.platformInfo.os,
                    "object",
                );
                assert.strictEqual(
                    typeof message.data.platformInfo.cpus,
                    "object",
                );
                assert.strictEqual(
                    typeof message.data.platformInfo.runtime,
                    "object",
                );
                assert.strictEqual(
                    typeof message.data.configAntiPatterns,
                    "object",
                );
                assert.deepEqual(
                    Object.keys(message.data.configAntiPatterns),
                    [],
                );
        ...
        it("should schedule the first status event", (done) => {
            ...
            setTimeout(() => {
                ...
                assert.strictEqual(
                    startupMessage.metadata.name,
                    startupEventName,
                );
                assert.strictEqual(
                    typeof startupMessage.data.sessionId,
                    "string",
                );
        ...
        it("should include all the options in execution profiles", () =>
            getStartupMessage().then((result) => {
                const defaultProfile =
                    result.message.data.executionProfiles["default"];
        ...
        it("should include the different data centers", () => {
            ...
            return getStartupMessage(options).then((result) => {
                assert.deepStrictEqual(result.message.data.dataCenters, [
                    "dc0",
                    "dc1",
                ]);
            });
        ...
        it("should generate the application name and set the generated flag", () =>
            getStartupMessage({
                clientOptions: { applicationName: null },
            }).then((result) => {
...
function getClient(options) {
    ...
    hostVersions.forEach((v, i) => {
        client.hosts.set(`10.10.10.${i}`, {
            getDseVersion: () =>
                v === "{empty}" ? utils.emptyArray : v.split("."),
            datacenter: "dc" + (i % 2),
            setDistance: () => {},
        });
    });
