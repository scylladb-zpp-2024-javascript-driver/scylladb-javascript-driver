...
describe("RoundRobinPolicy", function () {
  ...
  describe("#getOptions()", () => {
    it("should return an empty Map", () =>
      helper.assertMapEqual(new RoundRobinPolicy().getOptions(), new Map()));
  });
});
          ...
          policy.newQueryPlan(null, null, function (err, iterator) {
            ...
            for (let i = 0; i < originalHosts.length; i++) {
              ...
              assert.strictEqual(
                item.done,
                false,
                "It shouldn't be done at index " + i,
              );
              hosts.push(item.value);
            }
            next();
...
describe("TokenAwarePolicy", function () {
  ...
  it("should retrieve local replicas plus childPolicy hosts plus remote replicas", function (done) {
    ...
    client.getReplicas = toFunc(
      ["repl1_remote", "repl2_local", "repl3_remote", "repl4_local"].map(
        toHost,
      ),
    );
    utils.series(
      [
        ...
        function (next) {
          policy.newQueryPlan(
            null,
            getExecOptions({ routingKey: utils.allocBufferUnsafe(16) }),
            function (err, iterator) {
              ...
              assert.deepEqual(hosts.map(toAddress).slice(0, 2).sort(), [
                "repl2_local",
                "repl4_local",
              ]);
              // Child load balancing policy nodes, do not repeat repl2
              assert.deepEqual(hosts.map(toAddress).slice(2, 4), [
                "child1",
                "child2",
              ]);
              next();
            ...
            function (n, timesNext) {
              policy.newQueryPlan(
                ...
                function (err, iterator) {
                  ...
                  assert.deepEqual(hosts.map(toAddress).slice(0, 3).sort(), [
                    "repl2_local",
                    "repl4_local",
                    "repl5_local",
                  ]);
                  ...
                  // Child load balancing policy nodes, do not repeat repl2
                  assert.deepEqual(hosts.map(toAddress).slice(3, 5), [
                    "child1",
                    "child2",
                  ]);
                  timesNext();
                ...
                function (err, iterator) {
                  ...
                  hosts
                    ...
                    .forEach(function (address, i) {
                      replicaPositions[i][address] =
                        (replicaPositions[i][address] || 0) + 1;
                    });
  ...
  describe("#getOptions()", () => {
    ...
    context("when DCAwareRoundRobinPolicy is child policy", () => {
      it("should return a Map with the local data center name", () => {
        helper.assertMapEqual(
          new TokenAwarePolicy(new DCAwareRoundRobinPolicy("dc2")).getOptions(),
          new Map([
...
describe("AllowListPolicy", function () {
  it("should use the childPolicy to determine the distance", function () {
    ...
    assert.strictEqual(
      policy.getDistance({ address: "h1:9042" }),
      types.distance.local,
    );
    assert.strictEqual(getDistanceCalled, 1);
    assert.strictEqual(
      policy.getDistance({ address: "h2:9042" }),
      types.distance.local,
    );
    ...
    const childPolicy = {
      newQueryPlan: function (ks, o, cb) {
        cb(
          null,
          utils.arrayIterator([
            { address: "1.1.1.1:9042" },
            { address: "1.1.1.2:9042" },
            { address: "1.1.1.3:9042" },
          ]),
        );
      },
    };
    const policy = new AllowListPolicy(childPolicy, [
      "1.1.1.3:9042",
      "1.1.1.1:9042",
    ]);
  ...
  describe("#getOptions()", () => {
    it("should return a Map with the child policy name", () => {
      helper.assertMapEqual(
        new AllowListPolicy(new RoundRobinPolicy(), ["a", "b"]).getOptions(),
        new Map([
...
function createDummyPolicy(options) {
  ...
  childPolicy.newQueryPlan = function (k, o, cb) {
    ...
    const hosts = [
      new Host("repl2_local", 2, options),
      new Host("child1", 2, options),
      new Host("child2", 2, options),
    ];
