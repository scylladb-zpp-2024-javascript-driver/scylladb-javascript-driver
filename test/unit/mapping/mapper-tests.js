...
describe("Mapper", () => {
    describe("constructor", () => {
        ...
        it("should validate that client keyspace is set", () => {
            assert.throws(
                () => new Mapper({}, { models: { Sample: { table: "t1" } } }),
    ...
    describe("#forModel()", () => {
        ...
        it("should log when creating a ModelMapper instance using mapping information", () => {
            ...
            const mapper = new Mapper(client, {
                models: { T: { tables: ["t1", "t2"], keyspace: "ks1" } },
            });
            ...
            assert.match(
                args[1],
                /Creating model mapper for 'T' using mapping information. Keyspace: ks1; Tables: t1,t2.$/,
            );
        });
        ...
        it("should log when creating a ModelMapper instance using defaults", () => {
            ...
            assert.match(
                args[1],
                /Mapping information for model 'user' not found, creating .+Keyspace: abc; Table: user.$/,
            );
        });
    });
    ...
    describe("#batch()", () => {
        it("should throw when the parameters are not valid", () => {
            ...
            const message2 =
                "Batch items must be instances of ModelBatchItem, use modelMapper.batching object to create" +
                " each item";
        ...
        it("should set the execution options", () => {
            ...
            return Promise.all(
                items.map((executionOptions, index) => {
                    const queryItems = [
                        modelMapper.batching.insert({ id1: "a", id2: "b" }),
                    ];

                    return mapper
                        .batch(queryItems, executionOptions)
                        .then(() => {
                            helper.assertProperties(
                                clientInfo.batchExecutions[index].options,
                                executionOptions,
                            );
                            assert.strictEqual(
                                clientInfo.batchExecutions[index].options
        ...
        it("should throw error when one of the ModelBatchItem is invalid", () => {
            ...
            return mapper
                .batch([
                    modelMapper.batching.insert({
                        z: "this column does not exist",
                    }),
                    modelMapper.batching.update({
                        z: "this column does not exist",
                    }),
                    modelMapper.batching.remove({
                        z: "this column does not exist",
                    }),
                ])
                .catch((err) => (error = err))
            ...
            Promise.all(
                [true, false].map((isIdempotent) => {
                    ...
                    const items = [
                        modelMapper.batching.insert(
                            { id1: "value1" },
                            { ifNotExists: !isIdempotent },
                        ),
                        modelMapper.batching.insert({ id1: "value2" }),
                    ];

                    return mapper.batch(items).then(() => {
                        assert.strictEqual(
                            clientInfo.batchExecutions[0].options.isIdempotent,
                            isIdempotent,
                        );
                    });
                }),
        ...
        it("should be able to batch using custom field parser", () => {
            ...
            mapper.batch(items).then(() => {
                assert.lengthOf(clientInfo.batchExecutions[0].queries, 2);
                assert.isTrue(
                    clientInfo.batchExecutions[0].queries[0].params[1] ===
                        JSON.stringify(someFakeJson),
                );
                assert.isTrue(
                    clientInfo.batchExecutions[0].queries[1].params[0] ===
                        JSON.stringify(someFakeJson),
                );
            });
        });
