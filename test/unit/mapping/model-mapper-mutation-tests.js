...
describe("ModelMapper", () => {
  describe("#insert()", () => {
    ...
    it("should retrieve the table that apply and make a single execution", () => {
      const clientInfo = mapperTestHelper.getClient(
        ["id1", "id2", "name"],
        [1, 1],
      );
      ...
      testQueries("insert", [
        {
          ...
          query:
            'INSERT INTO ks1.table1 ("id2", "id1", "name") VALUES (?, ?, ?) IF NOT EXISTS',
      ...
      testQueries("insert", [
        {
          ...
          docInfo: { ttl: 1000 },
          query:
            'INSERT INTO ks1.table1 ("id2", "id1", "name") VALUES (?, ?, ?) USING TTL ?',
      ...
      testErrors("insert", [
        {
          ...
          message:
            "No table matches (all PKs have to be specified) fields: [id1,notAValidProp]",
        },
        {
          doc: { id1: "x" },
          docInfo: { fields: ["notAValidProp"] },
          message:
            "No table matches (all PKs have to be specified) fields: [notAValidProp]",
        },
        {
          doc: { id1: "x", name: "y" },
          message:
            "No table matches (all PKs have to be specified) fields: [id1,name]",
        },
        {
          doc: {},
    ...
    it("should warn when cache reaches 100 different queries", async () => {
      ...
      assert.strictEqual(
        clientInfo.logMessages.filter((l) => l.level === "warning").length,
        0,
      );
      ...
      const warnings = clientInfo.logMessages.filter(
        (l) => l.level === "warning",
      );
      assert.strictEqual(warnings.length, 1);
      helper.assertContains(
        warnings[0].message,
        `ModelMapper cache reached ${cacheHighWaterMark}`,
      );
    });

    it("should use the mapping function in the insert values", () =>
      testQueries({
        methodName: "insert",
        models: {
          Sample: {
        ...
        items: [
          {
            doc: {
              id1: "value_id1",
              id2: "value_id2",
              name: { prop1: 1, prop2: "two" },
            },
            query:
              'INSERT INTO ks1.table1 ("id1", "id2", "name") VALUES (?, ?, ?)',
  ...
  describe("#update()", () => {
      ...
      testQueries("update", [
        {
          doc: { id2: "value2", id1: "value1", name: "name1" },
          query:
            'UPDATE ks1.table1 SET "name" = ? WHERE "id2" = ? AND "id1" = ?',
      ...
      testQueries("update", [
        {
          ...
          query:
            'UPDATE ks1.table1 SET "name" = ? WHERE "id2" = ? AND "id1" = ? IF "name" = ?',
      ...
      testQueries("update", [
        {
          ...
          query:
            'UPDATE ks1.table1 SET "name" = ?, "list1" = "list1" + ? WHERE "id2" = ? AND "id1" = ?',
    ...
    it("should increment/decrement a counter", () => {
      const clientInfo = mapperTestHelper.getClient(
        ["id1", "id2", { name: "c1", type: { code: dataTypes.counter } }],
        [1, 1],
      );
      const modelMapper = mapperTestHelper.getModelMapper(clientInfo);
      const items = [
        {
          ...
          query:
            'UPDATE ks1.table1 SET "c1" = "c1" + ? WHERE "id2" = ? AND "id1" = ?',
        },
        {
      ...
      return Promise.all(
        items.map((item, index) =>
          modelMapper.update(item.doc).then(() => {
      ...
      testErrors("update", [
        {
          doc: { id1: "x", notAValidProp: "y" },
          message:
            "No table matches (all PKs and columns to set have to be specified) fields: [id1,notAValidProp]",
        },
        {
          doc: { id1: "x" },
          docInfo: { fields: ["notAValidProp"] },
          message:
            "No table matches (all PKs and columns to set have to be specified) fields: [notAValidProp]",
        },
        {
          doc: { id1: "x", name: "y" },
          message:
            "No table matches (all PKs and columns to set have to be specified) fields: [id1,name]",
        },
        {
          doc: { id1: "x", id2: "y", name: "z" },
          docInfo: { when: { notAValidProp: "m" } },
          message:
            "No table matches (all PKs and columns to set have to be specified) fields: [id1,id2,name]; condition: [notAValidProp]",
        },
        {
          doc: {},
          message: "Expected object with keys",
        },
        {
          doc: { id1: "x", id2: "y" },
          message:
            "No table matches (all PKs and columns to set have to be specified) fields: [id1,id2]",
        },
      ]));
      ...
      testQueries("update", [
        {
          ...
          query:
            'UPDATE ks1.table1 SET "description" = ? WHERE "id1" = ? AND "id2" = ?',
      ...
      testQueries("update", [
        {
          ...
          docInfo: { ttl: 360 },
          query:
            'UPDATE ks1.table1 USING TTL ? SET "name" = ? WHERE "id1" = ? AND "id2" = ?',
          params: [360, "value_name1", "value_id1", "value_id2"],
        },
      ]));

    it("should use the mapping function in the SET WHERE and IF clauses", () =>
      testQueries({
        methodName: "update",
        models: {
          Sample: {
        ...
        items: [
          {
            ...
            query:
              'UPDATE ks1.table1 SET "name" = ? WHERE "id1" = ? AND "id2" = ?',
            params: [
              '{"prop1":1,"prop2":"two"}',
              "value_id1",
              "value_id2_suffix",
          ...
          },
          {
            ...
            query:
              'UPDATE ks1.table1 SET "description" = ? WHERE "id1" = ? AND "id2" = ? IF "name" = ?',
            params: [
              "my description",
              "value_id1",
              "value_id2_suffix",
  ...
  describe("#remove()", () => {
    ...
    it("should throw an error when filter or conditions are not valid", () =>
      testErrors("remove", [
        ...
        },
        {
          ...
          docInfo: { when: { notAValidProp: "m" } },
          message:
            "No table matches (all PKs have to be specified) fields: [id1,id2,name]; condition: [notAValidProp]",
        },
        {
          doc: {},
          message: "Expected object with keys",
    ...
    it("should generate the query, params and set the idempotency", () =>
      testQueries("remove", [
        {
          doc: { id1: "x", id2: "y" },
        ...
        },
        {
          ...
          docInfo: { when: { name: "a" } },
          query:
            'DELETE FROM ks1.table1 WHERE "id1" = ? AND "id2" = ? IF "name" = ?',
          params: ["x", "y", "a"],
          isIdempotent: false,
        },
        {
          ...
          query:
            'DELETE FROM ks1.table1 WHERE "id1" = ? AND "id2" = ? IF EXISTS',
          params: ["x", "y"],
          isIdempotent: false,
        },
        {
    ...
    it("should use the mapping function in the WHERE and IF clauses", () =>
      testQueries({
        methodName: "remove",
        models: {
          Sample: {
...
async function testQueries(methodName, items) {
  ...
  const columns = [
    ...
    "id2",
    "name",
    { name: "list1", type: { code: dataTypes.list } },
    "description",
  ];
  ...
  for (const item of items) {
    ...
    const expectedOptions = {
      prepare: true,
      isIdempotent: item.isIdempotent !== false,
    };
