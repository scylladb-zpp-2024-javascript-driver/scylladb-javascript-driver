...
describe("Cache", function () {
    ...
    describe("getSelectKey()", () => {
        ...
        const getKey = (docKeys, doc, docInfo) =>
            Array.from(Cache.getSelectKey(docKeys, doc, docInfo));
        ...
        it("should consider limit, orderBy, fields", () => {
            ...
            // orderBy
            const key1WithOrderByA = getKey(
                docKeys1,
                {},
                { orderBy: { abc: "asc" } },
            );
            const key1WithOrderByB = getKey(
                docKeys1,
                {},
                { orderBy: { def: "desc" } },
            );
            const key1WithOrderByC = getKey(
                docKeys1,
                {},
                { orderBy: { abc: "asc" } },
            );
            const key1WithOrderByD = getKey(
                docKeys1,
                {},
                { orderBy: { abc: "asc", def: "desc" } },
            );
    ...
    describe("getUpdateKey()", () => {
        const getKey = (docKeys, doc, docInfo) =>
            Array.from(Cache.getUpdateKey(docKeys, doc, docInfo));
        ...
        testDocInfoProperties(getKey, [{ ifExists: true }, { ttl: 1000 }]);
    });

    describe("getRemoveKey", () => {
        const getKey = (docKeys, doc, docInfo) =>
            Array.from(Cache.getRemoveKey(docKeys, doc, docInfo));
    ...
    describe("getInsertKey", () => {
        const getKey = (docKeys, doc, docInfo) =>
            Array.from(Cache.getInsertKey(docKeys, docInfo));

        testFields(getKey);

        testDocInfoProperties(getKey, [{ ifNotExists: true }, { ttl: 20 }]);
    });
});

function testQueryOperators(getKey) {
    it("should consider query operators", () => {
        const key1 = getKey(["prop1", "prop2", "prop3"], {
            prop1: 1,
            prop2: q.lt(10),
        });
        const key2 = getKey(["prop1", "prop2", "prop3"], {
            prop1: 1,
            prop2: 10,
        });
        assert.notDeepEqual(key1, key2);
...
function testWhenOperators(getKey) {
    it("should consider when condition", () => {
        const key1 = getKey(
            ["prop1", "prop2", "prop3"],
            { prop1: 3 },
            { when: { prop1: 1, prop2: q.lt(10) } },
        );
        const key2 = getKey(
            ["prop1", "prop2", "prop3"],
            { prop1: 3 },
            { when: { prop1: 1, prop2: 10 } },
        );
        assert.notDeepEqual(key1, key2);
    });
...
function testDocInfoProperties(getKey, items) {
    it("should consider docInfo properties", () => {
        items.forEach((docInfo) => {
            ...
            assert.notDeepEqual(
                key,
                getKey(["prop1"], { prop1: 1 }, undefined),
            );
            ...
            items.forEach((compareDocInfo) => {
                ...
                assert.notDeepEqual(
                    key,
                    getKey(["prop1"], { prop1: 1 }, compareDocInfo),
                );
            });
        });
    });
