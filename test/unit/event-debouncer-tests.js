...
describe("EventDebouncer", function () {
  ...
  describe("#eventReceived()", function () {
    it("should invoke 1 handler and all the callbacks when one event is flagged as `all`", async () => {
      ...
      await Promise.all([
        debouncer.eventReceived(
          { handler: helper.failop, keyspace: "ks1" },
          false,
        ),
        debouncer.eventReceived(
          { handler: helper.failop, keyspace: "ks1", cqlObject: "abc" },
          false,
        ),
        debouncer.eventReceived(
          { handler: helper.failop, keyspace: "ks2" },
          false,
        ),
    ...
    it("should invoke 1 keyspace handler and all the callbacks when cqlObject is undefined", async () => {
      ...
      const promise = Promise.all([
        debouncer.eventReceived(
          { handler: getHandler("A"), keyspace: "ks1" },
          false,
        ),
        debouncer.eventReceived(
          { handler: getHandler("B"), keyspace: "ks1", cqlObject: "1a" },
          false,
        ),
        debouncer.eventReceived(
          { handler: getHandler("C"), keyspace: "ks2" },
          false,
        ),
        debouncer.eventReceived(
          { handler: getHandler("D"), keyspace: "ks2" },
          false,
        ),
        debouncer.eventReceived(
          { handler: getHandler("E"), keyspace: "ks2" },
          false,
        ),
        debouncer.eventReceived(
          { handler: getHandler("F"), keyspace: "ks3", cqlObject: "3a" },
          false,
        ),
        debouncer.eventReceived(
          { handler: getHandler("H"), keyspace: "ks3", cqlObject: "3b" },
          false,
        ),
      ]);
    ...
    it("should process queue immediately when processNow is true", async () => {
      ...
      const promise = Promise.all([
        debouncer
          .eventReceived({ handler: getHandler("A"), keyspace: "ks2" }, false)
          .then(spy),
        debouncer
          .eventReceived({ handler: getHandler("B"), keyspace: "ks1" }, false)
          .then(spy),
        // set with process now to true
        debouncer
          .eventReceived({ handler: getHandler("C"), keyspace: "ks1" }, true)
          .then(spy),
        debouncer
          .eventReceived({ handler: getHandler("D"), keyspace: "ks1" }, false)
          .then(spy),
      ]);
  ...
  describe("#shutdown()", () => {
    it("should invoke all callbacks", async () => {
      ...
      debouncer
        .eventReceived({ handler: helper.failop, keyspace: "ks1" }, false)
        .then(spy);
      debouncer
        .eventReceived(
          { handler: helper.failop, keyspace: "ks1", cqlObject: "1a" },
          false,
        )
        .then(spy);
      debouncer
        .eventReceived({ handler: helper.failop, keyspace: "ks2" }, false)
        .then(spy);
      debouncer
        .eventReceived({ handler: helper.failop, all: true }, false)
        .then(spy);
