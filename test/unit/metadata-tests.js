...
describe("Metadata", function () {
    ...
    it("should remove excess prepared queries when _maxPrepared has been exceeded", function () {
      ...
      assert.lengthOf(
        loggedMessages.filter(
          ({ level, message }) =>
            level === "warning" && message.indexOf("exceeded maximum") >= 0,
        ),
        1,
      );
    });
  ...
  describe("#getTable()", function () {
    ...
    it("should reject if have not connected yet", () => {
      const metadata = new Metadata(clientOptions.defaultOptions(), null);
      return metadata.getTable("ks1", "tbl1").catch(validateMetadataErr());
    });
      ...
      utils.mapSeries(
        ...
        function (err, results) {
          ...
          assert.lengthOf(
            cc.query
              .getCalls()
              .filter((call) => call.args[0].match(/system_schema.tables/)),
            100,
          );
    ...
    describe("with C*2.0 metadata rows", function () {
      ...
      it("should parse table with compact storage with clustering key", function (done) {
        const tableRow = {
          ...
          bloom_filter_fp_chance: 0.01,
          caching: '{"keys":"ALL", "rows_per_partition":"NONE"}',
          cf_id: types.Uuid.fromString("96c86920-ed84-11e4-8991-199e66562428"),
          column_aliases: '["id2"]',
          comment: "",
          compaction_strategy_class:
            "org.apache.cassandra.db.compaction.SizeTieredCompactionStrategy",
      ...
      it("should parse custom index (legacy)", function (done) {
        ...
        const columnRows = [
          ...
          },
          {
            ...
            index_options: '{"foo":"bar", "class_name":"dummy.DummyIndex"}',
            index_type: "CUSTOM",
            type: "regular",
            validator: "org.apache.cassandra.db.marshal.UTF8Type",
          },
    ...
    describe("with C*2.2 metadata rows", function () {
      it("should parse new 2.2 types", function (done) {
        ...
        const tableRow = {
          ...
          caching: '{"keys":"ALL", "rows_per_partition":"NONE"}',
          cf_id: types.Uuid.fromString("c05f4c40-fe05-11e4-8481-277ff03b5030"),
          comment: "",
          compaction_strategy_class:
            "org.apache.cassandra.db.compaction.SizeTieredCompactionStrategy",
          compaction_strategy_options: "{}",
        ...
        const columnRows = [
          {
            ...
            index_type: null,
            type: "regular",
            validator: "org.apache.cassandra.db.marshal.SimpleDateType",
          },
          {
            keyspace_name: "ks_tbl_meta",
            columnfamily_name: "tbl_c22",
    ...
    describe("with C*3.0+ metadata rows", function () {
      ...
      it("should parse with custom index", function (done) {
        ...
        const metadata = new Metadata(
          clientOptions.defaultOptions(),
          getControlConnectionForTable(tableRow, columnRows, indexRows),
        );
        metadata.initialized = true;
        metadata.setCassandraVersion([3, 0]);
        metadata.keyspaces = { ks_tbl_meta: { tables: {} } };
        ...
        const rows = [
          {
            ...
            aggregate_name: "sum",
            signature: ["bigint"],
            argument_types: ["org.apache.cassandra.db.marshal.LongType"],
            final_func: null,
            initcond: utils.allocBufferFromArray([0, 0, 0, 0, 0, 0, 0, 0]),
            return_type: "org.apache.cassandra.db.marshal.LongType",
            state_func: "plus",
      ...
      it("should parse aggregate metadata with 1 parameter", function (done) {
        const rows = [
          ...
          },
          {
            ...
            aggregate_name: "sum",
            signature: ["int"],
            argument_types: ["org.apache.cassandra.db.marshal.Int32Type"],
            final_func: null,
    ...
    it("should set sorted tokens from a single host", function (done) {
      ...
      const expectedRanges = new Set(
        [
          ...
          [25, 400],
          [400, 5],
        ].map((r) => new TokenRange(token(r[0]), token(r[1]), tokenizer)),
      );
  ...
  describe("#compareSchemaVersions()", function () {
    ...
    it("should return false when versions do not match", () => {
      ...
      const cc = {
        connection: {
          send: () =>
            Promise.resolve({ rows: [{ schema_version: String(index++) }] }),
        },
      };
...
function expectRanges(metadata, keyspace, host, expectedRanges) {
  const eRanges = new Set(
    expectedRanges.map(
      (tokens) =>
        new TokenRange(token(tokens[0]), token(tokens[1]), metadata.tokenizer),
    ),
  );
