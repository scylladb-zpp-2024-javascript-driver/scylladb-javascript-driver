...
describe("executeConcurrent(client, query, parameters)", function () {
  ...
  it("should validate parameters", () => {
    assert.throws(
      () => executeConcurrent(null),
      TypeError,
      /Client instance is not defined/,
    );
  ...
  it("should support a higher concurrency level than the number of items", () => {
    ...
    return executeConcurrent(testContext.client, "Q1", parameters, {
      ...
      assert.deepStrictEqual(
        result.resultItems.map((x) => x.rows[0]["col1"]),
        parameters,
      );
  ...
  it("should support empty parameters array", () => {
    ...
    return executeConcurrent(testContext.client, "Q1", []).then((result) =>
  ...
  context("when collectResults is false", () => {
    it("should throw when accessing resultItems property", () => {
      ...
      return executeConcurrent(testContext.client, "Q1", parameters).then(
        (result) =>
          assert.throws(
            () => result.resultItems,
  ...
  context("when raiseOnFirstError is true", () => {
    it("should throw when first error is encountered", () => {
      ...
      return (
        executeConcurrent(testContext.client, "Q1", parameters, {
          raiseOnFirstError: true,
          concurrencyLevel,
        })
          .catch((err) => (error = err))
          .then(() => {
      ...
      return executeConcurrent(testContext.client, "Q1", parameters, {
        ...
        .then((result) => {
          ...
          assert.deepStrictEqual(
            ...
            errorIndexes.map((i) => `Test error ${i}`),
          );
        });
    });
...
describe("executeConcurrent(client, query, stream)", () => {
  it("should support an empty stream", () => {
    ...
    return executeConcurrent(testContext.client, "INSERT...", stream).then(
      (result) => {
        assert.strictEqual(result.totalExecuted, 0);
  ...
  [
    {
      params: [["a"]],
      description: "when there is an invalid item on the first position",
    },
    {
      params: [[[1, 2], "a"]],
      description: "when there is an invalid item on the second position",
    },
    {
      params: [
        [
      ...
      description: "when there is an invalid item pushed later in time",
    },
  ].forEach((item) => {
  ...
  it("should stop reading according to the concurrency level", () => {
    ...
    return executeConcurrent(testContext.client, "INSERT...", stream, {
      ...
      assert.ok(
        stream.pauseCounter > Math.floor(values.length / concurrencyLevel) - 1,
      );
    });
  });
  ...
  context("when collectResults is false", () => {
    it("should throw when accessing resultItems property", () => {
      ...
      return executeConcurrent(testContext.client, "Q1", stream, {
  ...
  context("when raiseOnFirstError is true", () => {
    it("should throw when first error is encountered", () => {
      ...
      return (
        executeConcurrent(testContext.client, "Q1", stream, {
          raiseOnFirstError: true,
          concurrencyLevel,
        })
          .catch((err) => (error = err))
...
describe("executeConcurrent(client, queryAndParameters)", () => {
  it("should use the different query and parameters", () => {
    ...
    return executeConcurrent(testContext.client, queryAndParams).then(
      (result) => {
        assert.strictEqual(result.totalExecuted, queryAndParams.length);
        assert.deepStrictEqual(
          testContext.executions.map((x) => ({
            query: x.query,
            params: x.params,
          })),
          queryAndParams,
  ...
  it("should set the execution profile", () => {
    ...
    return executeConcurrent(testContext.client, queryAndParams, options).then(
      (result) => {
        assert.strictEqual(result.totalExecuted, queryAndParams.length);
        testContext.executions.forEach((item) => {
          assert.strictEqual(
  ...
  const testContext = {
    ...
    client: {
      execute: function (query, params, options) {
        ...
        testContext.maxInFlight = Math.max(
          ++testContext.inFlight,
          testContext.maxInFlight,
        );
...
class TestStream extends Readable {
  constructor(values, options) {
    ...
    const baseDelay = 20;
    const endAsync =
      typeof options.endAsync === "boolean" ? options.endAsync : true;
