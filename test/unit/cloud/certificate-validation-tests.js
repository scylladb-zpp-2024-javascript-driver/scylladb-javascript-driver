...
describe("checkServerIdentity()", () => {
  ...
  it("should not match incorrect domains in the common name", () => {
    wrongNames.forEach((name) => {
      ...
      assert.strictEqual(
        err.reason,
        `Host: ${sampleHostName} is not cert's CN/altnames: ${name} / undefined`,
      );
      assert.ok(err.cert);
  ...
  it("should not match incorrect domains in the alt names", () => {
    wrongNames.forEach((name) => {
      const err = checkServerIdentity(
        { subject: { CN: "xyz.org" }, subjectaltname: name },
        `${sampleHostName}:${port}`,
      );
      assert.instanceOf(err, Error);
      assert.strictEqual(
        err.reason,
        `Host: ${sampleHostName} is not cert's CN/altnames: xyz.org / ${name}`,
      );
      assert.ok(err.cert);
      assert.strictEqual(err.host, sampleHostName);
    });
  ...
  it("should allow certificate common name with wildcard", () => {
    [
      [
        ...
        [
          "host1.datastax.com",
          "cec95af1-a06c-42e1-9e59-068bb9be11f7.datastax.com",
        ],
      ],
      [
  ...
  it("should allow certificate alt names with wildcard", () => {
    [
      ["DNS:*.nodejs.org, DNS:nodejs.org", ["nodejs.org", "www.nodejs.org"]],
      [
      ...
      hostNames.forEach((hostName) =>
        assert.isUndefined(
          checkServerIdentity(
            { subject: { CN: "xyz.org" }, subjectaltname: item[0] },
            `${hostName}:${port}`,
          ),
        ),
      );
