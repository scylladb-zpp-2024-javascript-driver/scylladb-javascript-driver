    ...
    describe("fromString()", function () {
        it("should parse from string representation for decimal numbers", function () {
            [
                ...
                const expected = new MutableLong(
                    item[1][0],
                    item[1][1],
                    item[1][2],
                    item[1][3],
    ...
    describe("#multiply()", function () {
        it("should return the product for values < 2^53", function () {
            [
                ...
                assert.ok(
                    a.multiply(b).equals(MutableLong.fromNumber(expected)),
                    format("failed for value %d*%d", item[0], item[1]),
                );
            });
        });
        it("should return the product for int64 values", function () {
            [
                // [a, b, c]
                [
                    new MutableLong(0x4f23, 0xff32, 0x4220, 0x0d32),
                    new MutableLong(0x07),
                    new MutableLong(0x29f5, 0xfa60, 0xcee6, 0x5c5f),
                ],
                [
                    new MutableLong(0x2ebf, 0x4611, 0x9867, 0xffff),
                    new MutableLong(0x030),
                    new MutableLong(0xc3d0, 0x2338, 0x935d, 0xffec),
                ],
                [
                    MutableLong.fromBits(0xe084bca4, 0x28b1),
                    MutableLong.fromBits(0xed558ccd, 0xff51afd7),
                    MutableLong.fromBits(0xd7e8bf54, 0x9e9ed5ab),
                ],
            ].forEach(function (item) {
                const a = Long.fromBits(
                    item[0].getLowBitsUnsigned(),
                    item[0].getHighBitsUnsigned(),
                    false,
                );
                const b = Long.fromBits(
                    item[1].getLowBitsUnsigned(),
                    item[1].getHighBitsUnsigned(),
                    false,
                );
                const c = Long.fromBits(
                    item[2].getLowBitsUnsigned(),
                    item[2].getHighBitsUnsigned(),
                    false,
                );
    ...
    describe("#shiftRightUnsigned()", function () {
        it("should shift across int16 blocks", function () {
            for (let i = 1; i < 64; i++) {
                ...
                if (i < 32) {
                    expectedLow =
                        (expectedLow >>> i) | (expectedHigh << (32 - i));
                ...
                assert.strictEqual(
                    l.getLowBitsUnsigned().toString(16),
                    expectedLow.toString(16),
                );
                assert.strictEqual(
                    l.getHighBitsUnsigned().toString(16),
                    expectedHigh.toString(16),
                );
            }
        });
    });
    ...
    describe("#compare()", function () {
        it("should compare the values provided", function () {
            [
                ...
                [
                    new MutableLong(0xffff, 0xffff, 0xffff, 0xffff),
                    new MutableLong(0xfffe, 0xffff, 0xffff, 0xffff),
                ],
                [
                    new MutableLong(),
                    new MutableLong(0xfffe, 0xffff, 0xffff, 0xffff),
