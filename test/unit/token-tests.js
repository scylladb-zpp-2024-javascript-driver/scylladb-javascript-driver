...
class TokenTester {
    ...
    rangeSplitTester(start, end, numberOfSplits, expectedRanges) {
        return () => {
            ...
            const range = new token.TokenRange(
                startToken,
                endToken,
                this.tokenizer,
            );
...
describe("TokenRange", () => {
    ...
    describe("with Murmur3Token", () => {
        ...
        describe("#splitEvenly()", () => {
            ...
            it(
                ...
                tester.rangeSplitTester("4611686018427387904", "0", 3, [
            ...
            it(
                ...
                tester.rangeSplitTester("0", "11", 3, [
                    ["0", "4"],
                    ["4", "8"],
                    ["8", "11"],
                ]),
    ...
    describe("with ByteOrderedToken", () => {
        ...
        describe("#splitEvenly()", () => {
            it(
                "should split range",
                tester.rangeSplitTester(hex("a"), hex("d"), 3, [
                    [hex("a"), hex("b")],
                    [hex("b"), hex("c")],
                    [hex("c"), hex("d")],
                ]),
            ...
            it(
                "should split range that wraps around the ring",
                tester.rangeSplitTester(hex("d"), hex("a"), 3, [
                    [hex("d"), "B9"],
                    ["B9", "0D"],
                    ["0D", hex("a")],
                ]),
            ...
            it("should not allow split on minToken", () => {
                ...
                assert.throws(
                    ...
                    (err) =>
                        ...
                        err.message ===
                            "Cannot split whole ring with ordered partitioner",
                );
            });
        });
        describe("#compare()", () => {
            const token0 = _tokenizer.parse("00");
            const token1 = _tokenizer.parse(
                "0113427455640312821154458202477256070484",
            );
            const token2 = _tokenizer.parse(
                "56713727820156410577229101238628035242",
            );
    ...
    describe("#unwrap()", () => {
        ...
        it("should return two ranges when range is wrapped around ring", () => {
            ...
            const expectedRange0 = new token.TokenRange(
                token1,
                minToken,
                _tokenizer,
            );
            const expectedRange1 = new token.TokenRange(
                minToken,
                token0,
                _tokenizer,
            );
