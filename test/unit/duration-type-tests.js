...
const values = [
  ...
  [
    new Duration(0, 0, Long.fromNumber(2251799813685279)),
    "0000fe1000000000003e",
  ],
  ...
  [
    new Duration(0, 2, Long.fromNumber(120000000000)),
    "0004f837e11d6000",
    "2d2m",
    "P2DT2M",
  ],
  [
    new Duration(0, 0, Long.fromString("9223372036854775805")),
    "0000fffffffffffffffffa",
  ],
  [
    new Duration(0, 0, Long.fromString("-9223372036854775808")),
    "0000ffffffffffffffffff",
  ],
  [
...
describe("Duration", function () {
  ...
  describe("fromBuffer()", function () {
    it("should decode the vint values", function () {
      values.forEach(function (item) {
        assertDurationEqual(
          Duration.fromBuffer(utils.allocBufferFromString(item[1], "hex")),
          item[0],
          item[1],
        );
      });
  ...
  describe("fromString()", function () {
    [
      ...
      it(testInfo[0], function () {
        ...
        values.forEach(function (item) {
          ...
          assertDurationEqual(
            Duration.fromString(item[index]),
            item[0],
            item[index],
          );
        });
    ...
    it("should throw TypeError for invalid strings", function () {
      [
        ...
        const expectedError = testInfo.startsWith("-")
          ? testInfo.substr(1)
          : testInfo;
        assert.throws(
          function () {
            Duration.fromString(testInfo);
          },
          (err) =>
            err instanceof TypeError &&
            err.message ===
              "Unable to convert '" + expectedError + "' to a duration",
        );
      });
    });
  });
...
function assertDurationEqual(actual, expected, input) {
  assert.ok(
    ...
    util.format("%j not equals to %j, from %s", actual, expected, input),
  );
}
