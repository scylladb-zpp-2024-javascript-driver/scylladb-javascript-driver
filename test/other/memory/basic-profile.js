...
let client = new Client(
  utils.extend({ encoding: { copyBuffer: true } }, helper.baseOptions),
);
...
utils.series(
  [
    ...
    function (next) {
      client.execute(
        helper.createKeyspaceCql(keyspace, 2),
    ...
    function insertData(next) {
      ...
      const query = util.format(
        "INSERT INTO %s (id, int_sample, blob_sample) VALUES (?, ?, ?)",
        table,
      );
      let counter = 0;
      ...
      utils.timesLimit(
        10000,
        500,
        function (v, timesNext) {
          const n = counter++;
          const buffer = utils.allocBufferFromString(
            generateAsciiString(1024),
            "utf8",
          );
          client.execute(
            query,
            [types.Uuid.random(), n, buffer],
            { prepare: true },
            function (err) {
    ...
    function selectData(next) {
      ...
      client.eachRow(
        ...
        function (err, result) {
          ...
          console.log(
            util.format(
              "Retrieved %d rows and around %s",
              result.rowLength,
              formatLength(totalByteLength),
            ),
          );
          next();
        },
      ...
      setImmediate(function () {
        ...
        if (heapdump) {
          heapdump.writeSnapshot(
            heapdumpPath + "/" + Date.now() + ".heapsnapshot",
          );
        }
        helper.ccmHelper.removeIfAny();
        assert.ifError(err);
