...
async function myTest(client: Client): Promise<any> {
  ...
  o = await videoMapper.get({ name: "b" }, undefined, {
    executionProfile: "ep1",
  });
  o = await videoMapper.get({ userId: 1 }, {}, { isIdempotent: true });

  result = await videoMapper.find(
    { name: "a" },
    { fields: ["videoId"], limit: 10, orderBy: { name: "asc" } },
  );
  ...
  result = await videoMapper.insert(
    { name: "a" },
    { ifNotExists: true },
    "ep1",
  );

  result = await videoMapper.update({
    videoId: Uuid.random(),
    userId: 1,
    name: "a",
  });
  ...
  result = await videoMapper.update(
    { name: "a" },
    { ttl: 123, ifExists: true },
    { isIdempotent: true, executionProfile: "ep2" },
  );

  result = await videoMapper.remove({ videoId: Uuid.random() });
  result = await videoMapper.remove(
    { videoId: Uuid.random() },
    { ifExists: true },
  );
}
