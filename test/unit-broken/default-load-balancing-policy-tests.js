...
describe("DefaultLoadBalancingPolicy", () => {
    describe("constructor", () => {
        it("should support providing the localDc as a string instead of the options", () => {
            assert.strictEqual(
                new DefaultLoadBalancingPolicy("my-local-dc").localDc,
                "my-local-dc",
            );
        });
        ...
        it("should support providing the localDc in the options", () => {
            assert.strictEqual(
                new DefaultLoadBalancingPolicy({ localDc: "my-local-dc2" })
                    .localDc,
                "my-local-dc2",
            );
        });
    ...
    describe("#newQueryPlan()", () => {
        context("when keyspace or routing key is not defined", () => {
            it("should return only local nodes and balance the load", () => {
                ...
                return getQueryPlan(policy, localDcLength * occurrences).then(
                    (result) => {
                        assert.strictEqual(
                            result.length,
                            localDcLength * occurrences,
                        );
            ...
            it("should yield preferredHost first when defined", () =>
                testPreferredHost(false));
        });
        ...
        it("should return local replicas first and balance the load based on comparer", () => {
            ...
            const compareFn = (h1, h2) =>
                lastOctetOf(h1) > lastOctetOf(h2) ? 1 : -1;

            const policy = getNewInstance({
                local: localDcLength,
                remote: 3,
                getReplicas: (hostMap) =>
                    hostMap.values().filter(replicaCondition),
                compare: (h1, h2) => compareFn(h1, h2, comparerCalled++),
            });

            const repeat = 500;
            return getQueryPlan(policy, repeat, "ks1", routingKey).then(
                (result) => {
                    ...
                    result.forEach((hosts) => {
                        ...
                        assert.strictEqual(
                            replicas.filter(replicaCondition).length,
                            expectedReplicas,
                        );
                        assert.strictEqual(
                            hosts
                                .slice(expectedReplicas)
                                .filter(replicaCondition).length,
                            0,
                        );
        ...
        it("should yield preferredHost first when defined", () =>
            testPreferredHost(true));

        it("should yield preferredHost first when host is remote", () =>
            testPreferredHost(true, true));
        ...
        it("should send unhealthy replicas to the back of the list", () => {
            ...
            const policy = getNewInstance({
                ...
                compare: (h1, h2) => compareFn(h1, h2, comparerCalled++),
                healthCheck: (h) =>
                    unhealthyReplicas.indexOf(~~lastOctetOf(h)) === -1,
            });
                ...
                (result) => {
                    ...
                    result.forEach((hosts) => {
                        ...
                        assert.deepStrictEqual(
                            replicas
                                .slice(
                                    expectedReplicas - unhealthyReplicas.length,
                                    expectedReplicas,
                                )
                                .map((h) => ~~lastOctetOf(h))
                                .filter(
                                    (a) => unhealthyReplicas.indexOf(a) >= 0,
                                )
                                .sort(),
                            unhealthyReplicas,
                        );
            ...
            return getQueryPlan(policy, repeat, "ks1", routingKey).then(
                (result) => {
                    ...
                    result.forEach((hosts) => {
                        ...
                        assert.strictEqual(
                            hosts.filter((h) => filter(h)).length,
                            localDcLength - 1,
                        );
        ...
        it("should target newly up replicas fewer times than other replicas", () => {
            ...
            return getQueryPlan(policy, repeat, "ks1", routingKey).then(
                (result) => {
                    ...
                    const newlyUpAsCoordinator = result.filter(
                        (hosts) => ~~lastOctetOf(hosts[0]) === newlyUpReplica,
                    ).length;
                    assert.ok(
                        newlyUpAsCoordinator * 2 < repeat / expectedReplicas,
                        util.format(
                            "%d expected to be less than half than %d",
                            newlyUpAsCoordinator,
                            repeat / expectedReplicas,
                        ),
                    );
                },
            );
    ...
    describe("#getDistance()", () => {
        ...
        it("should mark preferredHost as local", () => {
            ...
            assert.strictEqual(
                policy.getDistance(preferredHost),
                types.distance.local,
            );
...
function testPreferredHost(useReplicas, isPreferredRemote) {
    ...
    const preferredHost = policy.getTestHostMap().values()[
        isPreferredRemote ? localDcLength : 2
    ];
