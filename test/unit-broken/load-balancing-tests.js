            ...
            [
                ...
                function checkNewQueryPlan(next) {
                    policy.newQueryPlan(null, null, function (err, iterator) {
                        assert.ifError(err);
                        assert.strictEqual(
                            utils.iteratorToArray(iterator).length,
                            3,
                        );
                        next();
                    });
            ...
            [
                ...
                function (next) {
                    policy.newQueryPlan(
                        null,
                        getExecOptions({
                            routingKey: utils.allocBufferUnsafe(16),
                        }),
                        function (err, iterator) {
                            ...
                            assert.deepEqual(
                                hosts.map(toAddress).slice(0, 2).sort(),
                                ["repl2_local", "repl4_local"],
                            );
                    ...
                    utils.timesLimit(
                        ...
                        function (n, timesNext) {
                            policy.newQueryPlan(
                                ...
                                function (err, iterator) {
                                    const hosts =
                                        helper.iteratorToArray(iterator);
                                    assert.strictEqual(hosts.length, 5);
                                    assert.deepEqual(
                                        hosts.map(toAddress).slice(0, 3).sort(),
                                        [
                                            "repl2_local",
                                            "repl4_local",
                                            "repl5_local",
