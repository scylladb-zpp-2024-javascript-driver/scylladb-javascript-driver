...
describe("ModelMapper", () => {
    describe("#find()", () => {
        ...
        it("should support fields to specify the selection columns", () =>
            testQueries("find", [
                {
                    doc: { id1: "value1", id2: q.gte("e") },
                    docInfo: {
                        fields: ["name", "description", "locationType"],
                    },
        ...
        it("should support relational operators, orderBy, limit and fields", () =>
            testQueries("find", [
                {
                ...
                },
                {
                    doc: {
                        id1: "value1",
                        id2: q.and(q.gte("a"), q.and(q.gte("e"), q.lt("z"))),
                    },
        ...
        it("should support orderBy in correct order", () =>
            testQueries("find", [
                {
                    doc: { id1: "value2" },
                    docInfo: { orderBy: { id2: "desc" } },
                    query: 'SELECT * FROM ks1.table1 WHERE "id1" = ? ORDER BY "id2" DESC',
                    params: ["value2"],
                },
                {
                    doc: { id1: "value3" },
                    docInfo: { orderBy: { id2: "asc" } },
                    query: 'SELECT * FROM ks1.table1 WHERE "id1" = ? ORDER BY "id2" ASC',
                    params: ["value3"],
                },
                {
                    doc: { id1: "value1", id2: q.gte("e") },
                    ...
                    query: 'SELECT "name" FROM ks1.table1 WHERE "id1" = ? AND "id2" >= ? ORDER BY "id2" ASC LIMIT ?',
                    params: ["value1", "e", 20],
                },
            ]));

        it("should support ordering by clustering key only providing a partition key", () =>
            testQueries("find", [
                {
                    doc: { id1: "value2" },
                    docInfo: { orderBy: { id2: "asc" } },
        ...
        it("should support mapping IN values with a mapping function", () =>
            testQueries({
                methodName: "find",
                models: {
                    Sample: {
                ...
                items: [
                    {
                        doc: {
                            id1: "value_id1",
                            id2: q.in_(["first", "second"]),
                        },
                        query: 'SELECT * FROM ks1.table1 WHERE "id1" = ? AND "id2" IN ?',
                        params: [
                            "value_id1",
                            ["first_mapped_value", "second_mapped_value"],
                        ],
                    },
                ],
            }));
                ...
                [
                    {
                        ...
                        message:
                            "No table matches the filter (PKs): [id1,notAValidProp]",
                    },
                    {
                        doc: { id1: "x" },
                        docInfo: { fields: ["notAValidProp"] },
                        message:
                            "No table matches the filter (PKs): [id1]; fields: [notAValidProp]",
                    },
                    {
                        doc: { id1: "x", name: "y" },
                        message:
                            "No table matches the filter (PKs): [id1,name]",
                    },
                    {
                        doc: { id1: "x" },
                        docInfo: { orderBy: { notAValidProp: "asc" } },
                        message:
                            "No table matches the filter (PKs): [id1]; orderBy: [notAValidProp]",
                    },
                ].map((item) => {
                    ...
                    const modelMapper =
                        mapperTestHelper.getModelMapper(clientInfo);

                    let catchCalled = false;

                    return modelMapper
                        .find(item.doc, item.docInfo)
                        .catch((err) => {
                            catchCalled = true;
        ...
        it("should use the default table name based on the model name", () => {
            ...
            return modelMapper.find({ id1: "x", id2: "y" }).then(() => {
                ...
                assert.strictEqual(
                    execution.query,
                    'SELECT * FROM ks1.NoTableSpecified WHERE "id1" = ? AND "id2" = ?',
                );
            });
        });
        ...
        it("should use the correct table when order by a clustering key", () => {
            ...
            function getTableMetadata(ks, name) {
                ...
                const columns = ["id1", "id2", "id3", "value"].map((c) => ({
                    name: c,
                    type: { code: dataTypes.text },
                }));
            ...
            const modelMapper = mapperTestHelper.getModelMapper(
                clientInfo,
                models,
            );
            ...
            return modelMapper
                ...
                .then(() =>
                    modelMapper.find(
                        { id1: "a" },
                        { orderBy: { id2: "desc", id3: "desc" } },
                    ),
                )
                .then(() =>
                    modelMapper.find({ id1: "a" }, { orderBy: { id2: "asc" } }),
                )
                .then(() => {
                    [
                        ...
                        assert.strictEqual(
                            clientInfo.executions[index].query,
                            query,
                        );
                        assert.deepStrictEqual(
                            clientInfo.executions[index].params,
                            ["a"],
                        );
                    });
                });
        ...
        it("should only execute the query if all PKs are defined", () => {
            ...
            return modelMapper
                ...
                .catch((err) => {
                    ...
                    assert.strictEqual(
                        err.message,
                        "No table matches the filter (all PKs have to be specified): [id1]",
                    );
                })
                .then(() => assert.strictEqual(catchCalled, true));
        });
    ...
    describe("#mapWithQuery", () => {
        it("should warn when cache reaches 100 different queries", async () => {
            ...
            for (let i = 0; i < 2 * cacheHighWaterMark; i++) {
                const executor = modelMapper.mapWithQuery(
                    `query-${i % (cacheHighWaterMark - 1)}`,
                    () => [],
                );
            ...
            assert.strictEqual(
                clientInfo.logMessages.filter((l) => l.level === "warning")
                    .length,
                0,
            );
