...
describe("Client", function () {
    ...
    describe("#stream(query, params, {prepare: 0})", function () {
        ...
        it("should emit end when no rows", function (done) {
            ...
            const stream = client.stream(helper.queries.basicNoResults, [], {
                prepare: false,
            });
            stream
                .on("end", done)
    ...
    describe("#stream(query, params, {prepare: 1})", function () {
            ...
            utils.series(
                [
                    ...
                    helper.toTask(
                        client.execute,
                        client,
                        helper.createKeyspaceCql(commonKs, 3),
                    ),
                    helper.toTask(
                        client.execute,
                        client,
                    ...
                    function (next) {
                        utils.timesLimit(
                            ...
                            function (n, timesNext) {
                                let query =
                                    "INSERT INTO %s (id, int_sample, bigint_sample) VALUES (%s, %d, %s)";
                                query = util.format(
                                    query,
                                    table,
                                    types.Uuid.random(),
                                    n,
                                    new types.Long(n, 0x090807).toString(),
                                );
        ...
        it("should emit argument parsing errors", function (done) {
            ...
            const stream = client.stream(
                helper.queries.basic + " WHERE key = ?",
                [{}],
                { prepare: 1 },
            );
            let errCalled = false;
        ...
        it("should wait buffer until read", function (done) {
            const client = newInstance();
            let allRead = false;
            const stream = client.stream(helper.queries.basic, null, {
                prepare: 1,
            });
            stream
                .on("end", function () {
            ...
            utils.series(
                [
                    function insert(next) {
                        ...
                        utils.timesLimit(
                            ...
                            function (n, timesNext) {
                                client.execute(
                                    query,
                                    [id, types.TimeUuid.now(), n.toString()],
                    ...
                    function testBuffering(next) {
                        const query = util.format(
                            "SELECT id2, text_sample from %s WHERE id1 = ?",
                            commonTable,
                        );
                        const stream = client.stream(query, [id], {
                            prepare: true,
                            fetchSize: fetchSize,
