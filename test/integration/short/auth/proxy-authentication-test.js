...
const cDescribe = helper.conditionalDescribe(
    helper.requireOptional("kerberos"),
    "kerberos required to run",
);
...
vdescribe("dse-5.1", "Proxy Authentication @SERVER_API", function () {
    ...
    before(function (done) {
        const client = new Client(
            helper.getOptions({
                authProvider: new DsePlainTextAuthProvider(
                    "cassandra",
                    "cassandra",
                ),
            }),
    ...
    describe("with DsePlainTextAuthProvider", function () {
        it("should allow plain text authorized user to login as", function (done) {
            connectAndQuery(
                new DsePlainTextAuthProvider("ben", "ben", "alice"),
                null,
                done,
            );
        });
        it("should allow plain text authorized user to execute as", function (done) {
            connectAndQuery(
                new DsePlainTextAuthProvider("steve", "steve"),
                "alice",
                done,
            );
        });
        it("should not allow plain text unauthorized user to login as", function (done) {
            connectAndQuery(
                new DsePlainTextAuthProvider("steve", "steve", "alice"),
                null,
                function (err) {
        ...
        it("should not allow plain text unauthorized user to execute as", function (done) {
            connectAndQuery(
                new DsePlainTextAuthProvider("ben", "ben"),
                "alice",
                function (err) {
                    ...
                    function (next) {
                        connectAndQuery(
                            ...
                            function (err) {
                                ...
                                const innerErrors = utils.objectValues(
                                    err.innerErrors,
                                );
                                assert.strictEqual(innerErrors.length, 1);
                                helper.assertInstanceOf(
...
function connectAndQuery(authProvider, executeAs, callback) {
    ...
    client.execute(
        "SELECT * FROM aliceks.alicetable",
        null,
        options,
        function (err, result) {
            client.shutdown();
