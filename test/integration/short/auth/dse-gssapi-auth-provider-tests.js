...
const cDescribe = helper.conditionalDescribe(
    helper.requireOptional("kerberos") && helper.isDseGreaterThan("5.0"),
    "kerberos and DSE required to run",
);
...
cDescribe("DseGssapiAuthProvider @SERVER_API", function () {
    ...
    vit(
        "dse-5.1",
        "should authenticate against DSE instance using Kerberos",
        (done) => {
    ...
    vit(
        "dse-5.1",
        "should fail with auth errors when authProvider is undefined",
        () => {
            const clientOptions = helper.getOptions({
