...
describe("pool", function () {
  ...
  context("with a 3-node simulated cluster", function () {
      ...
      return client
        ...
        .then(() =>
          Promise.all(
            new Array(150)
              .fill(null)
              .map(() => client.execute("SELECT * FROM paused")),
          ),
        )
        .then((results) => {
          ...
          assertArray(
            results.slice(0, 100).map((rs) => rs.info.queriedHost),
            hosts[0],
          );
          // The next 50 items should be the second host
          assertArray(
            results.slice(100).map((rs) => rs.info.queriedHost),
            hosts[1],
          );
        })
    ...
    it("should fail when all hosts are busy", function () {
      return client
        ...
        .then(() =>
          Promise.all(
            new Array(350).fill(null).map(() => {
              const resultTuple = {
                err: null,
                coordinator: null,
              };
              return client
                .execute("SELECT * FROM paused")
        ...
        .then((results) => {
          ...
          assertArray(
            results.slice(0, 100).map((r) => r.coordinator),
            hosts[0],
          );
          assertArray(
            results.slice(100, 200).map((r) => r.coordinator),
            hosts[1],
          );
          assertArray(
            results.slice(200, 300).map((r) => r.coordinator),
            hosts[2],
          );
          ...
          results.slice(300).forEach((r) => {
            ...
            assert.deepEqual(
              Object.keys(r.err.innerErrors).sort(),
              hosts.sort(),
            );
      ...
      return client
        ...
        .then(() =>
          Promise.all(
            new Array(150)
              .fill(null)
              .map(() => client.execute("SELECT * FROM system.local")),
          ),
        )
        .then(() => {
          assert.ok(
            client.hosts.values().filter((h) => h.getResponseCount() > 1)
              .length > 0,
          );
        })
        .then(
          () =>
            new Promise((resolve) =>
              setTimeout(resolve, healthResponseCountInterval * 2 + 20),
            ),
        )
        .then(() => {
          // After some time passed without any activity, the counter values should be 0.
          assert.strictEqual(
            client.hosts.values().filter((h) => h.getResponseCount() !== 0)
              .length,
            0,
          );
        });
    ...
    it("should retry on the same host when defined by the retry policy", () => {
      ...
      return client
        ...
        .then(() =>
          client.execute("SELECT * FROM paused_on_first", null, {
            readTimeout: 200,
            isIdempotent: true,
          }),
        )
      ...
      return (
        client
          ...
          .then(() => {
            ...
            const p2 = client.execute("SELECT * FROM paused_on_first", null, {
              readTimeout: 50,
              isIdempotent: true,
            });
            promises.push(p2);
          ...
          .then((rs) =>
            assert.strictEqual(rs.info.queriedHost, cluster.node(1).address),
          )
          .then(() => {
          ...
          .then(() =>
            helper.setIntervalUntilPromise(
              () => firstHost.getInFlight() === 0,
              10,
              100,
            ),
          )
    ...
    it("should reconnect in the background when hosts are back online", async () => {
      const client = newInstance(cluster, {
        policies: {
          reconnection: new policies.reconnection.ConstantReconnectionPolicy(
            200,
          ),
        },
      ...
      await Promise.all(
        [0, 1, 2].map((n) =>
          promiseUtils.fromCallback((cb) => cluster.dc(0).node(n).stop(cb)),
        ),
      );
      ...
      assert.deepStrictEqual(
        client.hosts.values().map((h) => h.isUp()),
        [false, false, false],
      );
      ...
      await Promise.all(
        [0, 1, 2].map((n) =>
          promiseUtils.fromCallback((cb) => cluster.dc(0).node(n).start(cb)),
        ),
      );
      ...
      client.hosts
        .values()
        .forEach((h) => assert.strictEqual(1, h.pool.connections.length));
    ...
    it("should connect when first contact point is down", () => {
      ...
      return promiseUtils
        .fromCallback((cb) => cluster.dc(0).node(0).stop(cb))
        .then(() => client.connect())
        .then(() =>
          assert.strictEqual(
    ...
    it("should have a single connection when warmup is false", async () => {
      ...
      const client = new Client({
        ...
        pooling: {
          warmup: false,
          coreConnectionsPerHost: { [types.distance.local]: 3 },
        },
      });
    ...
    it("should maintain the same ControlConnection host after nodes going UP and DOWN and back UP", () => {
      ...
      return client
        ...
        .then(() =>
          promiseUtils.fromCallback((cb) =>
            cluster.node(firstAddress).stop(cb),
          ),
        )
        .then(() =>
          helper.setIntervalUntilPromise(
            () =>
              (client.controlConnection.getEndpoint() || firstAddress) !==
              firstAddress,
            20,
            5000,
          ),
        ...
        .then(() =>
          promiseUtils.fromCallback((cb) =>
            cluster.node(firstAddress).start(cb),
          ),
        )
        .then(() =>
          helper.setIntervalUntilPromise(
            () => client.hosts.values().find((h) => !h.isUp()) === undefined,
            20,
            5000,
          ),
        )
    ...
    it("should create all connections eventually when warmup is false", async () => {
      ...
      await helper.wait.until(
        () =>
          hosts.reduce((count, h) => count + h.pool.connections.length, 0) ===
  ...
  context(
    "with a simulated cluster containing three nodes on each of the two datacenters",
    () => {
      let cluster;
      ...
      it("should not attempt to make a connection on a new node that is on an ignored/remote DC", () => {
        ...
        const addressTranslator =
          new CustomTestAddressTranslator().withTranslations(
            new Map([[node2Address, node1Address]]),
          );
        ...
        return client
          ...
          .then(() => {
            ...
            client.hosts.forEach((host) => {
              const expectedConnections =
                lbp.getDistance(host) === types.distance.local ? 1 : 0;
              assert.strictEqual(
            ...
            return promiseUtils.fromCallback((cb) =>
              cluster.dc(0).node(0).stop(cb),
            );
          })
          .then(() =>
            // Force the driver to acknowledge that the node is stopped (avoid half open)
            client
              .execute(helper.queries.basic, null, {
                host: initialControlConnectionHost,
              })
              .catch(() => {}),
          )
          .then(() =>
            ...
            helper.setIntervalUntilPromise(
              () =>
                client.controlConnection.host !== initialControlConnectionHost,
              500,
              20,
            ),
          )
          ...
          .then(() =>
            ...
            helper.setIntervalUntilPromise(
              () => initialControlConnectionHost.pool.connections.length > 0,
              500,
              40,
            ),
          )
          ...
          .then(() => {
            ...
            assert.strictEqual(
              client.hosts.get(newNodeAddress).pool.connections.length,
              0,
            );
          })
          .then(() => client.shutdown());
      ...
      it("should not create connections to nodes not selected by the load balancing policy on init", async () => {
        ...
        const client = newInstance(cluster, {
          policies: {
            loadBalancing: new helper.OrderedLoadBalancingPolicy(addresses),
          },
        });
      ...
      it("should not create connections to remote nodes when core connections per host is zero", async () => {
        ...
        const client = newInstance(cluster, {
          policies: {
            loadBalancing: new CustomLoadBalancingPolicy(
              localAddresses,
              remoteAddresses,
            ),
          },
        ...
        assert.lengthOf(
          client.hosts
            .values()
            .filter((h) => h._distance === types.distance.remote),
          2,
        );
      ...
      [true, false].forEach((warmup) => {
        it(`should create connections to remote nodes in the background when warmup is ${warmup}`, async () => {
          ...
          const client = newInstance(cluster, {
            policies: {
              loadBalancing: new CustomLoadBalancingPolicy(localAddresses, [
                remoteAddress,
              ]),
            },
            pooling: {
          ...
          const localHostsHosts = localAddresses.map((a) =>
            client.hosts.get(a),
          );

          // Two connections to local hosts
          localHostsHosts.forEach((h) =>
            assert.deepStrictEqual(client.getState().getOpenConnections(h), 2),
          );

          await helper.wait.until(
            () => client.getState().getOpenConnections(remoteHost) === 1,
          );
...
function assertArray(arr, value) {
  assert.deepStrictEqual(
    arr,
    new Array(arr.length).fill(null).map(() => value),
  );
}
...
function validateStateOfPool(client, cluster) {
  return client.execute("SELECT * FROM system.local").then((rs) => {
 ...
 */
class CustomTestAddressTranslator extends policies.addressResolution
  .AddressTranslator {
  constructor() {
    super();
 ...
 */
class CustomLoadBalancingPolicy extends policies.loadBalancing
  .LoadBalancingPolicy {
  ...
  *_queryPlan() {
    yield* utils.shuffleArray(
      this.localAddresses.map((a) => this.hosts.get(a)),
    );
    yield* utils.shuffleArray(
      this.remoteAddresses.map((a) => this.hosts.get(a)),
    );
  }
}
