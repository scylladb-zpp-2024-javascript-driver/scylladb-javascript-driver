    ...
    describe("#init()", function () {
        ...
        it("should subscribe to SCHEMA_CHANGE events and refresh keyspace information", async () => {
            ...
            await otherClient.execute(
                "CREATE KEYSPACE sample_change_1 " +
            ...
            await otherClient.execute(
                "ALTER KEYSPACE sample_change_1 WITH replication = " +
                    "{'class': 'SimpleStrategy', 'replication_factor' : 2}",
            );

            await helper.setIntervalUntilPromise(
                () =>
                    cc.metadata.keyspaces["sample_change_1"].strategyOptions
        ...
        it("should subscribe to STATUS_CHANGE events", async () => {
            ...
            class TestLoadBalancing extends policies.loadBalancing
                .RoundRobinPolicy {
                getDistance(h) {
            ...
            const cc = newInstance({
                policies: { loadBalancing: new TestLoadBalancing() },
            });
            disposeAfter(cc);
            ...
            const countUp = hosts.reduce(
                (value, host) => value + (host.isUp() ? 1 : 0),
                0,
            );
        ...
        it("should subscribe to TOPOLOGY_CHANGE add events and refresh ring info", async () => {
            ...
            options.policies.reconnection =
                new policies.reconnection.ConstantReconnectionPolicy(1000);
            ...
            await cc.init();
            await new Promise((r) =>
                options.policies.loadBalancing.init(null, cc.hosts, r),
            );

            await util.promisify(helper.ccmHelper.bootstrapNode)({
                nodeIndex: 3,
                dc: "dc1",
            });
            ...
            const countUp = cc.hosts
                .values()
                .reduce((value, host) => value + (host.isUp() ? 1 : 0), 0);
        ...
        /* it("should subscribe to TOPOLOGY_CHANGE remove events and refresh ring info", async () => {
            ...
            await cc.init();
            await new Promise((r) =>
                cc.options.policies.loadBalancing.init(null, cc.hosts, r),
            );
        ...
        it("should reconnect when host used goes down", async () => {
            const options = clientOptions.extend(
                utils.extend(
                    { pooling: helper.getPoolingOptions(1, 1, 500) },
                    helper.baseOptions,
                ),
            );
            ...
            await new Promise((r) =>
                lbp.init(
                    { log: utils.noop, options: { localDataCenter: "dc1" } },
                    cc.hosts,
                    r,
                ),
            );
...
function newInstance(options, localConnections, remoteConnections) {
    ...
    options.pooling.coreConnectionsPerHost[types.distance.local] =
        localConnections || 2;
    options.pooling.coreConnectionsPerHost[types.distance.remote] =
        remoteConnections || 1;
