...
describe("ModelMapper", function () {
  ...
  describe("#find()", () => {
    ...
    it("should use the correct table when no MappingOptions are specified", () => {
      ...
      return modelMapper.find(doc).then((result) => assert.ok(result.first()));
    });

    it("should use the another table", () => {
      const doc = { userId: mapperTestHelper.userIds[0] };
      return videoMapper
        .find(doc, { fields: ["id", "userId", "name"] }, "default")
        .then((result) => {
          assert.ok(result.first());
        });
    });
    ...
    it("should use the provided fields and order by when defined", () => {
      ...
      return videoMapper
        .find(
          doc,
          { fields: ["id", "yyyymmdd", "name"], orderBy: { addedDate: "asc" } },
          "default",
        )
        .then((result) => {
    ...
    it("should use select the correct table based on order by columns", () => {
      ...
      return Promise.all(
        items.map((item) =>
          clusteringMapper
            .find(doc, { orderBy: item.orderBy }, "default")
            .then((result) => {
              const expectedValues =
                utils.objectValues(item.orderBy)[0] === "asc"
                  ? [
                      `value_abc_table${item.table}`,
                      `value_azz_table${item.table}`,
                    ]
                  : [
                      `value_azz_table${item.table}`,
                      `value_abc_table${item.table}`,
    ...
    it("should support manual paging", () => {
      ...
      return Promise.all(
        videoIds.map((id, i) =>
          mapperTestHelper.insertVideoRows(client, {
            id,
            userId,
            addedDate: new Date(),
            name: `video${i}`,
        ...
        .then(() =>
          videoMapper.find({ userId }, null, { fetchSize: 3, pageState }),
        )
        .then((result) => {
          ...
          videosResult
            .map((v) => v.name)
            .sort()
            .forEach((name, i) => assert.strictEqual(name, `video${i}`));
          videosResult.forEach((v) =>
            assert.ok(
              videoIds.reduce(
                (acc, id) => acc + (v.id.toString() === id.toString() ? 1 : 0),
                0,
              ) === 1,
            ),
          );
    ...
    vit("3.0", "should support query operators", () => {
      ...
      return Promise.all(
        testItems.map((item, index) => {
          const doc = {
            yyyymmdd: mapperTestHelper.yyyymmddBuckets[0],
            addedDate: item.op(item.value),
          };
          return videoMapper.find(doc, "default").then((result) => {
            assert.strictEqual(
  ...
  describe("#findAll()", () => {
    it("should query without filter", () =>
      videoMapper.findAll().then((result) => {
    ...
    it("should use the correct table when no MappingOptions are specified", () => {
      ...
      return modelMapper
        .findAll()
        .then((result) => assert.ok(result.length > 0));
    });

    it("should support fields and limit", () =>
      videoMapper
        .findAll({ fields: ["id", "addedDate"], limit: 1 })
        .then((result) => {
          helper.assertInstanceOf(result, Result);
  ...
  describe("#get()", () => {
    ...
    it("should return null when not found", () =>
      videoMapper
        .get({ id: Uuid.random() })
        .then((resultDoc) => assert.strictEqual(resultDoc, null)));

    it("should throw an error when the table does not exist", () =>
      testTableNotFound(mapper, "get"));
  ...
  describe("#insert()", () => {
    it("should insert on all tables where the partition and clustering keys are specified", () => {
      const doc = {
        ...
        userId: Uuid.random(),
        addedDate: new Date(),
        name: "Video insert sample 1",
        description: "description insert 1",
        yyyymmdd: new Date().toISOString().substr(0, 10),
        tags: ["a", "b"],
        location: "a/b/c",
        locationType: 1,
        preview: "a/preview/c",
    ...
    it("should insert on some of the tables when those keys are not specified", () => {
      const doc = {
        ...
        userId: Uuid.random(),
        addedDate: new Date(),
        name: "Video insert 2 not in latest",
    ...
    it("should support inserting in parallel", () => {
      ...
      return Promise.all(
        ...
        .then((rs) => {
          ...
          rs.rows.forEach((row) =>
            assert.strictEqual(row["email"], `${row["userid"]}@example.com`),
          );
        });
    });
    ...
    it("should consider fields filter", () => {
      const doc = {
        ...
        userId: Uuid.random(),
        addedDate: new Date(),
        name: "Video insert sample 4",
        description: "description insert 4",
        yyyymmdd: new Date().toISOString().substr(0, 10),
        locationType: 1,
        preview: "a/preview/c",
      };
      ...
      return videoMapper
        .insert(
          doc,
          { fields: ["id", "userId", "addedDate", "name"] },
          "default",
        )
        .then(() => mapperTestHelper.getVideoRows(client, doc))
        .then((rows) => {
          ...
          rows
            .slice(0, 2)
            .forEach((row) => assertRowMatchesDoc(row, expectedDoc));
        });
    });
    ...
    it("should support conditional statements on a single table", () => {
      ...
      const doc = {
        id: Uuid.random(),
        name: "Conditional inserted",
        description: "description inserted",
      };

      return videoMapper
        .insert(doc, { ifNotExists: true })
        .then(() =>
          mapperTestHelper.getVideoRows(
            client,
            doc,
            "videoid, name, description",
          ),
        )
        .then((rows) => {
    ...
    it("should fail when conditional statements affect multiple tables", () => {
      const doc = {
        ...
        userId: Uuid.random(),
        addedDate: new Date(),
        name: "Video insert conditional",
        description: "description insert 5",
        yyyymmdd: new Date().toISOString().substr(0, 10),
        locationType: 1,
        preview: "a/preview/c",
      };
      ...
      return videoMapper
        .insert(doc, { ifNotExists: true }, "default")
        .catch((err) => (error = err))
        .then(() => {
    ...
    it("should support inserting static column value without clustering keys", () => {
      ...
      return staticMapper
        ...
        .then(() =>
          client.execute(
            "SELECT id1, s FROM table_static1 WHERE id1 = ?",
            [doc.id1],
            { prepare: true },
          ),
        )
    ...
    it("should support inserting static column value with clustering keys", () => {
      ...
      return staticMapper
        ...
        .then(() =>
          client.execute(
            "SELECT id1, id2, s FROM table_static1 WHERE id1 = ?",
            [doc.id1],
            { prepare: true },
          ),
        )
    ...
    it("should support inserting multiple static column values without clustering keys", () => {
      ...
      return static2Mapper
        .insert(doc)
        .then(() =>
          client.execute(
            "SELECT id1, s0, s1 FROM table_static2 WHERE id1 = ?",
            [doc.id1],
            { prepare: true },
          ),
        )
  ...
  describe("#update()", () => {
    it("should update on all tables where the partition and clustering keys are specified", () => {
      ...
      return mapperTestHelper
        .insertVideoRows(client, doc)
    ...
    it("should update a single table when it only matches one table", () => {
      ...
      return mapperTestHelper
        .insertVideoRows(client, doc)
        .then(() =>
          videoMapper.update({ id: doc.id, name: "updated single 1" }),
        )
        .then(() => mapperTestHelper.getVideoRows(client, doc))
        .then((rows) => {
          assert.strictEqual(rows.length, 3);
    ...
    it("should consider fields filter", () => {
      ...
      return videoMapper
        .update(doc, { fields: ["id", "userId", "addedDate", "name"] })
        .then(() => mapperTestHelper.getVideoRows(client, doc))
        .then((rows) => {
          assert.strictEqual(rows.length, 2);
          rows.forEach((row) => {
    ...
    it("should support conditional statements on a single table", () => {
      ...
      return mapperTestHelper
        .insertVideoRows(client, doc)
        .then(() =>
          videoMapper.update(docUpdated, { when: { name: "video cond 1" } }),
        )
        .then((result) => {
          helper.assertInstanceOf(result, Result);
    ...
    it("should fail when conditional statements affect multiple tables", () => {
      ...
      return mapperTestHelper
        .insertVideoRows(client, doc)
        .then(() =>
          videoMapper.update(docUpdated, { when: { name: "video cond 2" } }),
        )
        .catch((err) => (error = err))
        .then(() => {
          helper.assertInstanceOf(error, Error);
    ...
    it("should adapt results of a LWT operation", () => {
      ...
      return client
        .execute(
          insertQuery,
          [doc.id, doc.firstName, doc.lastName, doc.email],
          { prepare: true },
        )
        ...
        .then(() =>
          userMapper.update(doc, { when: { firstName: "a", lastName: "b" } }),
        )
        .then((result) => {
          assertNotApplied(result);
          const lwtDoc = result.first();
        ...
        .then(() =>
          userMapper.update(doc, {
            when: { firstName: "a", lastName: q.notEq(doc.lastName) },
          }),
        )
        .then((result) => {
          assertNotApplied(result);
    ...
    it("should add new items to a set", () => {
      ...
      return client
        .execute(
          "INSERT INTO videos (videoid, name, tags) VALUES (?, ?, ?)",
          [doc.id, doc.name, doc.tags],
          { prepare: true },
        )
        .then(() =>
          videoMapper.update({ id: doc.id, tags: q.append(["d", "e"]) }),
        )
    ...
    it("should support setting the ttl and CAS in the same statement", () => {
      const doc = {
        id: Uuid.random(),
        name: "Video w/ ttl and CAS",
        description: "Updated video with ttl and CAS",
      };

      return videoMapper
        .update(doc, { ttl: 360, when: { name: q.notEq("video1") } })
        .then(() => mapperTestHelper.getVideoRows(client, doc))
        .then((rows) => {
  ...
  describe("#remove()", () => {
    ...
    it("should use the correct table when no MappingOptions are specified", () => {
      ...
      const doc = {
        id: Uuid.random(),
        name: "to delete w/ no mapping",
        description: "desc 4",
      };

      return mapperTestHelper
        .insertVideoRows(client, doc)
        .then(() => modelMapper.remove({ videoid: doc.id }))
        .then(() => mapperTestHelper.getVideoRows(client, doc))
        .then((rows) => {
          assert.strictEqual(rows.length, 1);
          assert.strictEqual(rows[0], null);
    ...
    it("should fail when conditional statements affect multiple tables", () => {
      ...
      return mapperTestHelper
        .insertVideoRows(client, doc)
        .then(() =>
          videoMapper.remove(doc, { when: { name: "video to delete" } }),
        )
        .catch((err) => (error = err))
        .then(() => {
          helper.assertInstanceOf(error, Error);
...
function testTableNotFound(mapper, methodName) {
  ...
  return modelMapper[methodName]({ id: 1 })
    .catch((err) => {
      ...
      assert.strictEqual(
        err.message,
        'Table "TableDoesNotExist" could not be retrieved',
      );
    })
    .then(() => assert.strictEqual(catchCalled, true));
}
