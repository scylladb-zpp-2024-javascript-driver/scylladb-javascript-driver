...
vdescribe("dse-5.0", "Polygon @SERVER_API", function () {
  ...
  [0, 1].forEach(function (prepare) {
    ...
    it(
      util.format("should encode polygons for %s queries", name),
      function (done) {
        const client = new Client(helper.getOptions());
        utils.series(
          [
            ...
            function test(next) {
              ...
              const insertQuery =
                "INSERT INTO ks1.polygons (id, value) VALUES (?, ?)";
              const selectQuery =
                "SELECT toJSON(value) as json_value FROM ks1.polygons WHERE id = ?";
              let counter = 0;
              utils.each(
                values,
                function (polygon, eachNext) {
    ...
    it(
      util.format(
        "should be able to retrieve data where polygon is partition key for %s queries",
        name,
      ),
      function (done) {
        const client = new Client(helper.getOptions());
        const id = new Polygon([
          new Point(1, 3),
          new Point(3, 1),
          new Point(3, 6),
          new Point(1, 3),
  ...
  describe("with collections, tuples and udts", function () {
    ...
    [0, 1].forEach(function (prepare) {
      ...
      it(
        util.format(
          "should create and retrieve polygons in a udt for %s queries",
          name,
        ),
        function (done) {
          const client = new Client(helper.getOptions());
            ...
            [
              ...
              function (next) {
                client.execute(
                  insertQuery,
                  [id, udt],
                  { prepare: prepare, hints: [null, "udt<ks1.polygont>"] },
                  function (err) {
                    assert.ifError(err);
                    client.execute(
                      selectQuery,
      ...
      tupleTestCase(
        util.format(
          "should create and retrieve polygons in a tuple for %s queries",
          name,
        ),
        function (done) {
          const client = new Client(helper.getOptions());
      ...
      ["list", "set"].forEach(function (colType) {
        it(
          util.format(
            "should create and retrieve polygons in a %s for %s queries",
            colType,
            name,
          ),
          function (done) {
      ...
      it(
        util.format(
          "should create and retrieve polygons in a map for %s queries",
          name,
        ),
        function (done) {
          const client = new Client(helper.getOptions());
