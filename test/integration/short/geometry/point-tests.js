...
vdescribe("dse-5.0", "Point @SERVER_API", function () {
    ...
    [0, 1].forEach(function (prepare) {
        ...
        it(
            util.format("should encode points for %s queries", name),
            function (done) {
                ...
                utils.series(
                    [
                        ...
                        function test(next) {
                            ...
                            const insertQuery =
                                "INSERT INTO ks1.points (id, value) VALUES (?, ?)";
                            const selectQuery =
                                "SELECT toJSON(value) as json_value FROM ks1.points WHERE id = ?";
                            let counter = 0;
                            utils.each(
                                values,
                                function (p, eachNext) {
        ...
        it(
            util.format(
                "should be able to retrieve data where point is partition key for %s queries",
                name,
            ),
            function (done) {
                const client = new Client(helper.getOptions());
    ...
    describe("with collections, tuples and udts", function () {
        ...
        [0, 1].forEach(function (prepare) {
            ...
            it(
                util.format(
                    "should create and retrieve points in a udt for %s queries",
                    name,
                ),
                function (done) {
                    const client = new Client(helper.getOptions());
            ...
            tupleTestCase(
                util.format(
                    "should create and retrieve points in a tuple for %s queries",
                    name,
                ),
                function (done) {
                    const client = new Client(helper.getOptions());
            ...
            ["list", "set"].forEach(function (colType) {
                it(
                    util.format(
                        "should create and retrieve points in a %s for %s queries",
                        colType,
                        name,
                    ),
                    function (done) {
            ...
            it(
                util.format(
                    "should create and retrieve points in a map for %s queries",
                    name,
                ),
                function (done) {
                    const client = new Client(helper.getOptions());
