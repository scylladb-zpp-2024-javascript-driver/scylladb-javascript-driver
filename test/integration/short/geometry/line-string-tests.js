...
vdescribe("dse-5.0", "LineString @SERVER_API", function () {
    ...
    [0, 1].forEach(function (prepare) {
        ...
        it(
            util.format("should encode lines for %s queries", name),
            function (done) {
                const client = new Client(helper.getOptions());
                utils.series(
                    [
                        ...
                        function test(next) {
                            const values = [
                                ...
                                new LineString(
                                    new Point(0.21222, 32.9),
                                    new Point(10.21222, 312.9111),
                                    new Point(4.21222, 6122.9),
                                ),
                            ];
                            const insertQuery =
                                "INSERT INTO ks1.lines (id, value) VALUES (?, ?)";
                            const selectQuery =
                                "SELECT toJSON(value) as json_value FROM ks1.lines WHERE id = ?";
                            let counter = 0;
                            utils.each(
                                values,
                                function (line, eachNext) {
                                    const id = util.format(
                                        "%s-%d",
                                        name,
                                        ++counter,
                                    );
        ...
        it(
            util.format(
                "should be able to retrieve data where line is partition key for %s queries",
                name,
            ),
            function (done) {
                const client = new Client(helper.getOptions());
                ...
                utils.series(
                    [
                        ...
                        function (next) {
                            const selectQuery =
                                "SELECT value FROM ks1.keyed WHERE id = ?";
                            client.execute(
                                selectQuery,
                                [id],
                                function (err, result) {
    ...
    describe("with collections, tuples and udts", function () {
        ...
        [0, 1].forEach(function (prepare) {
            ...
            it(
                util.format(
                    "should create and retrieve lines in a udt for %s queries",
                    name,
                ),
                function (done) {
                    const client = new Client(helper.getOptions());
                    const insertQuery =
                        "INSERT INTO ks1.tbl_udts (id, udt_col) values (?, ?)";
                    const selectQuery =
                        "SELECT udt_col FROM ks1.tbl_udts WHERE id = ?";
            ...
            tupleTestCase(
                util.format(
                    "should create and retrieve lines in a tuple for %s queries",
                    name,
                ),
                function (done) {
                    const client = new Client(helper.getOptions());
                    const insertQuery =
                        "INSERT INTO ks1.tbl_tuple (id, tuple_col) values (?, ?)";
                    const selectQuery =
                        "SELECT tuple_col FROM ks1.tbl_tuple WHERE id = ?";
                    const id = Uuid.random();
            ...
            ["list", "set"].forEach(function (colType) {
                it(
                    util.format(
                        "should create and retrieve lines in a %s for %s queries",
                        colType,
                        name,
                    ),
                    function (done) {
                        ...
                        const insertQuery = util.format(
                            "INSERT INTO ks1.tbl_%s (id, %s_col) values (?, ?)",
                            colType,
                            colType,
                        );
                        const selectQuery = util.format(
                            "SELECT %s_col FROM ks1.tbl_%s WHERE id = ?",
                            colType,
                            colType,
                        );
                        const id = Uuid.random();
            ...
            it(
                util.format(
                    "should create and retrieve lines in a map for %s queries",
                    name,
                ),
                function (done) {
                    const client = new Client(helper.getOptions());
                    const insertQuery =
                        "INSERT INTO ks1.tbl_map (id, map_col) values (?, ?)";
                    const selectQuery =
                        "SELECT map_col FROM ks1.tbl_map WHERE id = ?";
                    const id = Uuid.random();
                    const map = { line: line };
                        ...
                        [
                            ...
                            function (next) {
                                client.execute(
                                    ...
                                    {
                                        prepare: prepare,
                                        hints: [null, types.dataTypes.map],
                                    },
                                    function (err) {
                                        assert.ifError(err);
                                        client.execute(
