      ...
      utils.series(
        [
          ...
          function selectData(seriesNext) {
            client.execute(
              util.format("SELECT id, val, varintAsBlob(val) FROM %s", table),
              [],
              { prepare: 1 },
              function (err, result) {
                assert.ifError(err);
          ...
          function insertData(seriesNext) {
            ...
            utils.timesLimit(
              ...
              function (n, next) {
                ...
                let value =
                  (n * 999).toString() + "." + (100 + n * 7).toString();
                if (n % 10 === 0) {
                  value = "-" + value;
                }
                if (n % 2 === 0) {
    ...
    it("should encode and decode maps using Map polyfills", function (done) {
      ...
      const values = [
        [
          ...
          new MapPF([
            [types.timeuuid(), types.BigDecimal.fromString("1.20008")],
            [types.timeuuid(), types.BigDecimal.fromString("-9.26")],
          ]),
        ],
      ];
          ...
          function selectData(seriesNext) {
            ...
            client.execute(
              ...
              function (err, result) {
                ...
                result.rows.forEach(function (row) {
                  const expectedValues = helper.first(values, (item) =>
                    item[0].equals(row.id),
                  );
      ...
      utils.series(
        [
          ...
          function insert2(next) {
            ...
            client2.execute(
              query,
              [id, types.TimeUuid.now(), "test null or unset", undefined],
              { prepare: true },
              next,
            );
          },
    ...
    describe("with a different keyspace", function () {
      it("should fill in the keyspace in the query options passed to the lbp", () => {
        ...
        return client
          ...
          .then(() => {
            const options =
              executionOptionsArray[executionOptionsArray.length - 1];
        ...
        utils.series(
          [
            ...
            function insert2(next) {
              client.execute(
                insertQuery,
                [Uuid.random(), new types.Tuple("unset pair", undefined), null],
                options,
                next,
              );
            },
            function insert3(next) {
              client.execute(
                insertQuery,
                [Uuid.random(), new types.Tuple("null pair", null), null],
                options,
                next,
              );
            },
        ...
        () => {
          ...
          return client
            ...
            .then((rs) => {
              ...
              assert.deepStrictEqual(tuple2Result.values().slice(1), [
                undefined,
                undefined,
              ]);
            });
        ...
        function (done) {
          const values = [
            ...
            [
              ...
              new LocalTime(types.Long.fromString("86399999999999")),
            ],
            [
              Uuid.random(),
              new LocalDate(-2147483648),
              new LocalTime(types.Long.fromString("6311999549933")),
            ],
          ];
          const client = setupInfo.client;
          utils.eachSeries(
        ...
        utils.series(
          [
            ...
            function insert1(next) {
              ...
              client1.execute(
                query,
                [id1, types.TimeUuid.now(), "test unset", types.unset],
                { prepare: true },
                next,
              );
            },
            ...
            function insert2(next) {
              ...
              client2.execute(
                query,
                [id2, types.TimeUuid.now(), "test unset 2", undefined],
                { prepare: true },
                next,
              );
            },
        ...
        utils.series(
          [
            helper.toTask(
              client.execute,
              client,
              util.format("CREATE TABLE %s (k int PRIMARY KEY, v int)", table),
            ),
            helper.toTask(
              client.execute,
              client,
              util.format("CREATE INDEX simple_index ON %s (v)", table),
            ),
            function insertData(seriesNext) {
              const query = util.format(
          ...
          utils.series(
            [
              ...
              helper.toTask(
                client.execute,
                client,
                util.format("CREATE INDEX frozen_index ON %s (full(v))", table),
              ),
              function insertData(seriesNext) {
                const query = util.format(
                  "INSERT INTO %s (k, v) VALUES (?, ?)",
          ...
          utils.series(
            [
              ...
              helper.toTask(
                client.execute,
                client,
                util.format("CREATE INDEX keys_index on %s (keys(v))", table),
              ),
              function insertData(seriesNext) {
                const query = util.format(
                  "INSERT INTO %s (k, v) VALUES (?, ?)",
          ...
          utils.series(
            [
              ...
              helper.toTask(
                client.execute,
                client,
                util.format("CREATE INDEX values_index on %s (v)", table),
              ),
              function insertData(seriesNext) {
                const query = util.format(
                  "INSERT INTO %s (k, v) VALUES (?, ?)",
          ...
          [
            ...
            (next) => {
              client2.execute(query, null, { prepare: true }, (err, result) => {
                assert.ok(result);
                if (compareMetadata) {
                  const info = client2.metadata.getPreparedInfo(
                    commonKs,
                    query,
                  );
