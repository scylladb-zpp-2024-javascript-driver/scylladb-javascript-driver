            ...
            describe("with { prepare: " + prepare + " }", function () {
                it("should serialize and deserialize when used as parameter", function (done) {
                    utils.eachSeries(
                        ...
                        function (v, next) {
                            const query =
                                "INSERT INTO tbl_date_range (pk, c1) VALUES (?, ?)";
                            ...
                            client.execute(
                                ...
                                function (err) {
                                    assert.ifError(err);
                                    const query =
                                        "SELECT pk, c1 FROM tbl_date_range WHERE pk = ?";
                                    client.execute(
                                        query,
                                        [id],
                                        { prepare: prepare },
                ...
                it("should be allowed in udt and tuple", function (done) {
                    ...
                    const u = {
                        i: 0,
                        range: DateRange.fromString("[2017-02 TO 2017-03]"),
                    };
                    const uf = {
                        i: 4,
                        range: DateRange.fromString("[2016-02-04 TO 2016-05]"),
                    };
                    const t = new types.Tuple(DateRange.fromString("*"), 3);
                    const tf = new types.Tuple(
                        DateRange.fromString("[* TO *]"),
                        9,
                    );
                    const query =
                        "INSERT INTO tbl_udt_tuple (k, u, uf, t, tf) VALUES (?, ?, ?, ?, ?)";
                    ...
                    client.execute(
                        ...
                        function (err) {
                            assert.ifError(err);
                            const query =
                                "SELECT * FROM tbl_udt_tuple WHERE k = ?";
                            client.execute(
                                query,
                                [id],
                                { prepare: prepare },
                ...
                it("should be allowed in collections", function (done) {
                    ...
                    const clientOptions = utils.deepExtend(
                        {
                    ...
                    const query =
                        "INSERT INTO tbl_collection (k, l, s, m0, m1) VALUES (?, ?, ?, ?, ?)";
                    utils.series(
                        [
                            ...
                            function (next) {
                                client.execute(
                                    ...
                                    function (err) {
                                        assert.ifError(err);
                                        const query =
                                            "SELECT * FROM tbl_collection WHERE k = ?";
                                        client.execute(
                                            query,
                                            [id],
                                            { prepare: prepare },
                ...
                it("should use DateRange as primary key", function (done) {
                    ...
                    const query =
                        "INSERT INTO tbl_date_range_pk (k, v) VALUES (?, ?)";
                    utils.eachSeries(
                        key_values,
                        function (v, next) {
                            const value = DateRange.fromString(v);
                            ...
                            client.execute(
                                ...
                                function (err) {
                                    assert.ifError(err);
                                    const searchQuery =
                                        "SELECT k FROM tbl_date_range_pk WHERE k = ?";
                                    client.execute(
                                        searchQuery,
                                        [value],
                                        { prepare: prepare },
                                        function (err, result) {
                                            ...
                                            assert.ok(actual.equals(value));
                                            client.execute(
                                                "DELETE FROM tbl_date_range_pk where k = ?",
                                                [value],
                                                { prepare: prepare },
                                                function (err) {
                                                    assert.ifError(err);
                ...
                it("should disallow invalid order", function (done) {
                    const query =
                        "INSERT INTO tbl_date_range (pk, c1) VALUES (?, ?)";
                    const id = types.Uuid.random();
                    const value = DateRange.fromString(
                        "[2020-01-01T10:15:30.009Z TO 2010-01-01T00:05:11.031Z]",
                    );
                    client.execute(
                        query,
                        [id, value],
                        ...
                        function (err) {
                            if (!err) {
                                assert.ifError(
                                    "Should throw error when DataRange order is inverted",
                                );
                            }
                            done();
                        },
                        ...
                        function (v, next) {
                            ...
                            client.execute(
                                ...
                                function (err) {
                                    assert.ifError(err);
                                    const query =
                                        "SELECT JSON c1 FROM tbl_date_range WHERE pk = ?";
                                    client.execute(
                                        query,
                                        [id],
                                        { prepare: prepare },
                                        function (err, result) {
                                            ...
                                            const jsonRow =
                                                JSON.parse(jsonRowStr);
                                            const actual = DateRange.fromString(
                                                jsonRow.c1,
                                            );
                                            assert.ok(actual.equals(value));
                                            next();
