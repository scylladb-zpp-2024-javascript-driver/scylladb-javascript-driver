...
vdescribe("dse-6.7", "Cloud support", function () {
    ...
    context("with a 3 node cluster", () => {
        ...
        it("should resolve dns name of the proxy, connect and set defaults", async () => {
            ...
            assert.match(
                client.options.sslOptions.checkServerIdentity.toString(),
                /sni\.address/,
            );
            ...
            client.hosts.forEach((h) => {
                ...
                assert.strictEqual(
                    h.pool.connections[0].endpointFriendlyName,
        ...
        it("should use all the proxy resolved addresses", () => {
            ...
            const ControlConnection = proxyquire(
                `${libPath}/control-connection`,
                {
                    dns: {
                        resolve4: (name, cb) => {
            ...
            const Client = proxyquire(`${libPath}/client`, {
                "./control-connection": ControlConnection,
            });

            const client = new Client(
                cloudHelper.getOptions({
                    cloud: {
                        secureConnectBundle: "certs/bundles/creds-v1.zip",
                    },
                }),
            );
            ...
            return client.connect().then(() => {
                ...
                hosts.forEach((h, index) => {
                    ...
                    assert.strictEqual(
                        h.pool.connections[0].endpoint.split(":")[0],
                        `127.0.0.${(index % 2) + 1}`,
                    );
                });
        ...
        it("should match system.local information of each node", async () => {
            const client = cloudHelper.getClient({
                policies: new policies.loadBalancing.RoundRobinPolicy(),
            });
            ...
            for (let i = 0; i < 3; i++) {
                ...
                assert.ok(host);
                assert.strictEqual(
                    row["host_id"].toString(),
                    host.hostId.toString(),
                );
                assert.strictEqual(
                    host.address,
                    `${row["rpc_address"]}:${port}`,
                );
            }
        ...
        it("should support leaving the auth unset", async () => {
            const client = cloudHelper.getClient({
                cloud: {
                    secureConnectBundle: "certs/bundles/creds-v1-wo-creds.zip",
                },
            });
        ...
        it("should support overriding the auth provider", async () => {
            const authProvider = new auth.DsePlainTextAuthProvider(
                "user1",
                "12345678",
            );
            ...
            try {
                ...
                assert.instanceOf(
                    utils.objectValues(err.innerErrors)[0],
        ...
        it("should callback in error when bundle file does not exist", async () => {
            const client = cloudHelper.getClient({
                cloud: {
                    secureConnectBundle: "certs/bundles/does-not-exist.zip",
                },
            });
            let error;
        ...
        it("should provide token-aware load balancing by default", async () => {
            ...
            for (const item of replicasByKey) {
                const query =
                    "INSERT INTO ks_network_rf1.table1 (id, name) VALUES (?, ?)";
                ...
                assert.strictEqual(
                    helper.lastOctetOf(rs.info.queriedHost),
                    replica,
                );
            }
        });
        ...
        context("with nodes going down", () => {
            ...
            it("should reconnect to the same host and refresh dns resolution", async () => {
                ...
                assert.strictEqual(
                    client.hosts.values().filter((h) => h.isUp()).length,
                    2,
                );
    ...
    context("without a metadata service", () => {
        it("should throw a NoHostAvailableError when there address is unreachable", () => {
            ...
            const client = cloudHelper.getClient({
                cloud: {
                    secureConnectBundle:
                        "certs/bundles/creds-v1-unreachable.zip",
                },
            });
            ...
            return client
                ...
                .then(() => {
                    ...
                    assert.deepStrictEqual(Object.keys(error.innerErrors), [
                        "192.0.2.255:30443/metadata",
                    ]);
                });
        });
