...
const DefaultRetryPolicy =
    require("../../../../lib/policies/retry").RetryPolicy;
...
vdescribe("dse-5.0", "graph query client timeouts", function () {
    ...
    function profiles() {
        return [
            ...
            new ExecutionProfile("withRetryPolicy", {
                readTimeout: 114,
    ...
    describe("when graphOptions.readTimeout is set", function () {
        it(
            "should be used instead of socketOptions.readTimeout",
    ...
    describe("when profile.readTimeout is set", function () {
        it(
            ...
            getTimeoutTest(
                123,
                { executionProfile: "123Profile" },
                { profiles: profiles() },
            ),
        );
        it(
            "should be used instead of graphOptions.readTimeout",
            getTimeoutTest(
                123,
                { executionProfile: "123Profile" },
                { profiles: profiles(), graphOptions: { readTimeout: 20 } },
            ),
        );
        it(
            "should be used on default profile when no profile specified",
    ...
    describe("when queryOptions.readTimeout is set", function () {
        ...
        it(
            "should be used instead of profile",
            getTimeoutTest(
                92,
                { readTimeout: 92, executionProfile: "123Profile" },
                { profiles: profiles() },
            ),
        );
        it(
            "should be used instead of default profile",
            getTimeoutTest(93, { readTimeout: 93 }, { profiles: profiles() }),
        );
        it(
            "should be used instead of graphOptions.readTimeout",
            getTimeoutTest(
    ...
    describe("when readTimeout not set on profile or graphOptions", function () {
        it("should not encounter a client timeout and instead depend on server timeout", function (done) {
            ...
            const client = newInstance({
                ...
                graphOptions: { name: "name1", source: "1sectimeout" },
            });
            utils.series(
                [
                    client.connect.bind(client),
                    function setTimeoutOnSource(next) {
                        client.executeGraph(
                            'graph.schema().config().option("graph.traversal_sources.1sectimeout.evaluation_timeout").set("1002 ms")',
                            next,
                        );
                    },
                    function executeQuery(next) {
                        client.executeGraph(
                            "java.util.concurrent.TimeUnit.MILLISECONDS.sleep(10000L);",
                            function (err) {
                                assert.ok(err);
                                ...
                                const match =
                                    err.message.match(serverTimeoutErrRE);
                                assert.ok(match);
                                assert.ok(match[1]);
                ...
                [
                    ...
                    function executeQuery(next) {
                        client.executeGraph(
                            ...
                            null,
                            {
                                executionProfile: "withRetryPolicy",
                                isIdempotent: true,
                            },
                            function (err) {
                                assert.ok(err);
...
function getTimeoutTest(expectedTimeoutMillis, queryOptions, clientOptions) {
    const operationTimeoutRE =
        /.*The host .* did not reply before timeout (\d+) ms.*/;
                ...
                function executeQuery(next) {
                    client.executeGraph(
                        ...
                        function (err, result) {
                            ...
                            assert.ok(
                                !(err instanceof errors.NoHostAvailableError),
                                "Error should be rethrown by the retry policy",
                            );
                            const match = err.message.match(operationTimeoutRE);
                            assert.ok(
                                match && match[1],
                                `Message does not match: "${err.message}"`,
                            );
                            assert.strictEqual(
                                parseFloat(match[1]),
