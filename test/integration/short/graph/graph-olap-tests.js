...
vdescribe("dse-5.0", "Client with spark workload", function () {
    ...
    before((done) =>
        helper.ccm.startAll(1, { workloads: ["graph", "spark"] }, done),
    );
    ...
    before((done) =>
        helper.ccm.bootstrapNode({ nodeIndex: 2, dc: "dc1" }, done),
    );
    ...
    describe("#executeGraph()", function () {
        function executeAnalyticsQueries(
            queryOptions,
            options,
            shouldQueryMasterOnly,
        ) {
                            ...
                            client.executeGraph(
                                ...
                                function (err, result) {
                                    ...
                                    if (shouldQueryMasterOnly) {
                                        ...
                                        const portSep =
                                            queriedHost.lastIndexOf(":");
                                        queriedHost =
                                            portSep !== -1
                                                ? queriedHost.substr(0, portSep)
                                                : queriedHost;
                                        assert.strictEqual(
                                            queriedHost,
                                            sparkMaster,
        ...
        it(
            "should make an OLAP query using 'a' traversal source",
            executeAnalyticsQueries({ graphSource: "a" }),
        );
        it(
            "should make an OLAP query using profile with 'a' traversal source",
            executeAnalyticsQueries(
                { executionProfile: "analytics" },
                {
                    profiles: [
                        new ExecutionProfile("analytics", {
                            graphOptions: { source: "a" },
                        }),
                    ],
                },
            ),
        ...
        it(
            "should make an OLAP query with default profile using 'a' traversal source",
            executeAnalyticsQueries(
                {},
                {
                    profiles: [
        ...
        it(
            "should contact spark master directly to make an OLAP query when using DefaultLoadBalancingPolicy",
            executeAnalyticsQueries(
                { graphSource: "a" },
                {
                    policies: {
        ...
        it(
            "should contact spark master directly to make an OLAP query when using profile with DefaultLoadBalancingPolicy",
            executeAnalyticsQueries(
                { executionProfile: "analytics" },
                {
                    profiles: [
        ...
        context("with no callback specified", function () {
            it("should return a promise for OLAP query", function () {
                const client = newInstance();
                const p = client.executeGraph("g.V().count()", {
                    graphSource: "a",
                });
...
async function waitForWorkers(client, expectedWorkers) {
    ...
    for (
        let attempts = 1;
        numWorkers < expectedWorkers && attempts <= maxAttempts;
        attempts++
    ) {
        ...
        try {
            const body = await helper.makeWebRequest({
                host: master,
                port: 7080,
                path: "/",
            });
            ...
            helper.trace(
                "(%d/%d) Got error while fetching workers: %s",
                attempts,
                maxAttempts,
                err.message,
            );
    ...
    if (numWorkers < expectedWorkers) {
        helper.trace(
            "WARNING: After %d attempts only %d/%d workers were active.",
            maxAttempts,
            numWorkers,
            expectedWorkers,
        );
