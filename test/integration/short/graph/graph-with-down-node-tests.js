...
vdescribe("dse-5.1", "Client with down node", function () {
    ...
    describe("#executeGraph()", function () {
        const addVertexQuery =
            'graph.addVertex(label, "person", "name", "joe", "age", 42);';
        ...
        it(
            "should be able to make a read query with ONE read consistency, ALL write consistency",
            wrapClient(function (client, done) {
                client.executeGraph(
                    ...
                    {
                        readTimeout: 5000,
                        graphReadConsistency: cl.one,
                        graphWriteConsistency: cl.all,
                    },
                    function (err, result) {
                        assert.ifError(err);
        ...
        it(
            "should fail to make a read query with ALL read consistency",
            wrapClient(function (client, done) {
                client.executeGraph(
                    getVertexQuery,
                    null,
                    { graphReadConsistency: cl.all },
                    expectFailAtAll(done),
                );
            }),
        );

        it(
            "should be able to make a write query with ALL read consistency, TWO write consistency",
            wrapClient(function (client, done) {
                client.executeGraph(
                    addVertexQuery,
                    null,
        ...
        it(
            "should fail to make a write query with ALL write consistency",
            wrapClient(function (client, done) {
                client.executeGraph(
                    addVertexQuery,
                    null,
        ...
        it(
            "should use read consistency from profile",
            wrapClient(
                function (client, done) {
                    client.executeGraph(
                        getVertexQuery,
                        null,
                        { executionProfile: "readALL" },
                        expectFailAtAll(done),
                    );
                },
                {
                    profiles: [
                        new ExecutionProfile("readALL", {
                            graphOptions: { readConsistency: cl.all },
                        }),
                    ],
                },
            ),
        ...
        it(
            "should use write consistency from profile",
            wrapClient(
                function (client, done) {
                    client.executeGraph(
                        addVertexQuery,
                        null,
                        { executionProfile: "writeALL" },
                        expectFailAtAll(done),
                    );
                },
                {
                    profiles: [
                        new ExecutionProfile("writeALL", {
                            graphOptions: { writeConsistency: cl.all },
                        }),
                    ],
                },
            ),
        ...
        it(
            "should use read consistency from default profile",
            wrapClient(
                function (client, done) {
        ...
        it(
            "should use write consistency from default profile",
            wrapClient(
                function (client, done) {
