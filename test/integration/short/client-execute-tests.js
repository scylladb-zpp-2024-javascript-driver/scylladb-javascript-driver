...
describe("Client @SERVER_API", function () {
  ...
  describe("#execute(query, params, {prepare: 0}, callback)", function () {
    ...
    it("should support retrieving empty buffers as values", () => {
      ...
      return client
        .execute(insertQuery, [id].concat(columns.map(() => emptyBuffer)))
    ...
    vit("2.1", "should encode CONTAINS parameter", function (done) {
      const client = setupInfo.client;
      client.execute(
        util.format("CREATE INDEX list_sample_index ON %s(list_sample)", table),
        function (err) {
          assert.ifError(err);
        ...
        [
          ...
          function ensureQueryCLUsed(next) {
            client.execute(
              selectAllQuery,
              [],
              { executionProfile: "cl", consistency: types.consistencies.one },
              function (err, result) {
                assert.ifError(err);
                assert.strictEqual(
                  result.info.achievedConsistency,
    ...
    vdescribe("dse-5.1.6", "with noCompact", function () {
      before(function (done) {
        ...
        const client = newInstance({
          keyspace: keyspace,
          protocolOptions: { noCompact: true },
        });
        utils.series(
          [
            client.connect.bind(client),
            helper.toTask(
              client.execute,
              client,
              "CREATE TABLE tbl_cs (key blob PRIMARY KEY, bar int, baz uuid) WITH COMPACT STORAGE",
            ),
            helper.toTask(
              client.execute,
              client,
              "INSERT INTO tbl_cs (key, bar, baz, column1, value) values (0xc0, 10, 33cb65d4-6721-4ca8-854f-1f020c5353cb, 'yak', 0xcafedead)",
            ),
          ],
          done,
        );
      ...
      it("set to true should reveal non-schema columns", () => {
        ...
        return client.execute("select * from tbl_cs").then((result) => {
          ...
          assert.deepEqual(
            row.value,
            utils.allocBufferFromArray([0xca, 0xfe, 0xde, 0xad]),
          );
      ...
      it("set to false should not reveal non-schema columns", () => {
        const client = newInstance({
          keyspace: keyspace,
          protocolOptions: { noCompact: false },
        });
        return client.execute("select * from tbl_cs").then((result) => {
            ...
            [
              function insert(next) {
                ...
                client.execute(
                  query,
                  [id, phone, address],
                  { hints: [null, "udt<phone>", "udt<address>"] },
                  next,
                );
              },
              function select(next) {
                client.execute(
                  ...
                  function (err, result) {
                    ...
                    assert.strictEqual(
                      row["address_col"]["phones"][0]["alias"],
      ...
      vit("2.1", "should allow tuple parameter hints", function (done) {
        ...
        utils.series(
          [
            ...
            function select(next) {
              client.execute(
                util.format("SELECT * FROM tbl_tuples WHERE id = ?"),
                [id],
                function (err, result) {
                  assert.ifError(err);
                  const row = result.first();
    ...
    describe("with named parameters", function () {
      ...
      vit("2.1", "should use parameter hints", function (done) {
        ...
        client.execute(
          query,
          values,
          { hints: { myFloat: "float", myInt: { code: types.dataTypes.int } } },
          function (err) {
            assert.ifError(err);
            verifyRow(
              table,
        ...
        utils.series(
          [
            ...
            helper.toTask(
              client.execute,
              client,
              util.format(insertQuery, sampleId, 0x0200, 2, "'two'"),
            ),
          ],
          done,
        );
    ...
    vdescribe("2.0", "with lightweight transactions", function () {
      ...
      [
        ...
        context("when it " + item[0], function () {
          it(
            "should return a ResultSet with wasApplied set to " + item[1],
            function (done) {
              client.execute(
