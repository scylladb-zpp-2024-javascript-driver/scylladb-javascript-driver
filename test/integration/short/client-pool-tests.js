...
describe("Client", function () {
  ...
  describe("#connect()", function () {
    ...
    it("should resolve host names", function (done) {
      ...
      const client = new Client(
        utils.extend({}, helper.baseOptions, { contactPoints: ["localhost"] }),
      );
      client.connect(function (err) {
        assert.ifError(err);
        assert.strictEqual(client.hosts.length, 3);
        ...
        utils.times(
          ...
          function (n, next) {
            client.connect(function (err) {
              ...
              assert.ok(
                err.message.toLowerCase().indexOf("keyspace") >= 0,
    ...
    it("should override default pooling options when specified", function (done) {
      ...
      client.connect(function (err) {
        ...
        assert.ok(
          client.options.pooling.coreConnectionsPerHost[types.distance.remote],
  ...
  describe("#connect() with auth", function () {
    ...
    it("should return an AuthenticationError", function (done) {
      const options = {
        authProvider: new PlainTextAuthProvider("not___EXISTS", "not___EXISTS"),
        keyspace: "system",
      };
      const client = newInstance(options);
      utils.timesSeries(
    ...
    context("with credentials", () => {
      ...
      it("should fail with AuthenticationError when role does not exist", () => {
        const client = newInstance({
          credentials: { username: "incorrect_user", password: "abcd" },
        });
        let error;
      ...
      it("should fail with AuthenticationError when password is incorrect", () => {
        const client = newInstance({
          credentials: { username: "cassandra", password: "invalid_password" },
        });
        let error;
  ...
  describe("#execute()", function () {
    ...
    it("should change the active keyspace after USE statement", function (done) {
      ...
      const client = newInstance({
        pooling: { coreConnectionsPerHost: { [types.distance.local]: 10 } },
      });
                ...
                utils.map(
                  ...
                  function execute(prepare, mapNext) {
                    client.execute(
                      ...
                      null,
                      { prepare: prepare, isIdempotent: true },
                      mapNext,
                    );
                  },
            ...
            utils.timesLimit(
              ...
              function (n, timesNext) {
                client.execute(helper.queries.basic, function (err, result) {
                  if (!err) {
                    coordinators[helper.lastOctetOf(result.info.queriedHost)] =
                      true;
                  }
                  timesNext(err);
                });
              ...
              setTimeout(function checkPoolStateDelayed() {
                ...
                if (ignoredHost === "1") {
                  ...
                  assert.notStrictEqual(
                    helper.lastOctetOf(client.controlConnection.host),
                    "1",
                  );
                } else {
                  assert.strictEqual(
            ...
            () => {
              ...
              return (
                client.hosts
                  ...
                  .reduce(
                    (accumulator, h) =>
                      accumulator + state.getInFlightQueries(h),
                    0,
                  ) === 0
              );
            },
