...
vdescribe("dse-6.7", "InsightsClient", function () {
    ...
    it("should send startup message when a client is connected", () => {
        ...
        const client = new Client(
            helper.getOptions({ contactPoints: [firstAddress] }),
        );
        ...
        return client
            ...
            .then(() => {
                ...
                const paramsBase64 =
                    rpcLog.frame.message.options.positional_values;
                ...
                assert.strictEqual(
                    message.data.initialControlConnection,
                    firstAddress,
                );
                assert.ok(message.data.sessionId);
                assert.ok(message.data.clientId);
                assert.strictEqual(
                    message.data.applicationNameWasGenerated,
                    true,
                );
        ...
        return client
            ...
            .then(() => {
                ...
                return new Promise((r) =>
                    setTimeout(r, statusEventDelay + 100),
                );
            })
            .then(
                () =>
            ...
            .then((logs) => {
                const statusMessages = logs
                    .filter(
                        (l) =>
                            l.type === "QUERY" && l.query === insightsRpcQuery,
                    )
                    .map((l) =>
                        parseBase64Text(
                            l.frame.message.options.positional_values[0],
                        ),
                    )
