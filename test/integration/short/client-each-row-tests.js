          ...
          function insert(next) {
            ...
            utils.timesSeries(
              ...
              function (n, timesNext) {
                client.eachRow(
                  util.format(query, table, types.Uuid.random(), n),
                  [],
                  noop,
                  timesNext,
                );
              ...
              [
                function (parallelNext) {
                  ...
                  client.eachRow(
                    query,
                    [],
                    { fetchSize: 39, autoPage: true, prepare: true },
                    function (n, row) {
                ...
                function (parallelNext) {
                  ...
                  client.eachRow(
                    query,
                    [],
                    { fetchSize: 23, autoPage: true, prepare: true },
                    function (n, row) {
                      rowCount++;
        ...
        [
          ...
          function selectDataRemainingWithMetaPageState(seriesNext) {
            ...
            client.eachRow(
              util.format("SELECT * FROM %s", table),
              [],
              { prepare: 1, fetchSize: 70, pageState: rawPageState },
              function (n, row) {
                assert.ok(row);
                counter++;
              },
        ...
        [
          ...
          function selectDataRemaining(seriesNext) {
            ...
            client.eachRow(
              util.format("SELECT * FROM %s", table),
              [],
              { prepare: 1, fetchSize: 100, pageState: pageState },
              function (n, row) {
                assert.ok(row);
                counter++;
                assert.strictEqual(
