...
const defaultOptions =
    require("../../../lib/client-options.js").defaultOptions();
...
describe("Connection", function () {
    ...
    describe("#open()", function () {
        ...
        vit(
            "3.0",
            "should callback in error when protocol version is not supported server side",
            function (done) {
        ...
        vit(
            "2.0",
            "should limit the max protocol version based on the protocolOptions",
            function (done) {
                const options = utils.extend({}, defaultOptions);
                options.protocolOptions.maxVersion =
                    protocolVersion.getLowerSupported(getProtocolVersion());
        ...
        context("using timers", () => {
            ...
            it("should set the timeout for the heartbeat", function (done) {
                ...
                c.open(function (err) {
                    ...
                    assert.strictEqual(
                        c.sendStream.callCount,
                        initialSendCalls + 1,
                    );

                    const request = c.sendStream.getCall(
                        c.sendStream.callCount - 1,
                    ).args[0];
    ...
    describe("#open with ssl", function () {
        ...
        it("should open to a ssl enabled host", function (done) {
            ...
            localCon.open(function (err) {
                ...
                assert.ok(localCon.connected, "Must be status connected");
                localCon.sendStream(
                    getRequest(helper.queries.basic),
                    null,
                    function (err, result) {
                        assert.ifError(err);
                        assert.ok(result);
    ...
    describe("#changeKeyspace()", function () {
        ...
        it("should support multiple parallel calls", async () => {
            ...
            const promises = Array(10)
                .fill(0)
                .map(() => localCon.changeKeyspace(keyspace));
    ...
    describe("#sendStream()", function () {
        ...
        it("should queue pending if there is not an available stream id", function (done) {
            ...
            const maxRequests =
                connection.protocolVersion < 3 ? 128 : Math.pow(2, 15);
            utils.series(
                [
        ...
        context("promisify", () => {
            it("should return a promise", (done) => {
                ...
                connection.open((err) => {
                    ...
                    connection
                        .send(getRequest(helper.queries.basic), null)
                        .then((response) => {
...
function newInstance(address, protocolVersion, options) {
    ...
    const c = new Connection(
        address + ":" + options.protocolOptions.port,
        protocolVersion,
        options,
    );
