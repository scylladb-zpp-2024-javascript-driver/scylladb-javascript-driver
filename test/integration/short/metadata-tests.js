            ...
            (next) => {
              ...
              nonSyncClient.metadata.refreshKeyspaces((err) => {
                ...
                checkVirtualKeyspace(
                  nonSyncClient.metadata.keyspaces["system_virtual_schema"],
                );
                next();
              });
            },
        ...
        utils.series(
          [
            ...
            function getRanges(next) {
              ...
              const rf1Ranges =
                client.metadata.getTokenRangesForHost("ksrf1", host1).size +
                client.metadata.getTokenRangesForHost("ksrf1", host2).size;
              assert.strictEqual(rf1Ranges, 512);
              // expect 512 ranges for each host (2 replica = 512 tokens)
              assert.strictEqual(
                client.metadata.getTokenRangesForHost("ksrf2", host1).size,
                512,
              );
              assert.strictEqual(
                client.metadata.getTokenRangesForHost("ksrf2", host2).size,
                512,
              );
              assert.strictEqual(
                client.metadata.getTokenRangesForHost("ksntsrf2", host1).size,
                512,
              );
              assert.strictEqual(
                client.metadata.getTokenRangesForHost("ksntsrf2", host2).size,
                512,
              );
              next();
            },
          ...
          [
            ...
            function checkPhoneUdt(next) {
              ...
              m.getUdt("ks_udt1", "phone", function (err, udtInfo) {
                ...
                assert.ok(
                  udtInfo.fields[0].type.code === types.dataTypes.varchar ||
                    udtInfo.fields[0].type.code === types.dataTypes.text,
                );
                assert.strictEqual(udtInfo.fields[1].name, "number");
                assert.ok(
                  udtInfo.fields[1].type.code === types.dataTypes.varchar ||
                    udtInfo.fields[1].type.code === types.dataTypes.text,
                );
            ...
            function checkAddressUdt(next) {
              ...
              m.getUdt("ks_udt1", "address", function (err, udtInfo) {
                ...
                assert.strictEqual(
                  udtInfo.fields[2].type.info.info.fields.length,
                  4,
                );
                assert.strictEqual(
                  udtInfo.fields[2].type.info.info.fields[0].name,
                  "alias",
                );
                next();
              });
    ...
    describe("#getTable()", function () {
        ...
        setupInfo.client.metadata
          ...
          .then((table) => {
            ...
            table.columns.forEach((c) =>
              assert.strictEqual(c.isStatic, c.name === "static_value"),
            );
          }),
      );
              ...
              function checkTable(next) {
                client.metadata.getTable(
                  ...
                  function (err, table) {
                    ...
                    assert.strictEqual(
                      table.columnsByName["smallint_sample"].type.code,
                      types.dataTypes.smallint,
                    );
                    assert.strictEqual(
                      table.columnsByName["tinyint_sample"].type.code,
                      types.dataTypes.tinyint,
                    );
                    assert.strictEqual(
                      table.columnsByName["date_sample"].type.code,
                      types.dataTypes.date,
                    );
                    assert.strictEqual(
                      table.columnsByName["time_sample"].type.code,
              ...
              function checkMetadata(next) {
                client.metadata.getTable(
                  ...
                  function (err, table) {
                    ...
                    assert.deepEqual(
                      udtColumn.type.info.fields.map(function (f) {
                        return f.name;
                      }),
                      ["i", "b", "t", "c"],
                    );
        ...
        function (done) {
          ...
          utils.mapSeries(
            [
              [
                "tbl_nodesync_true",
                { enabled: "true", deadline_target_sec: "86400" },
              ],
                ...
                function (clien, eachNext) {
                  client.metadata.getTable(
                    ...
                    function (err, table) {
                      ...
                      assert.strictEqual(
                        table.columnsByName["new_col1"].type.code,
      ...
      context("with ECMAScript Map and Set", () => {
        ...
        vit(
          ...
          () =>
            client.metadata.getTable(keyspace, "tbl_indexes1").then((table) => {
              assert.ok(table.indexes.length > 0);
              const index = table.indexes.filter(
                (x) => x.name === "map_values_index",
              )[0];
            ...
            function checkMeta(next) {
              client.metadata.getMaterializedView(
                ...
                function (err, view) {
                  ...
                  assert.strictEqual(
                    view.partitionKeys.map((x) => x.name).join(", "),
                    "game, year, month, day",
                  );
                ...
                function (client, eachNext) {
                  client.metadata.getMaterializedView(
                    ...
                    function (err, view) {
                      ...
                      assert.strictEqual(
                        view.clusteringKeys.map((x) => x.name).join(", "),
                        "score, user, day",
                      );
                      helper.assertContains(
            ...
            function checkForNewColumnsInAllView(next) {
              ...
              client.metadata.getMaterializedView(
                ...
                function (err, view) {
                  ...
                  assert.ok(
                    view.columnsByName["last_name"].type.code ===
