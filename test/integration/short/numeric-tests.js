  ...
  context("with numeric values", () => {
    ...
    it("should support setting numeric values using strings", () => {
      const insertQuery = `INSERT INTO tbl_numeric_values
         ...
         (?, ?, ?, ?, ?, ?, ?)`;
      const hints = !prepare
        ? [null, "bigint", "decimal", "double", "float", "varint", "int"]
        : null;
      ...
      return client
        ...
        .then(() =>
          client.execute("SELECT * FROM tbl_numeric_values WHERE id = ?", [id]),
        )
        .then((rs) => {
          const row = rs.first();
    ...
    it("should support setting numeric values using strings for collections", () => {
      ...
      return client
        .execute(insertQuery, params, { prepare, hints })
        .then(() =>
          client.execute("SELECT * FROM tbl_numeric_collections WHERE id = ?", [
            id,
          ]),
        )
        .then((rs) => {
      ...
      Promise.all(
        varintTextValues.map((textValue) => {
          ...
          return client
            .execute(insertQuery, params, { hints, prepare })
            .then(() =>
              client.execute(selectQuery, [textValue, BigInt(0)], {
                hints,
                prepare,
              }),
            )
