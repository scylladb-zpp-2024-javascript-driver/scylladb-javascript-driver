...
describe("custom payload @SERVER_API", function () {
  ...
  describe("using Client#execute(query, params, { prepare: 0 }, callback)", function () {
    vit(
      "2.2",
      "should encode and decode the payload with rows response",
      function (done) {
        const client = newInstance();
        const payload = {
              ...
              client.execute(
                ...
                function (err, result) {
                  ...
                  helper.assertInstanceOf(
                    result.info.customPayload["key1"],
                    Buffer,
                  );
                  assert.strictEqual(
                    result.info.customPayload["key1"].toString(),
                    "val1",
                  );
                  next();
                },
    ...
    vit(
      "2.2",
      "should encode and decode the payload with void response",
      function (done) {
        const client = newInstance();
        const payload = {
            ...
            function insert(next) {
              ...
              client.execute(
                ...
                function (err, result) {
                  ...
                  assert.ok(result.info.customPayload);
                  helper.assertInstanceOf(
                    result.info.customPayload["key2"],
                    Buffer,
                  );
                  assert.strictEqual(
                    result.info.customPayload["key2"].toString(),
                    "val2",
                  );
                  next();
                },
    ...
    vit(
      "2.2",
      "should encode and decode the payload and trace",
      function (done) {
        const client = newInstance();
        const payload = {
        ...
        utils.series(
          [
            ...
            function insert(next) {
              ...
              client.execute(
                query,
                [types.Uuid.random(), "text3"],
                { customPayload: payload, traceQuery: true },
                function (err, result) {
                  assert.ifError(err);
                  assert.ok(result);
                  assert.ok(result.info.customPayload);
                  ...
                  helper.assertInstanceOf(
                    result.info.customPayload["key3"],
                    Buffer,
                  );
                  assert.strictEqual(
                    result.info.customPayload["key3"].toString(),
                    "val3",
                  );
                  next();
                },
          ...
          [
            ...
            function execute(next) {
              client.execute(
                "SELECT key FROM system.local",
                [],
                { prepare: 1, customPayload: payload },
                function (err, result) {
                  assert.ifError(err);
                  assert.ok(result);
                  assert.ok(result.info.customPayload);
                  helper.assertInstanceOf(
                    result.info.customPayload["key-prep1"],
                    Buffer,
                  );
                  assert.strictEqual(
                    result.info.customPayload["key-prep1"].toString(),
                    "val-prep1",
                  );
                  helper.assertInstanceOf(
                    result.info.customPayload["key-prep2"],
                    Buffer,
                  );
                  assert.strictEqual(
                    result.info.customPayload["key-prep2"].toString(),
                    "val-prep2",
                  );
                  next();
                },
  ...
  describe("using Client#batch(queries, { prepare: 0 }, callback)", function () {
          ...
          function executeBatch(next) {
            ...
            client.batch(
              ...
              function (err, result) {
                ...
                assert.ok(result.info.customPayload);
                helper.assertInstanceOf(
                  result.info.customPayload["key-batch1"],
                  Buffer,
                );
                assert.strictEqual(
                  result.info.customPayload["key-batch1"].toString(),
                  "val-batch1",
                );
                next();
              },
    ...
    vit(
      "2.2",
      "should encode and decode the payload with warnings",
      function (done) {
        const client = newInstance();
        const payload = {
            ...
            function executeBatch(next) {
              ...
              client.batch(
                ...
                function (err, result) {
                  ...
                  assert.ok(result.info.customPayload);
                  helper.assertInstanceOf(
                    result.info.customPayload["key-batch2"],
                    Buffer,
                  );
                  assert.strictEqual(
                    result.info.customPayload["key-batch2"].toString(),
                    "val-batch2",
                  );
          ...
          function executeBatch(next) {
            ...
            client.batch(
              ...
              function (err, result) {
                ...
                assert.ok(result.info.customPayload);
                helper.assertInstanceOf(
                  result.info.customPayload["key-batch-prep1"],
                  Buffer,
                );
                assert.strictEqual(
                  result.info.customPayload["key-batch-prep1"].toString(),
                  "val-batch-prep1",
                );
                next();
              },
