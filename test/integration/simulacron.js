...
const simulacronHelper = {
    ...
    start: function (cb) {
        ...
        if (!fs.existsSync(simulacronJarPath)) {
            throw new Error(
                "Simulacron jar not found at: " + simulacronJarPath,
            );
        }
        ...
        const timeout = setTimeout(
            () =>
                cb(
                    new Error(
                        "Timed out while waiting for Simulacron server to start.",
                    ),
                ),
            10000,
        );
    ...
    setup: function (dcs, options) {
        ...
        before((done) => {
            this.start(() =>
                simulacronCluster.register(dcs, clientOptions, done),
            );
        });
        ...
        if (initClient) {
            ...
            client = new Client(
                utils.extend({}, options.clientOptions, baseOptions),
            );
