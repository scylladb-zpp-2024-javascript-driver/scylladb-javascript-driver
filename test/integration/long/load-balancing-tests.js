      ...
      [
        ...
        function testCase(next) {
          const options = utils.deepExtend({}, helper.baseOptions, {
            policies: { loadBalancing: new DCAwareRoundRobinPolicy() },
          });
...
describe("TokenAwarePolicy", function () {
  ...
  describe("with a 3:3 node topology", function () {
    ...
    const client = new Client({
      policies: {
        loadBalancing: new TokenAwarePolicy(new DCAwareRoundRobinPolicy()),
      },
      keyspace: keyspace,
      ...
      utils.times(
        ...
        function (n, timesNext) {
          ...
          client.execute(
            query,
            null,
            { routingKey: utils.allocBufferFromArray([0, 0, 0, id]) },
            function (err, result) {
              assert.ifError(err);
