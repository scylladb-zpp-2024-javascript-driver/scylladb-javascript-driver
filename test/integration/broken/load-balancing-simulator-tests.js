        ...
        context("when getReplicas() returns null", () => {
            it("should yield local hosts", () => {
                client = new Client({
                    ...
                    policies: {
                        loadBalancing:
                            new loadBalancing.DefaultLoadBalancingPolicy({
                                localDc,
                                getReplicas: () => null,
                            }),
                    },
                    ...
                    .then(() =>
                        Promise.all(
                            new Array(16)
                                .fill(null)
                                .map((_) =>
                                    client.execute(query, [], queryOptions),
                                ),
                        ),
                    )
                    .then((results) =>
        ...
        context("when no routing key is specified", () => {
            it("should balance between local hosts", () => {
                ...
                return client
                    ...
                    .then(() => {
                        assert.strictEqual(
                            Object.keys(hostCounts).length,
                            localDcLength,
                        );
                        Object.keys(hostCounts).forEach((address) => {
                            assert.strictEqual(
                                client.hosts.get(address).datacenter,
                                localDc,
                            );
            ...
            return client
                ...
                .then(() => {
                    ...
                    replicas = client.hosts
                        .values()
                        .filter(
                            (h) => h.datacenter !== localDc || counter++ < 3,
                        );
                    localReplicas = replicas.filter(
                        (h) => h.datacenter === localDc,
                    );
                })
                .then(() =>
            ...
            return client
                ...
                .then(() => {
                    ...
                    replicas = client.hosts
                        .values()
                        .filter(
                            (h) => h.datacenter !== localDc || counter++ < 5,
                        );
                    localReplicas = replicas.filter(
                        (h) => h.datacenter === localDc,
                    );
        ...
        it("should not send additional traffic when one node is paused", () => {
            ...
            return client
                ...
                .then(() => {
                    ...
                    assert.strictEqual(
                        localReplicas.filter((h) => h.address === pausedAddress)
                            .length,
                        1,
                    );
                })
                ...
                .then((results) =>
                    assert.ok(
                        results.filter(
                            (r) => r.info.queriedHost === pausedAddress,
                        ).length > 0,
                    ),
                );
        });
