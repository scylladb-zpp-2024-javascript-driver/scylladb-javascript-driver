...
const ConstantSpeculativeExecutionPolicy =
    require("../../../lib/policies/speculative-execution").ConstantSpeculativeExecutionPolicy;
const NoSpeculativeExecutionPolicy =
    require("../../../lib/policies/speculative-execution").NoSpeculativeExecutionPolicy;
const OrderedLoadBalancingPolicy =
    require("../../test-helper").OrderedLoadBalancingPolicy;
...
describe("Client", function () {
    ...
    [
        ...
        {
            title: "with NoSpeculativeExecutionPolicy",
            policy: new NoSpeculativeExecutionPolicy(),
        },
                        ...
                        function executeQuery(next) {
                            client.execute(
                                ...
                                function (err, result) {
                                    assert.ifError(err);
                                    assert.strictEqual(
                                        result.info.speculativeExecutions,
                                        0,
                                        "Should not have been any speculative executions",
                                    );
                                    assert.strictEqual(
                                        Object.keys(result.info.triedHosts)
                                            .length,
                                        1,
                                    );

                                    // Should have got response from 0th node.
                                    const queriedHost = cluster.node(
                    ...
                    function executeQuery(next) {
                        client.execute(
                            ...
                            function (err, result) {
                                ...
                                assert.strictEqual(
                                    Object.keys(result.info.triedHosts).length,
                                    1,
                                );
                        ...
                        client.execute(
                            ...
                            function (err, result) {
                                ...
                                assert.strictEqual(
                                    result.info.speculativeExecutions,
                                    1,
                                    "Should have been one speculative execution",
                                );
                                assert.strictEqual(
                                    Object.keys(result.info.triedHosts).length,
                                    2,
                        ...
                        client.execute(
                            ...
                            function (err, result) {
                                ...
                                assert.strictEqual(
                                    result.info.speculativeExecutions,
                                    2,
                                    "Should have been two speculative executions",
                                );
                                assert.strictEqual(
                                    Object.keys(result.info.triedHosts).length,
                                    3,
                        ...
                        client.execute(
                            ...
                            function (err, result) {
                                ...
                                assert.strictEqual(
                                    result.info.speculativeExecutions,
                                    0,
                                    "Should have been no speculative executions",
                                );
                                assert.strictEqual(
                                    Object.keys(result.info.triedHosts).length,
                                    2,
                                ...
                                const code =
                                    result.info.triedHosts[failureHost].code;
                                assert.strictEqual(
                                    code,
                                    responseErrorCodes.isBootstrapping,
                                    "Expected isBootstrapping",
                                );
                        ...
                        client.execute(
                            ...
                            function (err, result) {
                                ...
                                [1, 2].forEach(function (id) {
                                    const failureHost =
                                        cluster.node(id).address;
                                    const code =
                                        result.info.triedHosts[failureHost]
                                            .code;
                                    assert.strictEqual(
                                        code,
                                        responseErrorCodes.isBootstrapping,
                    ...
                    function executeQuery(next) {
                        client.execute(
                            ...
                            function (err, result) {
                                assert.ok(err);
                                assert.strictEqual(
                                    Object.keys(err.innerErrors).length,
                                    3,
                                );
                                ...
                                [0, 1, 2].forEach(function (id) {
                                    const failureHost =
                                        cluster.node(id).address;
                                    const code =
                                        err.innerErrors[failureHost].code;
                                    assert.strictEqual(
                                        code,
                                        responseErrorCodes.isBootstrapping,
                                        "Expected isBootstrapping",
            ...
            const clientOptions = {
                ...
                policies: {
                    speculativeExecution:
                        new ConstantSpeculativeExecutionPolicy(0, 3),
                },
            };
            const client = new Client(clientOptions);
