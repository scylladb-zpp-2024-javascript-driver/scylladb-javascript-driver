    ...
    [getClientMetrics, getDefaultMetrics].forEach((factory) => {
        context(`with ${factory().constructor.name}`, () => {
            ...
            it("should not retry non-idempotent queries", () => {
                ...
                return client
                    ...
                    .catch((err) => {
                        ...
                        assert.strictEqual(
                            client.metrics.successfulResponse,
                            undefined,
                        );
            ...
            it("should not retry syntax errors", () => {
                ...
                return client
                    .execute(queries.syntaxError.value, [], {
                        isIdempotent: true,
                    })
                    .catch((err) => {
                        catchCalled = true;
                        ...
                        assert.strictEqual(
                            client.options.policies.retry.called,
                            0,
                        );
                    })
                    .then(() => assert.strictEqual(catchCalled, true));
            ...
            it("should not retry when a speculative execution previously completed", () =>
                client
                    ...
                    .then((rs) => {
                        ...
                        assert.strictEqual(
                            client.metrics.clientTimeoutError,
                            undefined,
                        );
                        assert.strictEqual(
                            client.metrics.successfulResponse,
                        ...
                        assert.strictEqual(
                            client.metrics.speculativeExecution,
                            1,
                        );
                        assert.strictEqual(
                            client.metrics.clientTimeoutRetry,
                            undefined,
                        );
                        assert.strictEqual(client.metrics.response.length, 1);
                    })
            ...
            it("should use the retry policy on overloaded errors", () =>
                client
                    ...
                    .then((rs) => {
                        ...
                        assert.strictEqual(
                            client.options.policies.retry.requestError,
                            1,
                        );
                    }));
            ...
            it("should use the retry policy for unavailable", () =>
                client
                    ...
                    .then((rs) => {
                        ...
                        assert.strictEqual(
                            client.options.policies.retry.unavailable,
                            1,
                        );
            ...
            it("should use the retry policy for server read timeout", () =>
                client
                    ...
                    .then((rs) => {
                        ...
                        assert.strictEqual(
                            client.options.policies.retry.readTimeout,
                            1,
                        );
                        assert.strictEqual(client.metrics.response.length, 2);
                    }));
            ...
            it("should use the retry policy for server write timeout", () =>
                client
                    ...
                    .then((rs) => {
                        ...
                        assert.strictEqual(
                            client.options.policies.retry.writeTimeout,
                            1,
                        );
                        assert.strictEqual(client.metrics.response.length, 2);
                    }));
