    ...
    context("with a 3-node simulated cluster", function () {
            ...
            return client
                ...
                .then(() => {
                    assert.ok(
                        client.hosts
                            .values()
                            .filter((h) => h.getResponseCount() > 1).length > 0,
                    );
                })
                .then(
                    () =>
                        new Promise((resolve) =>
                            setTimeout(
                                resolve,
                                healthResponseCountInterval * 2 + 20,
                            ),
                        ),
                )
                .then(() => {
                    ...
                    assert.strictEqual(
                        client.hosts
                            .values()
                            .filter((h) => h.getResponseCount() !== 0).length,
                        0,
                    );
                });
        });
        ...
        it("should retry on the same host when defined by the retry policy", () => {
            ...
            const client = new Client({
                ...
                socketOptions: {
                    defunctReadTimeoutThreshold: Number.MAX_SAFE_INTEGER,
                },
            });
        ...
        it("should reconnect in the background when hosts are back online", async () => {
            const client = newInstance(cluster, {
                policies: {
                    reconnection:
                        new policies.reconnection.ConstantReconnectionPolicy(
                            200,
                        ),
                },
            ...
            hosts.forEach((h) =>
                assert.strictEqual(1, h.pool.connections.length),
            );

            await Promise.all(
                [0, 1, 2].map((n) =>
                    promiseUtils.fromCallback((cb) =>
                        cluster.dc(0).node(n).stop(cb),
                    ),
                ),
            );
        ...
        it("should connect when first contact point is down", () => {
            ...
            return promiseUtils
                ...
                .then(() =>
                    assert.strictEqual(
                        client.controlConnection.host.address
                            .split(":")[0]
                            .split(".")[3],
        ...
        it("should stop attempting to reconnect to down after shutdown", () => {
            ...
            return client
                ...
                .then(
                    () =>
                        new Promise((r) =>
                            setTimeout(r, reconnectionDelay * 2),
                        ),
                )
        ...
        it("should log the module versions on first connect only", function (done) {
            ...
            const versionLogRE =
                /^Connecting to cluster using .+ version (.+)$/;
        ...
        it("should create all connections eventually when warmup is false", async () => {
            ...
            const client = newInstance(cluster, {
                pooling: {
                    warmup: false,
                    coreConnectionsPerHost: {
                        [distance.local]: connectionsLength,
                    },
                },
            });
            ...
            await helper.wait.until(
                () =>
                    hosts.reduce(
                        (count, h) => count + h.pool.connections.length,
                        0,
                    ) ===
        ...
        [
            ...
            it(`should reject requests immediately after shutdown ${item.name}`, async () => {
                ...
                // Note: this will likely no longer work in our driver
                // await client.shut down();
    ...
    context(
        ...
        () => {
            ...
            it("should not attempt to make a connection on a new node that is on an ignored/remote DC", () => {
                ...
                const node1Address = cluster
                    .dc(1)
                    .node(1)
                    .address.split(":")[0];
                const node2Address = cluster
                    .dc(1)
                    .node(2)
                    .address.split(":")[0];
                ...
                return client
                    ...
                    .then(() => {
                        ...
                        client.hosts.forEach((host) => {
                            const expectedConnections =
                                lbp.getDistance(host) === types.distance.local
                                    ? 1
                                    : 0;
                            assert.strictEqual(
                    ...
                    .then(() =>
                        ...
                        helper.setIntervalUntilPromise(
                            () =>
                                initialControlConnectionHost.pool.connections
                                    .length > 0,
                            500,
                            40,
                        ),
                    ...
                    .then(() => {
                        ...
                        assert.strictEqual(
                            client.hosts.get(newNodeAddress).pool.connections
                                .length,
                            0,
                        );
                    });
            ...
            /* [true, false].forEach((warmup) => {
                it(`should create connections to remote nodes in the background when warmup is ${warmup}`, async () => {
                    ...
                    localHostsHosts.forEach((h) =>
                        assert.deepStrictEqual(
                            client.getState().getOpenConnections(h),
                            2,
                        ),
                    );
                    ...
                    await helper.wait.until(
                        () =>
                            client.getState().getOpenConnections(remoteHost) ===
                            1,
                    );
