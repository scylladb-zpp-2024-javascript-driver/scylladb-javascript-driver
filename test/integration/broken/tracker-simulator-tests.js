    ...
    describe("RequestTracker", function () {
        context("when using execute()", () => {
            [true, false].forEach((prepare) => {
                ...
                it(
                    "should be called when a valid response is obtained for " +
                        requestType +
                        " request",
                    () => {
                        const requestTracker = sinon.createStubInstance(
                        ...
                        return client
                            .connect()
                            .then(() =>
                                client.execute(query, parameters, { prepare }),
                            )
                            .then(() =>
                                verifyResponse(
                                    requestTracker,
        ...
        context("when using batch()", () => {
            [true, false].forEach((prepare) => {
                ...
                it(
                    "should be called when a valid response is obtained for BATCH request containing " +
                        requestType,
                    () => {
                        const requestTracker = sinon.createStubInstance(
                            tracker.RequestTracker,
        ...
        it("should be called when client is being shutdown", () => {
            ...
            return client
                .connect()
                .then(() =>
                    assert.strictEqual(requestTracker.shutdown.callCount, 0),
                )
                .then(() => client.shutdown())
                .then(() =>
                    assert.strictEqual(requestTracker.shutdown.callCount, 1),
                );
        });
    });

    describe("RequestLogger", function () {
        const logger = new tracker.RequestLogger({
            slowThreshold: 50,
            requestSizeThreshold: 1000,
        });
        const client = new Client({
            contactPoints: [simulacron.startingIp],
            localDataCenter: "dc1",
            requestTracker: logger,
        });
        ...
        context("when using execute()", () => {
            it("should log slow queries", () => {
                ...
                return client
                    .execute(queryDelayed, [id], { prepare: true })
                    .then(() => {
            ...
            [true, false].forEach((logged) => {
                it(
                    ...
                    () => {
                        const query = "INSERT INTO table1 (id) VALUES (?)";
                        const queries = [
                            { query, params: [utils.stringRepeat("a", 2000)] },
                        ];

                        return client
                            .batch(queries, { prepare: true, logged })
                            .then(() => {
                                assert.strictEqual(slowMessages.length, 0);
                                assert.strictEqual(largeMessages.length, 1);
                                assert.match(
                                    largeMessages[0],
                                    /Request exceeded length/,
                                );
                                if (logged) {
                                    assert.include(
                                        largeMessages[0],
                                        ": LOGGED BATCH w/ 1 queries (" + query,
                                    );
                                } else {
                                    assert.include(
                                        largeMessages[0],
                                        ": BATCH w/ 1 queries (" + query,
                                    );
                                }
                            });
                    },
...
function verifyCommon(info, query, parameters) {
    ...
    if (Array.isArray(query)) {
        assert.deepEqual(
            info[1].map((x) => ({ query: x.query, params: x.params })),
            query,
        );
    } else {
