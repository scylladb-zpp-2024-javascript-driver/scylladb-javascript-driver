    ...
    describe("Preparing statements on nodes behavior", function () {
        ...
        beforeEach((done) => {
            ...
            client = new Client({
                ...
                localDataCenter: "dc1",
                policies: {
                    reconnection: new reconnection.ConstantReconnectionPolicy(
                        100,
                    ),
                ...
                function (err, result) {
                    ...
                    utils.eachSeries(
                        ...
                        function (host, next) {
                            sCluster
                                .node(host.address)
                                .getLogs(function (err, logs) {
                                    assert.ifError(err);
                                    let prepareQuery;
                                    for (let i = 0; i < logs.length; i++) {
                                        const queryLog = logs[i];
                                        if (
                                            queryLog.type === "PREPARE" &&
                                    ...
                                    if (!prepareQuery) {
                                        assert.fail(
                                            "Query no prepared on all hosts",
                                        );
                                    }
                                    next();
                                });
        ...
        it("should re-prepare query when host go UP again", function (done) {
            ...
            const logMessages = [];
            const logRegex =
                /Re-preparing all queries on host [\d.:]+ before setting it as UP/i;
                    ...
                    function verifyPrepareQueryOnLastNode(next) {
                        sCluster
                            ...
                            .getLogs(function (err, logs) {
                                ...
                                if (!prepareQuery) {
                                    assert.fail(
                                        "Query no prepared on restarted host",
                                    );
                                }
