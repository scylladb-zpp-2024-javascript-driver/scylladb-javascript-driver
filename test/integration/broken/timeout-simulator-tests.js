...
const queryDelayedOnPrepareOnAllNodes =
    "INSERT INTO paused_prepare_on_all_nodes";
...
describe("client read timeouts", function () {
    ...
    beforeEach((done) =>
        simulacronCluster.node(0).prime(
            {
                when: {
                    request: "batch",
                    queries: [{ query: queryDelayedOnNode0 }],
                },
        ...
        simulacronCluster.prime(
            {
                when: {
                    request: "batch",
                    queries: [{ query: queryDelayedOnAllNodes }],
                },
                then: { result: "success", delay_in_ms: 1000 },
            },
            done,
    ...
    it(
        "should do nothing else than waiting when socketOptions.readTimeout is not set",
        testNodeUsedAsCoordinator(0),
    );

    it(
        "should move to next host by default for simple queries",
        testNodeUsedAsCoordinator(1, { readTimeout }),
    );

    it(
        "should move to next host for prepared queries executions",
        testNodeUsedAsCoordinator(1, { readTimeout, prepare: true }),
    );

    it(
        "should move to next host for prepared requests",
        testNodeUsedAsCoordinator(1, { readTimeout, prepare: true }),
        queryDelayedOnPrepareOnNode0,
    );
    ...
    it("should throw error when isIdempotent is false", async () => {
        ...
        await helper.assertThrowsAsync(
            client.execute(queryDelayedOnNode0, [], {
                isIdempotent: false,
                readTimeout,
            }),
    ...
    it("defunct the connection when the threshold passed", async () => {
        ...
        const client = newInstance({
            socketOptions: { defunctReadTimeoutThreshold, readTimeout },
        });
        await client.connect();
        let hostDown = null;
        ...
        assert.doesNotHaveAllKeys(coordinators, [
            simulacronCluster.node(0).address,
        ]);
    ...
    describe("with prepared batches", function () {
        ...
        it("should retry when preparing multiple queries", async () => {
            ...
            await client.connect();
            const rs = await client.batch([{ query: queryDelayedOnNode0 }], {
                prepare: true,
            });
        ...
        it("should produce a NoHostAvailableError when execution timed out on all hosts", async () => {
            ...
            const err = await helper.assertThrowsAsync(
                client.batch([{ query: queryDelayedOnAllNodes }], {
                    prepare: true,
                }),
        ...
        it("should produce a NoHostAvailableError when prepare tried and timed out on all hosts", async () => {
            ...
            // Throws on prepare
            await helper.assertThrowsAsync(
                client.execute(queryDelayedOnPrepareOnAllNodes, ["a"], {
                    prepare: true,
                }),
                errors.NoHostAvailableError,
            );
            ...
            await helper.assertThrowsAsync(
                client.batch(
                    [{ query: queryDelayedOnPrepareOnAllNodes, params: ["a"] }],
                    { prepare: true },
                ),
                errors.NoHostAvailableError,
            );
...
function testNodeUsedAsCoordinator(nodeIndex, queryOptions, query) {
    ...
    return async () => {
        ...
        const rs = await client.execute(
            query,
            [],
            Object.assign({ prepare }, queryOptions),
        );
        assert.strictEqual(
            rs.info.queriedHost,
