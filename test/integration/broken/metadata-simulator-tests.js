...
describe("Metadata", function () {
    ...
    describe("#getTrace()", function () {
        ...
        it("should set consistency level", function (done) {
            ...
            const sessionQuery = util.format(
                "SELECT * FROM system_traces.sessions WHERE session_id=%s",
                traceId,
            );
            const eventsQuery = util.format(
                ...
                rows: [
                    {
                        ...
                        started_at:
                            new types.LocalTime.now().getTotalNanoseconds(),
                    },
                ],
                column_types: {
                    session_id: "uuid",
                            ...
                            const verifyQuery = function (
                                ...
                                for (let i = 0; i < logsToSearch.length; i++) {
                                    ...
                                    if (
                                        log.type === "QUERY" &&
                                        log.query === query &&
                                        log.consistency_level === consistency
                                    ) {
                                        return true;
                                    }
                                }
    ...
    describe("#compareSchemaVersions()", function () {
        ...
        before(() =>
            client
                .execute("SELECT schema_version FROM system.peers")
