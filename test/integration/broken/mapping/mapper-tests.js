...
describe("Mapper", function () {
    ...
    describe("#batch()", () => {
        ...
        it("should execute a batch containing updates to multiple tables from the same doc", () => {
            ...
            return mapper
                .batch([videoMapper.batching.update(doc)])
                .then((result) => {
                ...
                .then(() =>
                    mapperTestHelper.getVideoRows(
                        client,
                        doc,
                        "videoid, userid, added_date, name",
                    ),
                )
                .then((rows) => {
        ...
        it("should execute a batch containing updates/inserts/deletes to multiple tables from the same doc", () => {
            ...
            return mapperTestHelper
                .insertVideoRows(client, docs[2])
                .then(() =>
                    mapper.batch([
                        videoMapper.batching.update(docs[0]),
                        videoMapper.batching.insert(docs[1]),
                        videoMapper.batching.remove(docs[2], {
                            fields: ["id", "userId", "addedDate"],
                        }),
                    ]),
                )
                .then(() =>
                ...
                .then((results) => {
                    ...
                    results.forEach((rows, index) => {
                        ...
                        if (index !== 2) {
                            rows.forEach((row) =>
                                assertRowMatchesDoc(row, docs[index]),
                            );
                        } else {
            ...
            return mapper
                ...
                .then((results) => {
                    ...
                    videoRows.forEach((row) =>
                        assertRowMatchesDoc(row, videoDoc),
                    );
        ...
        it("should adapt results of a LWT operation", () => {
            ...
            return client
                ...
                .then(() =>
                    mapper.batch([
                        userMapper.batching.update(doc, {
        ...
        it("should support updating counters in batch", () => {
            const doc1 = {
                id: Uuid.random(),
                ratingCounter: q.incr(1),
                ratingTotal: q.incr(5),
            };
            const doc2 = {
                id: Uuid.random(),
                ratingCounter: q.incr(2),
                ratingTotal: q.incr(8),
            };
            ...
            return mapper
                ...
                .then(() =>
                    Promise.all(
                        [doc1.id, doc2.id].map((id) =>
                            client.execute(selectQuery, [id], {
                                prepare: true,
                            }),
                        ),
                    ),
    ...
    describe("#forModel()", () => {
        it("should be able to query on a Client instance not connected", () => {
            ...
            const items = [
                ["insert", { userid }],
                ["remove", { userid }],
                ["update", { userid, email: "info@example.com" }],
                ["find", { userid }],
                ["findAll"],
            ];
            ...
            return Promise.all(
                items.map((item) => {
                    const client = new Client(
                        utils.extend(
                            { keyspace: mapperTestHelper.keyspace },
                            helper.baseOptions,
                        ),
                    );
