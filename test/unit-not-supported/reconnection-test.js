    ...
    describe("#getOptions()", () => {
        it("should return a Map with the delay", () => {
            helper.assertMapEqual(
                new reconnection.ConstantReconnectionPolicy(123).getOptions(),
                new Map([["delay", 123]]),
            );
        });
    });
...
describe("ExponentialReconnectionPolicy", function () {
    it("should exponentially grow the wait time", function () {
        ...
        const policy = new reconnection.ExponentialReconnectionPolicy(
            baseDelay,
            maxDelay,
            false,
        );
        ...
        assert.ok(
            value >= baseDelay && value < baseDelay * 1.15,
        ...
        for (let i = 1; i < 8; i++) {
            ...
            assert.ok(
                value >= expectedDelay * 0.85 && value < expectedDelay * 1.15,
                `Value is incorrect: ${value}`,
            );
        }
        ...
        for (let j = 8; j < 30; j++) {
            value = schedule.next().value;
            assert.ok(
                value >= maxDelay * 0.85 && value < maxDelay,
                `Value is incorrect: ${value}`,
            );
        }
    });
    ...
    it("should return independent schedules", () => {
        ...
        assert.ok(
            value >= baseDelay && value < baseDelay * 1.15,
            `Value is unexpected: ${value}`,
        );
        value = schedule1.next().value;
        assert.ok(
            value >= 2 * 0.85 * baseDelay,
            `Value is unexpected: ${value}`,
        );
    ...
    it("should support starting with no delay", () => {
        ...
        value = schedule1.next().value;
        assert.ok(
            value >= 0.85 * baseDelay && value < baseDelay * 1.15,
            `Value is unexpected: ${value}`,
        );
        value = schedule1.next().value;
        assert.ok(
