...
describe("ControlConnection", function () {
    describe("constructor", function () {
        it("should create a new metadata instance", function () {
            const cc = new ControlConnection(
                clientOptions.extend({}, helper.baseOptions),
            );
    ...
    describe("#init()", function () {
        ...
        async function testResolution(
            ...
            assert.deepStrictEqual(
                state.connectionAttempts.sort(),
                expectedHosts.sort(),
            );
            const resolvedContactPoints = cc.getResolvedContactPoints();
            assert.deepStrictEqual(
                resolvedContactPoints.get(contactPointHostName),
                expectedResolved,
            );
        }
        ...
        it("should resolve IPv4 and IPv6 addresses, default host (localhost) and port", () => {
            const ControlConnectionMock = proxyquire(
                "../../lib/control-connection",
                {
                    dns: {
            ...
            const ControlConnectionMock = proxyquire(
                "../../lib/control-connection",
                {
                    dns: {
                        ...
                        resolve6: function (name, cb) {
                            throw new Error(
                                "IPv6 resolution errors should be ignored",
                            );
                        },
                        lookup: function () {
                            throw new Error("dns.lookup() should not be used");
    ...
    describe("#getAddressForPeerHost()", function () {
        it("should handle null, 0.0.0.0 and valid addresses", async () => {
            ...
            assert.strictEqual(
                await cc.getAddressForPeerHost(
                    { rpc_address: getInet([1, 2, 3, 4]), peer },
                    9042,
                ),
                "1.2.3.4:9042",
            );
            assert.strictEqual(
                await cc.getAddressForPeerHost(
                    { rpc_address: getInet([0, 0, 0, 0]), peer },
                    9001,
                ),
                "100.100.100.100:9001",
            );

            assert.strictEqual(
                await cc.getAddressForPeerHost(
                    { rpc_address: null, peer },
                    9042,
                ),
                null,
            );
        ...
        it("should call the AddressTranslator", async () => {
            ...
            let address = null;
            let port = null;
            options.policies.addressResolution =
                policies.defaultAddressTranslator();
            options.policies.addressResolution.translate = function (
                addr,
                p,
                cb,
            ) {
            ...
            assert.strictEqual(
                await cc.getAddressForPeerHost(row, 9055),
                "5.2.3.4:9055",
            );
    ...
    describe("#setPeersInfo()", function () {
        ...
        it("should set the host datacenter and cassandra version", async () => {
            ...
            assert.ok(cc.hosts.get("5.4.3.2:9042"));
            assert.strictEqual(
                cc.hosts.get("5.4.3.2:9042").datacenter,
                "dc100",
            );
            assert.strictEqual(
                cc.hosts.get("5.4.3.2:9042").cassandraVersion,
                "2.1.4",
            );
            assert.ok(cc.hosts.get("9.8.7.6:9042"));
            assert.strictEqual(
                cc.hosts.get("9.8.7.6:9042").datacenter,
                "dc101",
            );
            assert.strictEqual(
                cc.hosts.get("9.8.7.6:9042").cassandraVersion,
                "2.1.4",
            );
        });
    ...
    describe("#refresh()", function () {
        it("should schedule reconnection when it cant borrow a connection", async () => {
            ...
            lbp.newQueryPlan = function (ks, o, cb) {
                ...
                return cb(
                    null,
                    [lbp.hosts.values()[1], lbp.hosts.values()[0]][
                        Symbol.iterator
                    ](),
                );
            };
            ...
            const cc = newInstance(
                {
                    contactPoints: ["::1"],
                    policies: { loadBalancing: lbp, reconnection: rp },
                },
                getContext({
                    state: state,
                    queryResults: {
                        peers: [
                            {
                                rpc_address:
                                    types.InetAddress.fromString("::2"),
                            },
                        ],
                    },
                    failBorrow: [-1, 1],
...
function getContext(options) {
    ...
    return {
        borrowHostConnection: function (h) {
            ...
            return (state.connection = getFakeConnection(
                h.address,
