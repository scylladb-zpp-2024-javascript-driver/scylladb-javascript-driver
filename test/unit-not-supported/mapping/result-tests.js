...
const expected = [
    { id: 1, name: "name1", adapted: true },
    { id: 2, name: "name2", adapted: true },
];
...
describe("Result", () => {
    describe("#toArray()", () => {
        ...
        it("should return empty array when is a VOID result", () => {
            const result = new Result(
                { columns: [], rows: undefined, rowLength: undefined },
                null,
                () => {},
            );
    ...
    describe("@@iterator", () => {
        it("should support iteration", () => {
            assert.strictEqual(
                typeof Result.prototype[Symbol.iterator],
                "function",
            );
            const arr = [];
    ...
    describe("[util.inspect.custom]()", () => {
        ...
        it("should return empty array representation when is LWT result", () => {
            const result = new Result(
                { columns: [{ name: "[applied]" }], rows: [{}], rowLength: 1 },
                null,
                () => {},
            );
            assert.strictEqual(util.inspect(result), util.inspect([]));
...
function getResult(columns, rows, rowAdapter) {
    ...
    return new Result(
        { columns, rows, rowLength: rows.length },
        null,
        rowAdapter,
    );
}
