...
describe("Tree", function () {
    ...
    describe("#getOrCreate()", () => {
        ...
        it("should create new branches when not all keys match", () => {
            const tree = new Tree();
            assert.strictEqual(
                tree.getOrCreate(["a", "b", "c"], () => 1),
                1,
            );
            assert.strictEqual(
                tree.getOrCreate(["a", "b", "d"], () => 2),
                2,
            );
            assert.strictEqual(
                tree.getOrCreate(["a", "b", "d", "e", "f"], () => 3),
                3,
            );
            assert.strictEqual(
                tree.getOrCreate(["r", "s", "t", "u"], () => 4),
                4,
            );
            assert.strictEqual(
                tree.getOrCreate(["r", "s", "x"], () => 5),
                5,
            );
            assert.strictEqual(
                tree.getOrCreate(["j", "k", "l"], () => 6),
                6,
            );
            assert.strictEqual(
                tree.getOrCreate(["j"], () => 7),
                7,
            );
            assert.strictEqual(
                tree.getOrCreate(["j", "k"], () => 8),
                8,
            );
            ...
            assert.strictEqual(
                tree.getOrCreate(["a", "b", "d"], () => false),
                2,
            );
            assert.strictEqual(
                tree.getOrCreate(["a", "b", "d", "e", "f"], () => false),
                3,
            );
            assert.strictEqual(
                tree.getOrCreate(["j"], () => false),
                7,
            );
            assert.strictEqual(
                tree.getOrCreate(["j", "k"], () => false),
                8,
            );
            assert.strictEqual(
                tree.getOrCreate(["j", "k", "l"], () => false),
                6,
            );
