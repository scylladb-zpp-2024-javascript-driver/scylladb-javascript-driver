...
describe("Connection", function () {
    describe("constructor", function () {
        it("should parse host endpoint into address and port", function () {
            const values = [
                ...
                ["::1:1234", "::1", "1234"],
                [
                    "aabb::eeff:11:2233:4455:6677:8899:9999",
    ...
    describe("#sendStream()", function () {
        ...
        it("should reset the timeout after each new request", function () {
            ...
            for (let i = 0; i < 4; i++) {
                clock.tick(10);
                c.sendStream(
                    new requests.QueryRequest("QUERY" + i),
                    null,
                    utils.noop,
                );
            }
        ...
        it("should set the request timeout", function (done) {
            ...
            c.sendStream(
                new requests.QueryRequest("QUERY1"),
                getExecOptions({ readTimeout: 20 }),
                function (err) {
    ...
    describe("#close", function () {
        it("should allow socket.close event to be emitted before calling back when connected", function (done) {
            ...
            const ConnectionInjected = proxyquire("../../lib/connection", {
                net: { Socket },
            });

            const c = new ConnectionInjected(
                "127.0.0.1:9042",
                9042,
