    ...
    describe("#eventReceived()", function () {
        ...
        it("should process queue immediately when processNow is true", async () => {
            ...
            const promise = Promise.all([
                debouncer
                    .eventReceived(
                        { handler: getHandler("A"), keyspace: "ks2" },
                        false,
                    )
                    .then(spy),
                debouncer
                    .eventReceived(
                        { handler: getHandler("B"), keyspace: "ks1" },
                        false,
                    )
                    .then(spy),
                ...
                debouncer
                    .eventReceived(
                        { handler: getHandler("C"), keyspace: "ks1" },
                        true,
                    )
                    .then(spy),
                debouncer
                    .eventReceived(
                        { handler: getHandler("D"), keyspace: "ks1" },
                        false,
                    )
                    .then(spy),
            ]);
