  ...
  return profileManager.getOrCreateGraphOptions(
    ...
    function createDefaultOptions() {
      ...
      if (baseOptions.executeAs) {
        options.customPayload[proxyExecuteKey] = utils.allocBufferFromString(
          baseOptions.executeAs,
        );
      }

      if (options.graphName) {
        options.customPayload[payloadKeys.name] = utils.allocBufferFromString(
          options.graphName,
        );
      }
...
class GraphExecutionOptions extends DefaultExecutionOptions {
  ...
  constructor(
    ...
    this._graphLanguage =
      queryOptions.graphLanguage || this._defaultGraphOptions.graphLanguage;
  }
