...
class CloudOptions {
    ...
    setAuthProvider(username, password) {
        ...
        if (
            ...
            !(this.clientOptions.authProvider instanceof NoAuthProvider)
        ) {
            // There is an auth provider set by the user
            return;
        }

        this.clientOptions.authProvider = new DsePlainTextAuthProvider(
            username,
            password,
        );
    }
...
async function parseZipFile(cloudOptions) {
    ...
    const zipEntries = new Map(
        zip.getEntries().map((e) => [e.entryName, e.getData()]),
    );
    ...
    if (!config["host"] || !config["port"]) {
        throw new TypeError(
            "Config file must include host and port information",
        );
    }
...
function getMetadataServiceInfo(cloudOptions, callback) {
    ...
    if (!matches || matches.length !== 4) {
        throw new TypeError(
            "url should be composed of host, port number and path, without scheme",
        );
    }
    ...
    const req = https.get(requestOptions, (res) => {
        ...
        utils.log(
            "verbose",
            `Connected to metadata service with SSL/TLS protocol ${res.socket.getProtocol()}`,
            {},
            cloudOptions,
        );
        ...
        res.on("data", (chunk) => (data += chunk.toString())).on("end", () => {
            ...
            try {
                ...
                if (!message || !message["contact_info"]) {
                    throw new TypeError(
                        "contact_info should be defined in response",
                    );
                }
            } catch (err) {
                return callback(
                    getServiceRequestError(err, requestOptions, true),
                );
            }
            ...
            cloudOptions.clientOptions.contactPoints =
                contactInfo["contact_points"];
            cloudOptions.clientOptions.localDataCenter =
                contactInfo["local_dc"];
    ...
    req.on("error", (err) =>
        callback(getServiceRequestError(err, requestOptions)),
    );
    ...
    req.setTimeout(
        cloudOptions.clientOptions.socketOptions.connectTimeout,
        () => req.abort(),
    );
}
...
function checkServerIdentity(cert, sniAddress) {
    ...
    if (!valid) {
        const error = new Error(
            `Host: ${hostName} is not cert's CN/altnames: ${cn} / ${altNames}`,
        );
