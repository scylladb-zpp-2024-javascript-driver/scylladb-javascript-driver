 ...
 * @returns {any}
 */
function getCqlObject(field, options) {
    ...
    switch (type) {
        ...
        case rust.CqlType.Set:
            ...
            for (let i = 0; i < fields.length; i++)
                res.push(getCqlObject(fields[i], options));
            return res;
        ...
        case rust.CqlType.Map:
            ...
            res = {};
            for (let i = 0; i < fields.length; i++)
                res[getCqlObject(fields[i][0], options)] = getCqlObject(
                    fields[i][1],
                    options,
                );
            return res;
        ...
        case rust.CqlType.Varint:
            return encodingOptions && encodingOptions.useBigIntAsVarint
                ? decodeVarintAsBigInt(field.getVarint())
                : Integer.fromBuffer(field.getVarint());
        default:
 ...
 */
function getRowsFromResultsWrapper(result, options) {
    ...
    for (let i = 0; i < rustRows.length; i++) {
        ...
        for (let j = 0; j < cols.length; j++) {
            ...
            collectedRow[colNames[j]] = getCqlObject(cols[j], options);
        }
