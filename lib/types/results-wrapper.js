...
function getCqlObject(field) {
    ...
    switch (type) {
        ...
        case rust.CqlType.Map:
            let mapKeys = field.getMap(true);
            let mapValues = field.getMap(false);
            let map = {};
            for (let i = 0; i < mapKeys.length; i++)
                map[getCqlObject(mapKeys[i])] = getCqlObject(mapValues[i]);
            return map;
        ...
        case rust.CqlType.Tuple:
            let tupleFields = field.getTuple();
            tupleFields = tupleFields.map((value) => getCqlObject(value));
            return new Tuple(...tupleFields);
        default:
