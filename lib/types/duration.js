...
function Duration(months, days, nanoseconds) {
  ...
  this.internal = rust.Duration.new(Math.trunc(months), Math.trunc(days), Math.trunc(ns1), Math.trunc(ns2), Math.trunc(maxInt32));
}
...
Duration.prototype.toBuffer = function () {
  ...
  const lengthMonths = VIntCoding.writeVInt(Long.fromNumber(data[0]), reusableBuffers.months);
  const lengthDays = VIntCoding.writeVInt(Long.fromNumber(data[1]), reusableBuffers.days);
