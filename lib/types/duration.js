...
function Duration(months, days, nanoseconds) {
  if (nanoseconds instanceof Long)
    this.internal = rust.DurationWrapper.new(months, days, long_to_bigint(nanoseconds));
  else if (isNumber(nanoseconds))
    this.internal = rust.DurationWrapper.new(months, days, BigInt(nanoseconds));
  else
    throw new TypeError(`Invalid nanosecond argument type: ${typeof(nanoseconds)}`);
}
...
Duration.prototype.equals = function (other) {
  ...
  return this.internal.months === other.internal.months &&
...
Duration.prototype.toBuffer = function () {
  ...
  const lengthMonths = VIntCoding.writeVInt(Long.fromNumber(this.internal.months), reusableBuffers.months);
  const lengthDays = VIntCoding.writeVInt(Long.fromNumber(this.internal.days), reusableBuffers.days);
  const lengthNanoseconds = VIntCoding.writeVInt(nanoseconds, reusableBuffers.nanoseconds);
...
Duration.prototype.toString = function () {
  ...
  if (this.internal.months < 0 || this.internal.days < 0 || nanoseconds.isNegative()) {
    value = '-';
  }
