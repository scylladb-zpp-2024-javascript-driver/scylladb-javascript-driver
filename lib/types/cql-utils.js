...
const {
    ...
    encodeVarintFromBigInt,
    encodeVarintFromInteger,
} = require("../new-utils");
 ...
 */
function encodeListLike(values, subtype, options) {
    ...
    for (let i = 0; i < values.length; i++) {
        ...
        res.push(wrapValueBasedOnType(subtype, values[i], options));
    }
    return res;
}
 ...
 */
function encodeMap(value, parentType, options) {
 ...
 */
function getWrapped(type, value, options) {
    ...
    return rust.MaybeUnsetQueryParameterWrapper.fromNonNullNonUnsetValue(
        wrapValueBasedOnType(type, value, options),
    );
}
 ...
 */
function wrapValueBasedOnType(type, value, options) {
    let encodedElement;
    let tmpElement;
    let encodingOptions = options && options.encoding;
    ...
    switch (type.baseType) {
        ...
        case rust.CqlType.List:
            encodedElement = encodeListLike(
                value,
                type.getFirstSupportType(),
                options,
            );
        ...
        case rust.CqlType.Map:
            ...
            encodedElement = encodeMap(value, type, options);
        ...
        case rust.CqlType.Varint:
            encodedElement =
                encodingOptions && encodingOptions.useBigIntAsVarint
                    ? encodeVarintFromBigInt(value)
                    : encodeVarintFromInteger(value);
            return rust.QueryParameterWrapper.fromVarint(encodedElement);
        default:
 ...
 * @returns {Array<rust.QueryParameterWrapper>}
 */
function parseParams(expectedTypes, params, options, allowGuessing) {
    ...
    for (let i = 0; i < params.length; i++) {
        ...
        res.push(getWrapped(type, params[i], options));
    }
    return res;
}
