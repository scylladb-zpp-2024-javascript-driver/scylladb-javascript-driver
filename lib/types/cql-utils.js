...
function encodeListLike(values, subtype) {
    ...
    for (let i = 0; i < values.length; i++) {
        ...
        res.push(getWrappedType(subtype, values[i]));
    }
    return res;
}
...
function encodeMap(value, parentType) {
    ...
    for (const key in value) {
        ...
        const val = value[key];
        res.push([
            getWrappedType(keySubtype || guessTypeChecked(key), key),
            getWrappedType(valueSubtype || guessTypeChecked(val), val),
        ]);
    }
    return res;
}
...
function getWrapped(type, value) {
    ...
    return rust.MaybeUnsetQueryParameterWrapper.fromType(
        getWrappedType(type, value),
    );
}
 ...
 */
function getWrappedType(type, value) {
    let encodedElement;
