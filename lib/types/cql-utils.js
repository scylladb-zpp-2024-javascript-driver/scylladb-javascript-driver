...
/**
 * Ensures the value isn't one of may ways to express null value
 * @param {string} message Message of error, in case value is "null"
 ...
 */
function encodeList(values, subtype) {
...
function encodeMap(value, parentType) {
    const keySubtype = parentType.getSupportType(1);
    const valueSubtype = parentType.getSupportType(2);
    let res = [];
 ...
 * @returns {rust.Tuple}
 */
function encodeTuple(value, complexType) {
    let res = [];
    for (let i = 0; i < value.length; i++) {
        res.push(getWrapped(value.get(i), complexType.getSupportType(1)));
        complexType = complexType.getSupportType(2);
    }
    return res;
}
...
function getWrapped(type, value) {
    ...
    switch (type.baseType) {
        ...
        case rust.CqlType.Set:
            ...
            let encodedSet = encodeList(value, type.getSupportType(1));
        ...
        case rust.CqlType.List:
            let encodedList = encodeList(value, type.getSupportType(1));
        ...
        case rust.CqlType.Tuple:
            encodedTuple = encodeTuple(value, type);
            return QueryParameterWrapper.fromTuple(encodedTuple, type);
