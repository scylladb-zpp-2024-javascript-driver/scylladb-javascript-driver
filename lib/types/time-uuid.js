...
class TimeUuid extends Uuid {
    ...
    getDatePrecision() {
        ...
        let timeHigh = 0;
        timeHigh |= (buffer[4] & 0xff) << 8;
        timeHigh |= buffer[5] & 0xff;
        timeHigh |= (buffer[6] & 0x0f) << 24;
        timeHigh |= (buffer[7] & 0xff) << 16;
     ...
     */
    getNodeId() {
        return this.getBuffer().slice(10);
    }
     ...
     */
    getClockId() {
        return this.getBuffer().slice(8, 10);
    }
     ...
     */
    getNodeIdString() {
        return this.getBuffer().slice(10).toString("ascii");
    }
}
