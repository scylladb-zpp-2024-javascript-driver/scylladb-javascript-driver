...
class Connection extends events.EventEmitter {
  ...
  startup(callback) {
    if (this._checkingVersion) {
      this.log(
        "info",
        "Trying to use protocol version 0x" + this.protocolVersion.toString(16),
      );
    }
      ...
      if (err && self._checkingVersion) {
        ...
        if (invalidProtocol) {
          ...
          if (lowerVersion === self.protocolVersion) {
            lowerVersion = types.protocolVersion.getLowerSupported(
              self.protocolVersion,
            );
          } else if (
            !types.protocolVersion.isSupported(self.protocol.version)
          ) {
  ...
  startAuthenticating(authenticatorName, callback) {
    if (!this.options.authProvider) {
      return callback(
        new errors.AuthenticationError("Authentication provider not set"),
      );
    }
    ...
    this.sendStream(request, null, function authResponseCallback(err, result) {
      ...
      callback(
        new errors.DriverInternalError(
          "Unexpected response from Cassandra: " + util.inspect(result),
        ),
      );
    });
  }
  ...
  handleResult(header, err, result) {
    ...
    this.log(
      "verbose",
      "Received frame #" + streamId + " from " + this.endpointFriendlyName,
    );
