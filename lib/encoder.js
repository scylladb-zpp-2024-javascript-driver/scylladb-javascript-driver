...
function defineInstanceMembers() {
  ...
  this.setProtocolVersion = function (value) {
    ...
    if (
      !types.protocolVersion.uses4BytesCollectionLength(this.protocolVersion)
    ) {
  ...
  this.encodeTime = function (value) {
    ...
    if (!(value instanceof types.LocalTime)) {
      throw new TypeError(
        "Expected String/LocalTime, obtained " + util.inspect(originalValue),
      );
    }
    return value.toBuffer();
  };
  ...
  this._encodeVarintFromBigInt = function (value) {
    ...
    if (typeof value !== "bigint") {
      throw new TypeError(
        "Not a valid varint, expected BigInt, obtained " + util.inspect(value),
      );
    }
  ...
  this.encodeBlob = function (value) {
    if (!(value instanceof Buffer)) {
      throw new TypeError(
        "Not a valid blob, expected Buffer obtained " + util.inspect(value),
      );
    }
    return value;
  };
    ...
    while (++i < length) {
      switch (typesString[i]) {
        ...
        case "(":
          if (
            nested === 0 &&
            typesString.indexOf(complexTypeNames.collection, startIndex) ===
              startIndex
          ) {
...
Encoder.prototype.encode = function (value, typeInfo) {
  if (value === undefined) {
    value =
      this.encodingOptions.useUndefinedAsUnset && this.protocolVersion >= 4
        ? types.unset
        : null;
  }
