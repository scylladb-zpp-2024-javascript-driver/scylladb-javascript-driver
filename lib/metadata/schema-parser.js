...
class SchemaParserV1 extends SchemaParser {
  ...
  async _parseTableOrView(tableInfo, tableRow, columnRows, indexRows, virtual) {
    ...
    tableInfo.populateCacheOnFlush =
      tableRow["populate_io_cache_on_flush"] || tableInfo.populateCacheOnFlush;
    tableInfo.memtableFlushPeriod =
      tableRow["memtable_flush_period_in_ms"] || tableInfo.memtableFlushPeriod;
    ...
    if (!tableInfo.isCompact) {
      ...
      tableInfo.isCompact =
        ...
        (!comparator.hasCollections &&
          tableInfo.clusteringKeys.length !== comparator.types.length - 1);
    }
  ...
  async _parseAggregate(row) {
    ...
    aggregate.argumentTypes = (row["argument_types"] || utils.emptyArray).map(
