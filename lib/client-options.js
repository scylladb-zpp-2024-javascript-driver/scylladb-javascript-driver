...
function validateCloudOptions(options) {
  ...
  if (
    !(
      ...
      (typeof URL !== "undefined" && bundle instanceof URL)
    )
  ) {
    throw new TypeError(
      "secureConnectBundle in cloud options must be of type string",
    );
  }

  if (options.contactPoints) {
    throw new TypeError(
      "Contact points can not be defined when cloud settings are provided",
    );
  }

  if (options.sslOptions) {
    throw new TypeError(
      "SSL options can not be defined when cloud settings are provided",
    );
  }
}
...
function validateProtocolOptions(protocolOptions) {
  ...
  if (
    version &&
    (typeof version !== "number" || !types.protocolVersion.isSupported(version))
  ) {
    throw new TypeError(
      util.format(
...
function validateEncodingOptions(encodingOptions) {
  ...
  if (
    (encodingOptions.useBigIntAsLong || encodingOptions.useBigIntAsVarint) &&
