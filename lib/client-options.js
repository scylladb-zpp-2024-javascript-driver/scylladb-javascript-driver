...
function setRustOptions(options) {
    ...
    // - have a address resolution policy, that works as a policy, but is not inherited from the default policy (duck typing),
    ...
    rustOptions.addressTranslationPolicy =
        options.policies && options.policies.addressResolution
            ? options.policies.addressResolution.isDefaultPolicy
                ? options.policies.addressResolution.isDefaultPolicy()
