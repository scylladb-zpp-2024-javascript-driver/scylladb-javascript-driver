...
function setRustOptions(options) {
    ...
    // We can either:
    // - not have policies,
    // - have policies, but not have address resolution policy,
    // - have an address resolution policy, that works as a policy, but is not inherited from the default policy (duck typing),
    // - have a proper address resolution policy.
    // In the first two cases, we treat that as lack of policy, and the third case as custom policy.
    rustOptions.addressTranslationPolicy =
        options.policies && options.policies.addressResolution
            ? options.policies.addressResolution.whichDefaultPolicy()
                ? options.policies.addressResolution.whichDefaultPolicy()
                : rust.AddressTranslatorPolicies.CustomPolicy
            : null;
