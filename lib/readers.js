...
class FrameReader {
  ...
  readMetadata(kind) {
    ...
    const meta = {};
    if (
      types.protocolVersion.supportsResultMetadataId(this.header.version) &&
      isPrepared
    ) {
    ...
    if (
      types.protocolVersion.supportsPreparedPartitionKey(this.header.version) &&
      isPrepared
    ) {
      //read the pk columns
    ...
    switch (code) {
      ...
      case types.responseErrorCodes.readFailure:
        ...
        if (code === types.responseErrorCodes.readFailure) {
          if (
            types.protocolVersion.supportsFailureReasonMap(this.header.version)
          ) {
