...
Client.prototype._connect = async function () {
  ...
  try {
    this.rust_client = await SessionWrapper.createSession(this.rust_options);
...
Client.prototype.execute = function (query, params, options, callback) {
  // This method acts as a wrapper for the async method _execute(), replaced by _rustyExecute()
  ...
  try {
    ...
    return promiseUtils.optionalCallback(
      this._rustyExecute(query, params, execOptions),
      callback,
    );
  } catch (err) {
...
/**
 * Call the rust driver to execute the query. The given query is always prepared.
 * @param {string} query
 * @param {Array} params
 * @param {ExecutionOptions} execOptions
 * @returns {Promise<ResultSet>}
 * @private
 */
Client.prototype._rustyExecute = async function (query, params) {
  if (
  ...
  if (!this.connected) {
    // Check this logic and decide if it's needed. Probably toDo while implementing (better) connection
  ...
  return new Promise(async (resolve, reject) => {
    try {
      let statement = await this.rust_client.prepareStatement(query);
      let parsed_params = parseParams(statement, params);
      let rusty = await this.rust_client.querySimple(statement, parsed_params);
