...
class Client extends events.EventEmitter {
    ...
    constructor(options) {
        super();
        let rustOptions = SessionOptions.empty();
    ...
    async _connect() {
        ...
        try {
            this.rustClient = await SessionWrapper.createSession(
                this.rustOptions,
            );
     ...
     * @ignore
     */
    async _waitForSchemaAgreement(connection) {
     ...
     * @private
     */
    async _execute(query, params, execOptions) {
        const version = this.controlConnection.protocolVersion;
        ...
        if (!execOptions.isPrepared()) {
            return new Promise(async (resolve, reject) => {
                let rusty = await this.rustClient.queryUnpagedNoValues(query);
                let x = new ResultSet(rusty);
                resolve(x);
            });
            request = await this._createQueryRequest(
                query,
                execOptions,
                params,
            );
     ...
     * @private
     */
    async _createBatchRequest(queryItems, info) {
     ...
     * @private
     */
    async _setRoutingInfo(execOptions, params, meta) {
