...
const { SessionOptions, SessionWrapper, createBatch } = require("../index.js");
...
class Client extends events.EventEmitter {
    ...
    async #rustyBatch(queries, options) {
        ...
        for (let i = 0; i < queries.length; i++) {
            ...
            if (typeof element === "string") {
                preparedQueries.push(
                    await this.rust_client.prepareStatement(element),
                );
                parametersRows.push([]);
            } else {
                let query = queries[i].query;
                let params = queries[i].params;
                let prepared = await this.rust_client.prepareStatement(query);
        ...
        let wrappedResult = await this.rust_client.queryBatch(
            batch,
            parametersRows,
        );
