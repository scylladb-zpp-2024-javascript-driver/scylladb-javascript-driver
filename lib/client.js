...
class Client extends events.EventEmitter {
    ...
    /**
     * Manually create final execution options.
     * As this ensures rust option wrapper is created,
     * this allows to reduce a number of round trips required
     * for executing repeated queries with the same options.
    ...
    createOptions(options) {
        if (
            options instanceof ExecOptions.ExecutionOptions &&
            options.final === true
        ) {
            return options;
        }
    ...
    async #rustyBatch(queries, options) {
        ...
        for (let i = 0; i < queries.length; i++) {
            ...
            if (shouldBePrepared) {
                query = preparedCache.getElement(query);
                if (!query) {
                    let newQuery =
                        await this.rustClient.prepareStatement(query);
                    preparedCache.storeElement(query, newQuery);
                    query = newQuery;
                }
        ...
        try {
            ...
            if (execOptions.isPrepared()) {
                ...
                if (!params) {
                    ...
                    let expectedTypes = convertHints(
