...
class Client extends events.EventEmitter {
    ...
    execute(query, params, options, callback) {
        ...
        try {
            const execOptions = executionOptions.DefaultExecutionOptions.create(
                options,
                this,
            );
    ...
    eachRow(query, params, options, rowCallback, callback) {
        ...
        let execOptions;
        try {
            execOptions = executionOptions.DefaultExecutionOptions.create(
                options,
                this,
                rowCallback,
            );
    ...
    async #rustyBatch(queries, options) {
        ...
        let prepared = execOptions.isPrepared();
        let allQueries = [];
        let parametersRows = [];
        ...
        for (let i = 0; i < queries.length; i++) {
            ...
            if (prepared) {
                ...
                types = convertHints(execOptions.getHints() || []);
            }

            if (items) {
                items = parseParams(types, items, prepared === false);
            }
            allQueries.push(query);
            parametersRows.push(items);
        }

        let batch = prepared
            ? rust.createPreparedBatch(allQueries)
            : rust.createUnpreparedBatch(allQueries);
     ...
     * @param {string} query
     * @param {Array} params
     * @param {executionOptions.ExecutionOptions} execOptions
     * @returns {Promise<ResultSet>}
     * @private
     */
    ...
    async #executePromise(resolve, reject, query, params, execOptions) {
        try {
            ...
            if (execOptions.isPrepared()) {
                ...
                result = await this.rustClient.queryUnpagedNoValues(query);
            } else {
