...
class Client extends events.EventEmitter {
    ...
    execute(query, params, options, callback) {
        ...
        try {
            const execOptions = executionOptions.DefaultExecutionOptions.create(options, this);
    ...
    async #rustyBatch(queries, options) {
        ...
        const _execOptions = executionOptions.DefaultExecutionOptions.create(options, this);
    ...
    async #executePromise(resolve, reject, query, params, execOptions) {
        try {
            ...
            if (execOptions.isPrepared()) {
                ...
                let expectedTypes = execOptions.getHints() || [];
                let parsedParams = parseParams(
                    expectedTypes,
                    params,
                    true
                );
                result = await this.rustClient.queryUnpaged(query, parsedParams);
            }
