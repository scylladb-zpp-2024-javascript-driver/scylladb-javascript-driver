...
class MappingHandler {
  ...
  createInsertQueries(docKeys, doc, docInfo) {
    ...
    return this._client
      .connect()
      .then(() =>
        ObjectSelector.getForInsert(this._client, this.info, propertiesInfo),
      )
      .then((tables) => {
      ...
      return self._client
        ...
        .then((rs) => {
          ...
          if (resultAdapter === undefined) {
            const resultAdapterInfo = ResultMapper.getCustomQueryAdapter(
              self.info,
              rs,
            );
  ...
  _setSingleExecutor(cacheItem, queryInfo) {
    ...
    cacheItem.executor = function singleExecutor(
      ...
      return self._client
        ...
        .then(
          (rs) =>
            new Result(rs, self.info, ResultMapper.getMutationAdapter(rs)),
        );
    };
