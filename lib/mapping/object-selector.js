  ...
  static getForSelect(
    ...
    return Promise.all(
      ...
      for (let i = 0; i < tables.length; i++) {
        ...
        if (allPKsDefined) {
          if (
            keysAreIncluded(table.clusteringKeys, propertiesInfo) !==
            keyMatches.all
          ) {
        ...
        const containsAllFields = fieldsInfo.reduce(
          (acc, p) => acc && table.columnsByName[p.columnName] !== undefined,
          true,
        );
      ...
      const filteredTables = tables.filter((table, i) => {
        ...
        return (
          propertiesInfo.length ===
            table.partitionKeys.length + staticColumns && staticColumns > 0
        );
      });
    ...
    return Promise.all(
      ...
      const filteredTables = tables.filter((table, i) => {
        ...
        const applicableColumns = propertiesInfo.reduce(
          (acc, p) =>
            acc + (table.columnsByName[p.columnName] !== undefined ? 1 : 0),
          0,
        );
...
function keysAreIncluded(keys, propertiesInfo) {
  ...
  const matches = propertiesInfo.reduce(
    (acc, p) => acc + (contains(keys, (k) => p.columnName === k.name) ? 1 : 0),
    0,
  );
  if (matches === 0) {
