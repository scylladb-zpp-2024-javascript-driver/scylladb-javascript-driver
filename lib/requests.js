...
class ExecuteRequest extends Request {
  ...
  write(encoder, streamId) {
    ...
    if (
      types.protocolVersion.supportsResultMetadataId(encoder.protocolVersion)
    ) {
...
class PrepareRequest extends Request {
  ...
  write(encoder, streamId) {
    ...
    if (types.protocolVersion.supportsPrepareFlags(encoder.protocolVersion)) {
      const flags =
        this.keyspace &&
        types.protocolVersion.supportsKeyspaceInRequest(encoder.protocolVersion)
  ...
  write(encoder, streamId) {
    ...
    this.queries.forEach(function eachQuery(item, i) {
      ...
      params.forEach((param, index) =>
        frameWriter.writeBytes(encoder.encode(param, getParamType(index))),
      );
    }, this);
