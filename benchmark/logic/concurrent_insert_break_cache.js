...
async.series(
    [
        ...
        function dropTable(next) {
            const query =
                "DROP TABLE IF EXISTS benchmarks.basic2";
            client.execute(query, next);
        },
        function createTable(next) {
            const query =
                "CREATE TABLE benchmarks.basic2 (id uuid, val text, PRIMARY KEY(id))";
            client.execute(query, next);
        },
        async function insert(next) {
            let allParameters = [];
            for (let i = 0; i < iterCnt; i++) {
                const myUUID = cassandra.types.Uuid.random();
                allParameters.push({
                    query: `INSERT INTO benchmarks.basic2 (id, val) VALUES (?, '${myUUID.toString()[0]}${myUUID.toString()[1]}')`,
                    params: [myUUID]
                });
            }
        ...
        async function test(next) {
            const query = "SELECT COUNT(1) FROM benchmarks.basic2 USING TIMEOUT 120s;";
