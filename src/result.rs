...
impl QueryResultWrapper {
    ...
    pub fn get_columns_specs(&self) -> Vec<MetaColumnWrapper> {
        ...
        .map(|f| MetaColumnWrapper {
            ...
            type_code: map_column_type_to_cql_type(f.typ()).base_type,
        })
        .collect()
    }
...
pub(crate) fn map_column_type_to_cql_type(typ: &ColumnType) -> ComplexType {
    match typ {
        ColumnType::Custom(_) => panic!("No support fro custom type"),
        ...
        ColumnType::List(t) => {
            ComplexType::one_support(CqlType::List, Some(map_column_type_to_cql_type(t)))
        }
        ColumnType::Map(t, v) => ComplexType::two_support(
            CqlType::Map,
            Some(map_column_type_to_cql_type(t)),
            Some(map_column_type_to_cql_type(v)),
        ),
        ColumnType::Set(t) => {
            ComplexType::one_support(CqlType::Set, Some(map_column_type_to_cql_type(t)))
        }
