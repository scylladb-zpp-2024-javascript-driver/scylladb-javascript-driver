...
/// or you can add a new error type. Note that `with_custom_error!` macro
...
pub(crate) trait IntoConvertedError {
    type Final;
    fn into_converted_error(self) -> Self::Final;
}

impl<T, E> IntoConvertedError for Result<T, E>
where
    ConvertedError: From<E>,
{
    type Final = Result<T, ConvertedError>;
    fn into_converted_error(self) -> Result<T, ConvertedError> {
...
pub(crate) async fn with_custom_error_async<T, C>(code: C) -> JsResult<T>
where
    C: AsyncFnOnce() -> Result<T, ConvertedError>,
{
    let c = code().await;
    c.into()
}
...
pub(crate) fn with_custom_error_sync<T, C>(code: C) -> JsResult<T>
where
    C: FnOnce() -> Result<T, ConvertedError>,
{
    let c = code();
    c.into()
}
